...
exports.extractX509Data = exports.extractIssuerData = exports.extractPrivateKeyData = exports.extractPrivateKeyAndCertificateFromPkcs12 = exports.getHash = exports.sign = void 0;
...
const getBancoCentralPkcs12PrivateKey = (pkcs8ShroudedKeyBags) => {
    ...
    if (!privateKeyBag) {
        throw new errors_1.UnsuportedPkcs12Error("No private key bag found in BCE .p12");
    }
    const privateKey = privateKeyBag.key;
    if (!privateKey) {
        throw new errors_1.UnsuportedPkcs12Error("No valid key found in BCE .p12");
    }
    return privateKey;
};
const extractPrivateKeyAndCertificateFromPkcs12 = (pkcs12RawData, password = '') => {
    var _a, _b, _c;
    ...
    if (!certBag) {
        throw new errors_1.UnsuportedPkcs12Error("No certificate found in PKCS#12");
    }
    if (!pkcs8ShroudedKeyBags || Object.keys(pkcs8ShroudedKeyBags).length === 0) {
        throw new errors_1.UnsuportedPkcs12Error("No private key found in PKCS#12");
    }
    ...
    if (/banco central/i.test(friendlyName) || /eci|bce/i.test(friendlyName)) {
        ...
        privateKey = (firstKeyBag === null || firstKeyBag === void 0 ? void 0 : firstKeyBag.key) ? firstKeyBag.key : null;
    }
    if (!privateKey) {
        throw new errors_1.UnsuportedPkcs12Error("Couldn't find private key");
    }
...
exports.extractPrivateKeyAndCertificateFromPkcs12 = extractPrivateKeyAndCertificateFromPkcs12;
const normalizeIssuerAttributeShortName = (shortName) => {
    switch (shortName) {
        case 'E':
...
const extractIssuerData = (certificate) => {
...
exports.extractIssuerData = extractIssuerData;
const extractX509Data = (certificate) => {
...
exports.extractX509Data = extractX509Data;
const extractPrivateKeyData = (privateKey) => {
    const modulus = forge.util.encode64(forge.util.hexToBytes(privateKey.n.toString(16)));
    const exponent = forge.util.encode64(forge.util.hexToBytes(privateKey.e.toString(16)));
    return {
        modulus,
        exponent
    };
};
exports.extractPrivateKeyData = extractPrivateKeyData;
