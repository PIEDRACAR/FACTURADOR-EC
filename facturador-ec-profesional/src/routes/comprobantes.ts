...
export async function registrarRutasComprobantes(app: FastifyInstance) {
  ...
  app.post<{ Params: { id: string }; Body: { email?: string } }>('/comprobantes/:id/reenviar-email', async (request, reply) => {
    ...
    try {
      ...
      const asunto = `Reenvío de factura ${secuencial} — ${emisor?.nombre_comercial || emisor?.razon_social || 'Facturador EC'}`;
