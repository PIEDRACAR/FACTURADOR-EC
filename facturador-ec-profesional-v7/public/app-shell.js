(function () {
  ...
  function renderShell(negocio) {
    ...
    const active = path;
    const navLink = ([href,ic,label]) => `<a class="${active===href?'active':''}" href="${link(href)}" data-label="${esc(label)}"><span class="ec-menu-icon">${icon(ic)}</span><span class="ec-menu-label">${esc(label)}</span></a>`;
    ...
    aside.innerHTML = `
      ...
      <nav class="ec-menu" aria-label="Navegación principal">
        ...
        <div class="ec-menu-section">Gestión</div>${modulo.slice(5,11).map(navLink).join('')}
