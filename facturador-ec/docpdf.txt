...
export async function obtenerDocumentoSriPdf(id: string): Promise<{buffer: Buffer; filename: string; tipo: string; secuencial: string; email: string; estado: string}> {
  ...
  renderHeader(doc,{emisor,tipoNombre,sec,estado:String(d.estado),esAutorizado,logoBase64,logoMime,clave:d.clave_acceso,autorizacion:d.numero_autorizacion,fechaAut:d.fecha_autorizacion,fechaEmision:datos.fechaEmision??d.created_at,ambiente:datos.ambiente});
  doc.y=162;
...
function renderHeader(doc:any,o:any){
  const w=doc.page.width-PAGE.left-PAGE.right;
  const hasLogo=!!(o.logoBase64&&['image/png','image/jpeg'].includes(o.logoMime));
  doc.rect(0,0,doc.page.width,10).fill(C.navy);

  // Encabezado inspirado en la composición del RIDE publicado por el SRI:
  // emisor a la izquierda y bloque del comprobante/autorización a la derecha.
  doc.roundedRect(PAGE.left,22,270,122,4).lineWidth(.8).strokeColor(C.line).stroke();
  if(hasLogo){ try { doc.image(Buffer.from(o.logoBase64,'base64'),PAGE.left+10,31,{fit:[72,45],valign:'center'}); } catch {} }
  const ex=hasLogo?PAGE.left+91:PAGE.left+10;
  doc.font('Helvetica-Bold').fontSize(11).fillColor(C.navy).text(text(o.emisor?.razon_social||o.emisor?.nombre_comercial||'CONTSERTRIB FACTURACIÓN'),ex,31,{width:166,height:28});
  doc.font('Helvetica-Bold').fontSize(7.2).fillColor(C.ink).text(`R.U.C.: ${text(o.emisor?.ruc)}`,ex,63,{width:166});
  doc.font('Helvetica').fontSize(6.8).fillColor(C.muted).text(text(o.emisor?.direccion_matriz),ex,75,{width:166,height:28});
  doc.font('Helvetica-Bold').fontSize(6.7).fillColor(C.navy).text(o.esAutorizado?'DOCUMENTO ELECTRÓNICO':'BORRADOR PARA REVISIÓN',PAGE.left+10,116,{width:250,align:'center'});

  const bx=318,bw=doc.page.width-PAGE.right-bx;
  doc.roundedRect(bx,22,bw,122,4).lineWidth(1).strokeColor(C.navy2).stroke();
  doc.font('Helvetica-Bold').fontSize(9.2).fillColor(C.navy).text(o.tipoNombre,bx+10,31,{width:bw-20,align:'center'});
  doc.font('Helvetica-Bold').fontSize(11).fillColor(C.ink).text(`No. ${text(o.sec)}`,bx+10,49,{width:bw-20,align:'center'});
  doc.font('Helvetica-Bold').fontSize(6.7).fillColor(C.muted).text('NÚMERO DE AUTORIZACIÓN',bx+10,68,{width:bw-20,align:'center'});
  doc.font('Helvetica').fontSize(6.6).fillColor(C.ink).text(text(o.esAutorizado?o.autorizacion:'Pendiente de autorización'),bx+10,78,{width:bw-20,align:'center',height:17,ellipsis:true});
  doc.font('Helvetica-Bold').fontSize(6.4).fillColor(C.muted).text('FECHA Y HORA DE AUTORIZACIÓN',bx+10,98,{width:bw-20,align:'center'});
  doc.font('Helvetica').fontSize(6.5).fillColor(C.ink).text(o.esAutorizado?fechaEc(o.fechaAut):'—',bx+10,108,{width:bw-20,align:'center'});
  doc.font('Helvetica-Bold').fontSize(6.4).fillColor(C.muted).text(`AMBIENTE: ${String(o.ambiente||'PRUEBAS').toUpperCase()}   ·   EMISIÓN: NORMAL`,bx+8,126,{width:bw-16,align:'center'});

  if(!o.esAutorizado){
    doc.roundedRect(PAGE.left+8,128,254,12,2).fill('#FEF3C7');
    doc.fillColor(C.amber).font('Helvetica-Bold').fontSize(6.4).text('BORRADOR · SIN VALIDEZ TRIBUTARIA',PAGE.left+10,132,{width:250,align:'center'});
  }
  doc.moveTo(PAGE.left,153).lineTo(doc.page.width-PAGE.right,153).strokeColor(C.line).lineWidth(.8).stroke();
}

function renderReceptor(doc:any,d:any,created:any){
  heading(doc,'Identificación del adquirente / sujeto');
  const x=PAGE.left,w=doc.page.width-PAGE.left-PAGE.right,y=doc.y;
  doc.roundedRect(x,y,w,49,3).lineWidth(.6).strokeColor(C.line).stroke();
  ...
  labelValue(doc,'Dirección / domicilio',d.direccionComprador??d.direccionProveedor,c1,y+29,345);
  labelValue(doc,'Correo electrónico',d.correoElectronico,c3,y+29,w-370);
  doc.y=y+56;
}
