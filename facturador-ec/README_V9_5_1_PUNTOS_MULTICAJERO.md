...
## v9.5.2 — correcciones de despliegue sin regresión
- Corregido error TypeScript TS2345 en `guardarPermisosUsuario` mediante un type guard para validar permisos del catálogo.
- Corregido error TS2304 en POS: consulta de configuración del RUC proveedor usa `body.emisorId`.
- Optimizado `railway.json`: Railpack ya instala dependencias, por lo que el build ejecuta `npm run build` y evita repetir `npm ci`.
- No se eliminaron módulos ni funcionalidades de v9.5.1.
