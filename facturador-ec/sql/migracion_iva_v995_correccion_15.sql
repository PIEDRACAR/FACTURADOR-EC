...
update catalogo_iva_sri
set descripcion='IVA tarifa general 15%', tarifa=15, tipo='TARIFA',
    base_legal='Parametrización vigente de CONTSERTRIB; validar siempre contra la normativa/ficha SRI vigente.',
    activo=true, updated_at=now()
where codigo_porcentaje='4' and fecha_desde=date '2026-01-01';

insert into catalogo_iva_sri
  (codigo_porcentaje, descripcion, tarifa, tipo, fecha_desde, fecha_hasta, base_legal, activo)
select '4','IVA tarifa general 15%',15,'TARIFA',date '2026-01-01',null,
       'Parametrización vigente de CONTSERTRIB; validar siempre contra la normativa/ficha SRI vigente.',true
where not exists (
  select 1 from catalogo_iva_sri where codigo_porcentaje='4' and fecha_desde=date '2026-01-01'
);
...
select v.codigo_porcentaje,v.descripcion,v.tarifa,v.tipo,date '2026-01-01',v.base_legal,true
from (values
  ('0','IVA tarifa 0%',0::numeric,'TARIFA','Catálogo SRI'),
  ('6','No objeto de IVA',0::numeric,'NO_OBJETO','Catálogo SRI'),
  ('7','Exento de IVA',0::numeric,'EXENTO','Catálogo SRI')
) v(codigo_porcentaje,descripcion,tarifa,tipo,base_legal)
where not exists (
  select 1 from catalogo_iva_sri c
  where c.codigo_porcentaje=v.codigo_porcentaje and c.fecha_desde=date '2026-01-01'
);
