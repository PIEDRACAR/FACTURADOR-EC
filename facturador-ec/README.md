...
Endpoints adicionales (todavía sin pantalla propia — se consumen desde
`/` y `/cuentas-por-pagar`/`/cuentas-por-cobrar`, o directamente por API):

- `GET /reportes/ventas-periodo` — total vendido y cantidad de facturas,
  día por día, en un rango de fechas.
- `GET /reportes/cuentas-por-cobrar` / `GET /reportes/cuentas-por-pagar` —
  resumen: total pendiente, total vencido, cantidad de cuentas vencidas.
- `GET /reportes/inventario-valorizado` — cuánto vale el stock actual, al
  costo promedio de cada producto (stock × costo promedio).
...
- Reportería adicional expuesta solo por API, sin pantalla propia todavía:
  ventas por período, inventario valorizado (ver "Reportes" arriba). El
  resumen de cuentas por cobrar/pagar sí se ve en `/` y en cada pantalla
  de cuentas.
