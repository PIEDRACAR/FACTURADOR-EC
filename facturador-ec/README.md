...
## v9.9.11 — Contabilidad completa + POS + exportaciones

- Menú lateral: Caja se renderiza una sola vez en Finanzas y contabilidad.
- POS: reorganización compacta; Punto de emisión y Carrito aprovechan la fila superior; Productos/Cliente quedan a la izquierda y Pagos/Resumen a la derecha.
- Protección de páginas: se eliminó la inyección duplicada de `app.css`/`app-shell.js`; todo usa 9.9.11 y los recursos ya presentes no se vuelven a insertar. Esto corrige el origen de solicitudes duplicadas de versiones antiguas.
- Contabilidad: consultas desacopladas de relaciones PostgREST anidadas para evitar `Could not find a relationship ... in the schema cache`.
- Nueva migración `sql/migracion_contabilidad_completa_v9910.sql`: FK explícitas para asientos/líneas, índices, nómina, períodos de nómina y cuentas contables de nómina.
- Contabilidad UI: Balance general, Estado de resultados, Flujo de efectivo, Libro diario, Libro mayor, Balanza, Plan de cuentas, Nómina, Asiento manual y Cierres.
- Nómina parametrizada: IESS personal 9,45%, patronal 11,15% y SBU 2026 de USD 482 como parámetros iniciales; provisiones de décimo tercero, décimo cuarto, vacaciones y fondo de reserva.
- Proformas: PDF completo con encabezado empresarial, cliente, detalle, totales, validez y leyenda de que la proforma no es comprobante SRI; botón PDF en el listado.
- Exportación PDF de reportes: nunca genera una hoja visualmente vacía cuando no existen registros; muestra mensaje informativo.


## v9.9.11 — Corrección de migración contable
La migración contable es autocontenida: crea primero `plan_cuentas_contables`, `asientos_contables`, `asiento_lineas_contables`, `periodos_contables` y `control_migraciones` cuando no existan, y después crea las relaciones requeridas por PostgREST. Esto corrige el error `42P01 relation periodos_contables does not exist` cuando se ejecuta la migración nueva sobre una base que no tenía aplicadas las migraciones contables anteriores.


## v9.9.12 — Reparación de esquema contable

Incluye `sql/migracion_contabilidad_completa_v9912_reparacion.sql`, diseñada para instalaciones donde las tablas contables ya existían con un esquema anterior y `CREATE TABLE IF NOT EXISTS` no agregó columnas como `emisor_id`. La migración es aditiva, agrega columnas/relaciones/índices faltantes y solicita a PostgREST recargar el schema cache.


## v9.9.13 — Contabilidad/Nómina + POS + flujo operativo

- POS reorganizado: punto de emisión y carrito en la fila superior; escáner integrado al bloque de productos; diseño compacto y responsive.
- `Caja` queda una sola vez en el menú.
- PDF de proforma resuelve automáticamente el emisor desde la proforma, evitando el 409 de selección de negocio cuando el usuario ya trabaja dentro de una empresa.
- Plantilla de productos accesible sin selección de negocio.
- Contabilidad: Balance general, Estado de resultados, Flujo de efectivo, Diario, Mayor, Balance de comprobación, Plan de cuentas, Nómina y Cierres.
- Plan de cuentas editable y exportable/importable por Excel. Se precarga una base alineada con el catálogo publicado por la Superintendencia de Compañías para información financiera; se mantienen las cuentas operativas internas necesarias para automatización.
- Nómina: fondos de reserva mensual/acumulado, décimos mensualizados/acumulados, extensión de cónyuge parametrizable (3,41% por defecto), provisiones, contabilización y registro de pagos.
- Sincronización/reconciliación de ventas autorizadas existentes hacia contabilidad. Las nuevas facturas autorizadas intentan contabilizarse automáticamente.
- Migración `sql/migracion_contabilidad_completa_v9913.sql` aditiva e idempotente.


## v9.9.14
- negocio activo persistente en sesión; elimina 409 de selección repetida.
- POS nivelado: carrito/pagos/resumen bajan a filas alineadas con cliente.
- nota de crédito preserva datos de sustento al agregar detalle y precarga detalle/valores.
- reparación de unicidad de plan, clientes, proveedores y nómina.
- empleados sin dependencia de ON CONFLICT; extensión cónyuge 3,41%, modalidades de décimos/fondo de reserva.
- reportes CxC/CxP muestran detalle y proveedores existentes aparecen aunque no tengan movimientos.
- ATS permite descargar e importar XML.
- fechas/hora visibles con zona America/Guayaquil.
