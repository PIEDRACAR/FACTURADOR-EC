...
## v9.9.82 — Promociones comerciales administrables

Se incorporó un catálogo global de promociones SaaS administrado exclusivamente por ROOT. La promoción pública de prueba queda desactivada inicialmente, por lo que el registro público ya no concede automáticamente 30 días gratis.

Desde **Panel Maestro → Promociones comerciales** ROOT puede activar/desactivar la promoción, definir días (0–365), texto público y vigencia por fechas. Los clientes que ya tienen suscripciones activas no son modificados al desactivar una promoción.

Migración: `sql/migracion_v9982_promociones_maestro.sql`.


## v9.9.83 — SaaS / PayPhone / Resend definitivo

- Generación automática de link PayPhone cuando no existe promoción de prueba vigente.
- Reenvío manual desde Panel Maestro conservado.
- Correo ROOT y correo del cliente independientes del procesamiento del pago.
- Activación automática e idempotente tras notificación PayPhone aprobada.
- Reintento de activación si el pago quedó aprobado pero la activación no terminó.
- El fallo de correo ROOT no convierte un pago correctamente procesado en error PayPhone.
- Eliminado el `upsert` de puntos de emisión que dependía de una restricción UNIQUE potencialmente inexistente.
- Dirección matriz obligatoria para activación pagada; no se usa texto ficticio.
- Endpoint de diagnóstico: `/pagos/payphone/health`.
- Migración: `sql/migracion_v9983_saas_payphone_resend_definitivo.sql`.
