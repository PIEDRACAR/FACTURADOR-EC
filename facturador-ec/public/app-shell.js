(function () {
  ...
  function renderShell(negocio) {
    ...
    function pintarResultados(data,q){
      const groups=[['Facturas',data.facturas||[],x=>`<a href="/reportes?emisorId=${encodeURIComponent(emisorId)}&q=${encodeURIComponent(q)}"><strong>Factura ${esc(x.numero||x.secuencial||'')}</strong><small>${esc(x.cliente||'')} · $${Number(x.total||0).toFixed(2)}</small></a>`],['Clientes',data.clientes||[],x=>`<a href="/clientes-admin?emisorId=${encodeURIComponent(emisorId)}"><strong>${esc(x.razon_social||'Cliente')}</strong><small>${esc(x.identificacion||'')}</small></a>`],['Productos',data.productos||[],x=>`<a href="/productos-admin?emisorId=${encodeURIComponent(emisorId)}"><strong>${esc(x.descripcion||'Producto')}</strong><small>${esc(x.codigo_principal||'')} · $${Number(x.precio_venta||0).toFixed(2)}</small></a>`]];
