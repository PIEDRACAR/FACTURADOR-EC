(function () {
  ...
  const modulo = [
    ['/', 'home', 'Inicio'], ['/pos', 'invoice', 'Facturar'], ['/proformas', 'file', 'Proformas'],
  ...
  function actualizarShell(negocio) {
    ...
    // Si el usuario es administrador, completa el acceso a gestión de usuarios
    // aunque el shell haya sido creado primero con el usuario placeholder.
    const userPanel = document.getElementById('ec-user-panel');
    if (userPanel && String(negocio?.rol || '').toLowerCase() === 'admin' && !userPanel.querySelector('[data-user-action="usuarios"]')) {
      const a = document.createElement('a');
      a.href = link('/usuarios-admin');
      a.setAttribute('data-user-action', 'usuarios');
      a.textContent = '👥 Gestión de usuarios';
      userPanel.insertBefore(a, userPanel.firstElementChild?.nextElementSibling || null);
    }
  ...
  function prepararShellInmediato() {
    ...
    renderShell(negocioPlaceholder());
  }
  ...
  function renderShell(negocio) {
    ...
    aside.innerHTML = `
      ...
      <nav class="ec-menu" aria-label="Navegación principal">
        ...
        <div class="ec-menu-group">${modulo.slice(4,8).map(navLink).join('')}</div>
        <div class="ec-menu-section"><span>Documentos SRI</span></div>
        <div class="ec-menu-group">${navLink(['/documentos','file','Documentos SRI'])}<div class="ec-doc-submenu"><a href="${link('/documentos?tipo=nota_credito')}">Nota de crédito</a><a href="${link('/documentos?tipo=nota_debito')}">Nota de débito</a><a href="${link('/documentos?tipo=liquidacion_compra')}">Liquidación de compra</a><a href="${link('/documentos?tipo=guia_remision')}">Guía de remisión</a><a href="${link('/documentos?tipo=retencion')}">Retención</a><a href="${link('/ats')}">ATS</a></div></div>
        <div class="ec-menu-section"><span>Finanzas y contabilidad</span></div>
        <div class="ec-menu-group">${navLink(['/caja','cash','Caja'])}${navLink(['/contabilidad','chart','Contabilidad'])}${navLink(['/cuentas-por-cobrar','receive','Cuentas por cobrar'])}${navLink(['/cuentas-por-pagar','pay','Cuentas por pagar'])}${navLink(['/reportes','chart','Reportes'])}</div>
    ...
    header.innerHTML = `<button class="ec-menu-toggle" id="ec-menu-toggle" aria-label="Abrir menú">${icon('menu')}</button><div class="ec-head-logo"><div class="ec-brand-icon">${icon('invoice')}</div><b>CONTSERTRIB FACTURACIÓN</b></div><label class="ec-search"><span>${icon('search')}</span><input id="ec-global-search" placeholder="Buscar facturas, clientes, productos..." autocomplete="off"></label><div class="ec-head-spacer"></div><button class="ec-head-business" id="ec-business-button"><span class="ec-head-business-icon">${icon('box')}</span><span><strong data-ec-business-name>${esc(negocio.nombreComercial || negocio.razonSocial || 'Mi empresa')}</strong><small data-ec-business-ruc>${negocio.ruc ? `RUC ${esc(negocio.ruc)}` : 'RUC configurado'}</small></span>${icon('chevron','ec-caret')}</button><button class="ec-top-action ec-notify" id="ec-notify-button" type="button" title="Centro de notificaciones" aria-label="Abrir notificaciones" data-action="notifications" aria-expanded="false"><span class="ec-action-icon">${icon('bell')}</span><span class="ec-action-label">Notificaciones</span><b id="ec-notify-count">0</b></button><button class="ec-top-action ec-head-user ec-user-button" id="ec-user-button" type="button" title="Menú de usuario" aria-label="Abrir menú de usuario" data-action="user" aria-expanded="false"><span class="ec-avatar" data-ec-avatar>${esc((negocio.usuarioNombre || negocio.rol || 'U').slice(0,1).toUpperCase())}</span><span class="ec-user-info"><strong data-ec-user-name>${esc(negocio.usuarioNombre || 'Usuario')}</strong><small data-ec-user-role>${esc(negocio.rol || 'Administrador')}</small></span><span class="ec-action-label">Usuario</span>${icon('chevron','ec-caret')}</button>`;
    ...
    // Centro de notificaciones + menú de usuario (v9.9.2)
    ...
    userPanel.innerHTML=`<div class="ec-user-panel-head"><span class="ec-avatar ec-avatar-large" data-ec-avatar>${esc((negocio.usuarioNombre || negocio.rol || 'U').slice(0,1).toUpperCase())}</span><div><strong data-ec-user-name>${esc(negocio.usuarioNombre || 'Usuario')}</strong><small data-ec-user-role>${esc(negocio.rol || 'Administrador')}</small></div></div>${esAdmin ? `<a href="${link('/usuarios-admin')}" data-user-action="usuarios">👥 Gestión de usuarios</a>` : ''}<a href="${link('/configuracion')}" data-user-action="configuracion">⚙ Configuración</a><button id="ec-user-logout" type="button" data-user-action="logout">↪ Cerrar sesión</button>`;
