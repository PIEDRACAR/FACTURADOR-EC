(function () {
  ...
  function renderShell(negocio) {
    ...
    document.getElementById('ec-global-search')?.addEventListener('keydown',e=>{if(e.key==='Enter'){const v=e.target.value.trim();if(v)location.href=link('/reportes')+'&q='+encodeURIComponent(v);}});
        const panel = document.createElement('div'); panel.id='ec-notify-panel'; panel.className='ec-popover ec-notify-panel'; panel.innerHTML='<div class="ec-popover-head"><div><strong>Centro de notificaciones</strong><small>Alertas y eventos de tu negocio</small></div><button id="ec-read-all" type="button">Marcar todas como leídas</button></div><div id="ec-notify-list" class="ec-notify-list"><div class="ec-notify-empty">Cargando…</div></div>'; document.body.appendChild(panel);
    const userPanel = document.createElement('div'); userPanel.id='ec-user-panel'; userPanel.className='ec-popover ec-user-panel'; userPanel.innerHTML=`<div class="ec-user-panel-head"><span class="ec-avatar ec-avatar-large">${esc((negocio.usuarioNombre || negocio.rol || 'U').slice(0,1).toUpperCase())}</span><div><strong>${esc(negocio.usuarioNombre || 'Usuario')}</strong><small>${esc(negocio.rol || 'Administrador')}</small></div></div><a href="/usuarios-admin?emisorId=${encodeURIComponent(emisorId)}">👥 Gestión de usuarios</a><a href="/configuracion?emisorId=${encodeURIComponent(emisorId)}">⚙ Configuración</a><button id="ec-user-logout" type="button">↪ Cerrar sesión</button>`; document.body.appendChild(userPanel);
    ...
    const cerrarPopovers=()=>{panel.classList.remove('open');userPanel.classList.remove('open');notifyButton?.setAttribute('aria-expanded','false');userButton?.setAttribute('aria-expanded','false');};
    notifyButton?.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();userPanel.classList.remove('open');userButton?.setAttribute('aria-expanded','false');panel.classList.toggle('open');notifyButton?.setAttribute('aria-expanded',panel.classList.contains('open')?'true':'false');if(panel.classList.contains('open'))await renderNotifications();});
    ...
    userButton?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();panel.classList.remove('open');notifyButton?.setAttribute('aria-expanded','false');userPanel.classList.toggle('open');userButton?.setAttribute('aria-expanded',userPanel.classList.contains('open')?'true':'false');});
    ...
    document.addEventListener('click',e=>{if(!panel.contains(e.target)&&!e.target.closest('#ec-notify-button')&&!userPanel.contains(e.target)&&!e.target.closest('#ec-user-button'))cerrarPopovers();});
    document.addEventListener('keydown',e=>{if(e.key==='Escape')cerrarPopovers();});
    renderNotifications();
    setInterval(()=>{if(document.visibilityState==='visible')renderNotifications();},30000);
