(function () {
  ...
  function renderShell(negocio) {
    ...
    // Menú lateral: en escritorio se OCULTA completamente; en móvil funciona como panel deslizable.
    const shellHiddenKey='contsertrib_sidebar_hidden';
    const aplicarEstadoSidebar=()=>{
      const hidden=localStorage.getItem(shellHiddenKey)==='1';
      if(window.innerWidth>900){
        document.body.classList.toggle('ec-sidebar-hidden',hidden);
        document.body.classList.remove('ec-sidebar-open');
      }
      const b=document.getElementById('ec-menu-toggle');
      if(b){
        const abierto=window.innerWidth<=900 ? document.body.classList.contains('ec-sidebar-open') : !hidden;
        b.setAttribute('aria-expanded',abierto?'true':'false');
        b.setAttribute('title',abierto?'Ocultar menú':'Mostrar menú');
        b.setAttribute('aria-label',abierto?'Ocultar menú':'Mostrar menú');
      }
    };
    aplicarEstadoSidebar();
    document.getElementById('ec-menu-toggle')?.addEventListener('click',()=>{
      if(window.innerWidth<=900){
        document.body.classList.toggle('ec-sidebar-open');
      }else{
        const hidden=!document.body.classList.contains('ec-sidebar-hidden');
        document.body.classList.toggle('ec-sidebar-hidden',hidden);
        localStorage.setItem(shellHiddenKey,hidden?'1':'0');
      }
      aplicarEstadoSidebar();
    });
