(function () {
  ...
  function renderShell(negocio) {
    ...
    header.innerHTML = `<button class="ec-menu-toggle" id="ec-menu-toggle" aria-label="Contraer o expandir menú" title="Contraer o expandir menú" aria-expanded="true">${icon('menu')}</button><div class="ec-head-logo"><div class="ec-brand-icon">${icon('invoice')}</div><b>CONTSERTRIB FACTURACIÓN</b></div><label class="ec-search"><span>${icon('search')}</span><input id="ec-global-search" placeholder="Buscar facturas, clientes, productos..." autocomplete="off"></label><div class="ec-head-spacer"></div><button class="ec-head-business" id="ec-business-button"><span class="ec-head-business-icon">${icon('box')}</span><span><strong data-ec-business-name>${esc(negocio.nombreComercial || negocio.razonSocial || 'Mi empresa')}</strong><small data-ec-business-ruc>${negocio.ruc ? `RUC ${esc(negocio.ruc)}` : 'RUC configurado'}</small></span>${icon('chevron','ec-caret')}</button><button class="ec-top-action ec-notify" id="ec-notify-button" type="button" title="Notificaciones" aria-label="Abrir notificaciones" data-action="notifications" aria-expanded="false"><span class="ec-action-icon">${icon('bell')}</span><b id="ec-notify-count">0</b></button><button class="ec-top-action ec-theme-button" id="ec-theme-button" type="button" title="Cambiar tema" aria-label="Cambiar entre tema claro y oscuro"><span class="ec-action-icon" id="ec-theme-icon">${icon('moon')}</span></button><button class="ec-top-action ec-head-user ec-user-button" id="ec-user-button" type="button" title="Menú de usuario" aria-label="Abrir menú de usuario" data-action="user" aria-expanded="false"><span class="ec-avatar ec-avatar-user-icon" data-ec-avatar>${icon('user')}</span><span class="ec-user-info"><strong data-ec-user-name>${esc(negocio.usuarioNombre || 'Usuario')}</strong><small data-ec-user-role>${esc(negocio.rol || 'Administrador')}</small></span>${icon('chevron','ec-caret')}</button>`;
    ...
    const shellCollapsedKey='contsertrib_sidebar_collapsed';
    const aplicarEstadoSidebar=()=>{const collapsed=localStorage.getItem(shellCollapsedKey)==='1';document.body.classList.toggle('ec-sidebar-collapsed',collapsed);const b=document.getElementById('ec-menu-toggle');if(b){b.setAttribute('aria-expanded',collapsed?'false':'true');b.setAttribute('title',collapsed?'Expandir menú':'Contraer menú');b.setAttribute('aria-label',collapsed?'Expandir menú':'Contraer menú')}};
    aplicarEstadoSidebar();
    document.getElementById('ec-menu-toggle')?.addEventListener('click',()=>{const collapsed=!document.body.classList.contains('ec-sidebar-collapsed');document.body.classList.toggle('ec-sidebar-collapsed',collapsed);localStorage.setItem(shellCollapsedKey,collapsed?'1':'0');const b=document.getElementById('ec-menu-toggle');if(b){b.setAttribute('aria-expanded',collapsed?'false':'true');b.setAttribute('title',collapsed?'Expandir menú':'Contraer menú');b.setAttribute('aria-label',collapsed?'Expandir menú':'Contraer menú')}});
