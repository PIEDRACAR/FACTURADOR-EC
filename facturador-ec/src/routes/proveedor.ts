...
import { obtenerSesion, crearOEncontrarUsuario, agregarUsuarioANegocio, obtenerUsuarioAuthPorEmail } from '../auth/sesiones.js';
...
import { crearLinkPagoSaas } from '../services/pagosSaas.js';
...
export async function registrarRutasProveedor(app: FastifyInstance) {
  ...
  app.get('/proveedor/salud', async (request, reply) => {
    ...
    checks.push({clave:'correo',nombre:'Correo transaccional',estado:correoOk?'ok':'error',detalle:correoOk?`Resend configurado. Remitente: ${env.emailFrom}`:'Faltan RESEND_API_KEY y/o EMAIL_FROM'});
    const payOk=Boolean(env.payphoneToken && env.payphoneStoreId);
    checks.push({clave:'payphone',nombre:'PayPhone API Link',estado:payOk?'ok':'error',detalle:payOk?'Token y StoreID configurados':'Faltan PAYPHONE_TOKEN y/o PAYPHONE_STORE_ID'});
  ...
  // Promoción comercial global: solo ROOT puede editarla.
  app.get('/proveedor/promocion', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    const { data, error } = await supabase.from('promociones_saas').select('*').eq('codigo','TRIAL_PUBLICO').maybeSingle();
    if (error) return reply.status(500).send({ error: error.message });
    return reply.send(data ?? { codigo:'TRIAL_PUBLICO', nombre:'Prueba gratuita', activa:false, dias_prueba:0, texto_publico:'', fecha_desde:null, fecha_hasta:null, aplicar_automaticamente:false });
  });

  app.patch<{ Body: { nombre?: string; activa?: boolean; diasPrueba?: number; textoPublico?: string; fechaDesde?: string | null; fechaHasta?: string | null; aplicarAutomaticamente?: boolean } }>('/proveedor/promocion', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    const b = request.body ?? {};
    const cambios: Record<string, unknown> = { updated_at: new Date().toISOString() };
    if (b.nombre !== undefined) cambios.nombre = String(b.nombre).trim() || 'Promoción comercial';
    if (b.activa !== undefined) cambios.activa = Boolean(b.activa);
    if (b.diasPrueba !== undefined) {
      const n = Number(b.diasPrueba);
      if (!Number.isInteger(n) || n < 0 || n > 365) return reply.status(400).send({ error:'Los días de prueba deben ser un número entero entre 0 y 365.' });
      cambios.dias_prueba = n;
    }
    if (b.textoPublico !== undefined) cambios.texto_publico = String(b.textoPublico).trim();
    if (b.fechaDesde !== undefined) cambios.fecha_desde = b.fechaDesde ? String(b.fechaDesde) : null;
    if (b.fechaHasta !== undefined) cambios.fecha_hasta = b.fechaHasta ? String(b.fechaHasta) : null;
    if (b.aplicarAutomaticamente !== undefined) cambios.aplicar_automaticamente = Boolean(b.aplicarAutomaticamente);
    const fechaDesde = cambios.fecha_desde as string | null | undefined;
    const fechaHasta = cambios.fecha_hasta as string | null | undefined;
    if (fechaDesde && !/^\d{4}-\d{2}-\d{2}$/.test(fechaDesde)) return reply.status(400).send({ error:'Fecha desde inválida.' });
    if (fechaHasta && !/^\d{4}-\d{2}-\d{2}$/.test(fechaHasta)) return reply.status(400).send({ error:'Fecha hasta inválida.' });
    if (fechaDesde && fechaHasta && fechaHasta < fechaDesde) return reply.status(400).send({ error:'La fecha hasta no puede ser anterior a la fecha desde.' });
    const { data, error } = await supabase.from('promociones_saas').update(cambios).eq('codigo','TRIAL_PUBLICO').select('*').single();
    if (error) return reply.status(500).send({ error: error.message });
    return reply.send(data);
  });
  ...
  app.patch<{ Params: { id: string }; Body: { nombre?: string; descripcion?: string; precioMensual?: number; precioAnual?: number; periodicidad?: 'mensual'|'anual'; activo?: boolean; maxDocumentosMes?: number | null; maxDocumentosAnio?: number | null; maxContribuyentes?: number; maxEstablecimientos?: number; maxPuntosEmision?: number; maxUsuarios?: number; incluyeInventario?: boolean; incluyeAts?: boolean; incluyeCargaElectronica?: boolean; incluyeReportesAvanzados?: boolean; incluyeTicketPos?: boolean; modulosConfig?: Record<string, boolean> } }>('/proveedor/planes/:id', async (request, reply) => {
    ...
    if (b.modulosConfig !== undefined) { const cfg = b.modulosConfig && typeof b.modulosConfig === 'object' ? b.modulosConfig : {}; cambios.modulos_config = Object.fromEntries(Object.entries(cfg).map(([k,v]) => [String(k).slice(0,60), Boolean(v)])); }
  ...
  app.get('/proveedor/clientes', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    const { data: emisores, error } = await supabase.from('emisores').select('id,ruc,razon_social,nombre_comercial,direccion_matriz,ambiente,ticket_pos_habilitado,created_at').order('created_at', { ascending: false });
    if (error) return reply.status(500).send({ error: error.message });
  ...
  app.delete<{ Params: { id: string } }>('/proveedor/solicitudes-registro/:id', async (request, reply) => {
    const auth = await exigirProveedor(request, reply);
    if (!auth) return;
    const { data: solicitud, error: se } = await supabase.from('solicitudes_registro_saas').select('id,ruc,razon_social,estado').eq('id', request.params.id).maybeSingle();
    if (se) return reply.status(500).send({ error: se.message });
    if (!solicitud) return reply.status(404).send({ error: 'Solicitud no encontrada.' });
    if (solicitud.estado !== 'rechazada') return reply.status(409).send({ error: 'Solo se pueden eliminar solicitudes que estén rechazadas.' });
    const { error } = await supabase.from('solicitudes_registro_saas').delete().eq('id', request.params.id);
    if (error) return reply.status(500).send({ error: error.message });
    await registrarAuditoriaSaas({ userId: auth.userId, evento: 'SAAS_SOLICITUD_RECHAZADA_ELIMINADA', recurso: 'solicitud_registro_saas', recursoId: request.params.id, detalle: { ruc: solicitud.ruc, razonSocial: solicitud.razon_social } });
    return reply.send({ ok: true, eliminado: request.params.id });
  });

  app.post<{ Params: { id: string } }>('/proveedor/solicitudes-registro/:id/pago', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    try {
      const pago = await crearLinkPagoSaas(request.params.id, true);
      return reply.send({ ok:true, pago, paymentLink:pago.payment_link, correoCliente:pago.notificacion_cliente_estado ?? 'pendiente' });
    } catch (err) {
      return reply.status(502).send({ error:'No se pudo generar/reenviar el link de pago.', detalle:err instanceof Error?err.message:String(err) });
    }
  });
  ...
  app.post<{ Body: { ruc?: string; razonSocial?: string; nombreComercial?: string; direccionMatriz?: string; emailAdmin?: string; passwordTemporal?: string; planCodigo?: string; ambiente?: 'pruebas'|'produccion'; diasIniciales?: number; datosSri?: Record<string, unknown>; solicitudId?: string } }>('/proveedor/clientes', async (request, reply) => {
    const proveedorAuth = await exigirProveedor(request, reply);
    if (!proveedorAuth) return;
    const b = request.body ?? {};
    const solicitudId = String(b.solicitudId ?? '').trim();
    if (solicitudId) {
      const { data: solicitud, error: solicitudError } = await supabase.from('solicitudes_registro_saas').select('id,ruc,plan_codigo,pago_estado,estado').eq('id', solicitudId).maybeSingle();
      if (solicitudError) return reply.status(500).send({ error: solicitudError.message });
      if (!solicitud) return reply.status(404).send({ error: 'La solicitud pública no existe.' });
      if (solicitud.pago_estado !== 'pagado') return reply.status(409).send({ error: 'No se puede activar esta solicitud porque el pago SaaS todavía no figura como aprobado.', pagoEstado: solicitud.pago_estado });
      if (String(b.ruc ?? '').trim() !== String(solicitud.ruc)) return reply.status(400).send({ error: 'El RUC no coincide con la solicitud pagada.' });
      if (String(b.planCodigo ?? '').trim().toUpperCase() !== String(solicitud.plan_codigo ?? '').trim().toUpperCase()) return reply.status(400).send({ error: 'El plan no coincide con la solicitud pagada.' });
    }
    ...
    try {
      ...
      userId = existente?.id ?? await crearOEncontrarUsuario(email, password);
    ...
    return reply.status(201).send({ ok: true, emisorId: emisor.id, puntoEmisionId: punto.id, userId, plan: { codigo: plan.codigo, nombre: plan.nombre, precioMensual: plan.precio_mensual, precioAnual: plan.precio_anual, periodicidad: plan.periodicidad }, fechaInicio: inicio, proximoVencimiento: vencimiento, correoEnviado, correoError, acceso: { url: `${env.appUrl}/login`, email } });
  });
  ...
  app.post<{ Params: { emisorId: string } }>('/proveedor/clientes/:emisorId/reenvio-acceso', async (request, reply) => {
    ...
    const redirectTo=`${env.appUrl.replace(/\/$/,'')}/restablecer-password`;
    const { error: resetError }=await supabase.auth.resetPasswordForEmail(email,{redirectTo});
    if (resetError) return reply.status(502).send({error:'No se pudo solicitar el enlace seguro de recuperación.'});
    return reply.send({ok:true,correo:email,correoEnviado:true,mensaje:'Supabase envió un enlace seguro de recuperación. La contraseña actual no fue modificada.'});
  });

  app.get<{ Params: { emisorId: string } }>('/proveedor/clientes/:emisorId/ticket-pos', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    const { data, error } = await supabase.from('emisores').select('id,ticket_pos_habilitado').eq('id', request.params.emisorId).maybeSingle();
    if (error) return reply.status(500).send({ error: error.message });
    if (!data) return reply.status(404).send({ error: 'Contribuyente no encontrado.' });
    return reply.send({ emisorId: data.id, habilitado: data.ticket_pos_habilitado !== false });
  });

  app.patch<{ Params: { emisorId: string }; Body: { habilitado?: boolean } }>('/proveedor/clientes/:emisorId/ticket-pos', async (request, reply) => {
    const proveedorAuth = await exigirProveedor(request, reply);
    if (!proveedorAuth) return;
    if (request.body?.habilitado === undefined) return reply.status(400).send({ error: 'Debes indicar si Ticket / POS queda habilitado.' });
    const { data: planSub } = await supabase.from('suscripciones').select('plan_id,planes_suscripcion(codigo,nombre,incluye_ticket_pos)').eq('emisor_id', request.params.emisorId).maybeSingle();
    const plan: any = (planSub as any)?.planes_suscripcion;
    if (request.body.habilitado && plan && plan.incluye_ticket_pos === false) {
      return reply.status(409).send({ error: `El plan ${plan.nombre} no incluye Ticket / POS. Activa esta característica en el plan antes de habilitarla para el contribuyente.` });
    }
    const { data, error } = await supabase.from('emisores').update({ ticket_pos_habilitado: Boolean(request.body.habilitado), updated_at: new Date().toISOString() }).eq('id', request.params.emisorId).select('id,ticket_pos_habilitado').single();
    if (error) return reply.status(500).send({ error: error.message });
    await registrarAuditoriaSaas({ userId: proveedorAuth.userId, emisorId: request.params.emisorId, evento: request.body.habilitado ? 'TICKET_POS_HABILITADO_CONTRIBUYENTE' : 'TICKET_POS_DESHABILITADO_CONTRIBUYENTE', recurso: 'emisor', recursoId: request.params.emisorId, detalle: { habilitado: Boolean(request.body.habilitado), plan: plan?.codigo ?? null } });
    return reply.send({ ok: true, emisorId: data.id, habilitado: data.ticket_pos_habilitado });
  });
  ...
  app.post<{ Params: { emisorId: string }; Body: { ruc?: string; razonSocial?: string; direccionMatriz?: string; nombreComercial?: string; emailAdmin?: string } }>('/proveedor/clientes/:emisorId/contribuyentes', async (request, reply) => {
    ...
    const {data:emisor,error:e}=await supabase.from('emisores').insert({ruc,razon_social:razon,nombre_comercial:String(b.nombreComercial??'').trim()||null,direccion_matriz:direccion,obligado_contabilidad:false,ambiente:'produccion',ticket_pos_habilitado:false}).select('id').single();
    ...
    else if(email){ try { const existente=await obtenerUsuarioAuthPorEmail(email); const uid=existente?.id??await crearOEncontrarUsuario(email,generarPasswordTemporal()); await agregarUsuarioANegocio(uid,emisor.id,'admin'); } catch {} }
  ...
  // Promoción pública: solo expone campos comerciales seguros.
  app.get('/promocion-publica', async (_request, reply) => {
    const { data, error } = await supabase.from('promociones_saas').select('nombre,activa,dias_prueba,texto_publico,fecha_desde,fecha_hasta,aplicar_automaticamente').eq('codigo','TRIAL_PUBLICO').maybeSingle();
    if (error) return reply.status(500).send({ error: error.message });
    const p = data ?? { nombre:'', activa:false, dias_prueba:0, texto_publico:'', fecha_desde:null, fecha_hasta:null, aplicar_automaticamente:false };
    const hoy = new Date().toISOString().slice(0,10);
    const vigente = Boolean(p.activa && p.aplicar_automaticamente && Number(p.dias_prueba||0) > 0 && (!p.fecha_desde || hoy >= p.fecha_desde) && (!p.fecha_hasta || hoy <= p.fecha_hasta));
    return reply.send({ ...p, vigente });
  });
  ...
  app.get('/planes-publicos', async (_request, reply) => {
    const { data, error } = await supabase.from('planes_suscripcion')
      .select('codigo,nombre,descripcion,precio_mensual,precio_anual,periodicidad,orden,max_documentos_mes,max_documentos_anio,max_contribuyentes,max_establecimientos,max_puntos_emision,max_usuarios,incluye_inventario,incluye_ats,incluye_carga_electronica,incluye_reportes_avanzados,incluye_ticket_pos,modulos_config,descripcion_comercial')
