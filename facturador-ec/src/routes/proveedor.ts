...
export async function registrarRutasProveedor(app: FastifyInstance) {
  ...
  app.patch<{ Params: { emisorId: string }; Body: { planCodigo?: string; nota?: string } }>('/proveedor/clientes/:emisorId/plan', async (request, reply) => {
    ...
    if (relInfo?.cuenta_id) { const { count: contribs } = await supabase.from('contribuyentes_cliente_saas').select('id',{count:'exact',head:true}).eq('cuenta_id',relInfo.cuenta_id).eq('activo',true); if (Number(contribs ?? 0) > Number(plan.max_contribuyentes ?? 999999)) return reply.status(409).send({ error: `El cliente ya tiene ${contribs ?? 0} contribuyentes y el plan ${plan.nombre} permite ${plan.max_contribuyentes}.` }); }
