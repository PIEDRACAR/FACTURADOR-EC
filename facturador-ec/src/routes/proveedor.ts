...
export async function registrarRutasProveedor(app: FastifyInstance) {
  ...
  app.post<{ Body: { ruc?: string; razonSocial?: string; nombreComercial?: string; direccionMatriz?: string; emailAdmin?: string; passwordTemporal?: string; planCodigo?: string; ambiente?: 'pruebas'|'produccion'; diasIniciales?: number } }>('/proveedor/clientes', async (request, reply) => {
    ...
    let sError: any = null;
    if (subExistente) {
      const r = await supabase.from('suscripciones').update({ cuenta_id: cuenta.id, plan_id: plan.id, precio_contratado: Number(plan.precio_mensual), estado: 'activa', modalidad: 'prueba', ultima_activacion: inicio, dias_activacion: dias, fecha_inicio: inicio, proximo_vencimiento: vencimiento, fecha_cancelacion: null, updated_at: new Date().toISOString() }).eq('id', subExistente.id).select('*').single();
      sub = r.data; sError = r.error;
    } else {
      const r = await supabase.from('suscripciones').insert({ emisor_id: emisor.id, cuenta_id: cuenta.id, plan_id: plan.id, precio_contratado: Number(plan.precio_mensual), estado: 'activa', modalidad: 'prueba', ultima_activacion: inicio, dias_activacion: dias, fecha_inicio: inicio, proximo_vencimiento: vencimiento }).select('*').single();
      sub = r.data; sError = r.error;
    }
  ...
  app.patch<{ Params: { emisorId: string }; Body: { planCodigo?: string; nota?: string } }>('/proveedor/clientes/:emisorId/plan', async (request, reply) => {
    ...
    // Una sola relación por alcance: evita redeclaraciones y reutiliza el resultado
    // para actualizar la cuenta SaaS después del cambio de plan.
    const [docsResult, docsComplementariosResult, estsResult, puntosResult, relCuentaResult] = await Promise.all([
      supabase.from('comprobantes').select('id',{count:'exact',head:true}).eq('emisor_id',emisorId).eq('estado','autorizado').gte('created_at',mesDesde),
      supabase.from('documentos_sri_borrador').select('id',{count:'exact',head:true}).eq('emisor_id',emisorId).eq('estado','autorizado').gte('created_at',mesDesde),
    ...
    const docsComplementarios = docsComplementariosResult.count;
    const ests = estsResult.count;
    const puntos = puntosResult.count;
    const relCuenta = relCuentaResult.data;
    const docsConsumidos = Number(docs ?? 0) + Number(docsComplementarios ?? 0);
    if (plan.max_documentos_mes != null && docsConsumidos > Number(plan.max_documentos_mes)) return reply.status(409).send({ error: `No se puede bajar al plan ${plan.nombre}: el consumo actual (${docsConsumidos}) supera su límite mensual (${plan.max_documentos_mes}).` });
    ...
    if (relCuenta?.cuenta_id) { const { count: contribs } = await supabase.from('contribuyentes_cliente_saas').select('id',{count:'exact',head:true}).eq('cuenta_id',relCuenta.cuenta_id).eq('activo',true); if (Number(contribs ?? 0) > Number(plan.max_contribuyentes ?? 999999)) return reply.status(409).send({ error: `El cliente ya tiene ${contribs ?? 0} contribuyentes y el plan ${plan.nombre} permite ${plan.max_contribuyentes}.` }); }
    const { data: planAnterior } = await supabase.from('planes_suscripcion').select('id,codigo,nombre,precio_mensual').eq('id',sub.plan_id).maybeSingle();
    const { data, error } = await supabase.from('suscripciones').update({ plan_id: plan.id, precio_contratado: Number(plan.precio_mensual), fecha_cambio_plan: hoyEcuadorIso(), updated_at: new Date().toISOString(), nota: String(request.body?.nota ?? '').trim() || null }).eq('id',sub.id).select('*').single();
    if (error) return reply.status(500).send({ error: error.message });
    const actor = (await esProveedorAdmin(request)).userId ?? null;
    const { error: histError } = await supabase.from('historial_planes_saas').insert({ suscripcion_id: sub.id, emisor_id: emisorId, plan_anterior_id: planAnterior?.id ?? null, plan_nuevo_id: plan.id, precio_anterior: planAnterior?.precio_mensual ?? null, precio_nuevo: plan.precio_mensual, consumo_documentos_mes: docsConsumidos, motivo: String(request.body?.nota ?? '').trim() || 'Cambio de plan', actor_user_id: actor });
    if (histError) request.log.warn({ err: histError }, 'No se pudo registrar historial de plan; el cambio principal sí se aplicó.');
    if (relCuenta?.cuenta_id) await supabase.from('cuentas_cliente_saas').update({ plan_id: plan.id, updated_at: new Date().toISOString() }).eq('id', relCuenta.cuenta_id);
  ...
  app.patch<{ Params: { emisorId: string }; Body: { estado?: Estado; dias?: number; nota?: string } }>('/proveedor/clientes/:emisorId/suscripcion', async (request, reply) => {
    ...
    if (b.dias !== undefined) {
      ...
      if (!Number.isFinite(dias)) return reply.status(400).send({ error: 'Los días de vigencia no son válidos.' });
      cambios.proximo_vencimiento = fechaMasDias(dias);
      if (b.estado === 'activa') {
        const { data: actual } = await supabase.from('suscripciones').select('modalidad,ultima_activacion,prueba_usada_at').eq('emisor_id', request.params.emisorId).maybeSingle();
        ...
        if (esPruebaInicial) cambios.prueba_usada_at = hoyEcuadorIso();
      }
    }
  ...
  app.post<{ Params: { emisorId: string }; Body: { monto?: number; metodo?: string; referencia?: string; dias?: number; nota?: string } }>('/proveedor/clientes/:emisorId/pago', async (request, reply) => {
    ...
    if (!Number.isFinite(monto) || monto <= 0 || monto > 1000000) return reply.status(400).send({ error: 'El monto debe ser mayor que cero y no superar $1.000.000.' });
    const { data: sub } = await supabase.from('suscripciones').select('id,proximo_vencimiento,plan_id').eq('emisor_id', request.params.emisorId).maybeSingle();
    if (!sub) return reply.status(404).send({ error: 'El cliente no tiene una suscripción.' });
    ...
    if (!metodosPermitidos.has(metodo)) return reply.status(400).send({ error: 'Método de pago no permitido.' });
    ...
    if (referencia) {
      const { data: pagoExistente } = await supabase.from('pagos_suscripcion').select('id').eq('emisor_id', request.params.emisorId).eq('referencia', referencia).maybeSingle();
      if (pagoExistente) return reply.status(409).send({ error: 'La referencia de pago ya fue registrada. Verifica el comprobante antes de intentar nuevamente.' });
    }
    const { data: planPago } = await supabase.from('planes_suscripcion').select('codigo,precio_mensual').eq('id',sub.plan_id).maybeSingle();
    const actor = (await esProveedorAdmin(request)).userId ?? null;
    ...
    if (error) {
      if (String(error.message).toLowerCase().includes('duplicate') || String(error.code ?? '') === '23505') return reply.status(409).send({ error: 'La referencia de pago ya fue registrada. Verifica el comprobante antes de intentar nuevamente.' });
      return reply.status(500).send({ error: error.message });
    }
    const { data: actualizada, error: uError } = await supabase.from('suscripciones').update({ estado: 'activa', modalidad: 'pagada', proximo_vencimiento: hasta, ultimo_pago_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', sub.id).select('*').single();
  ...
  app.get<{ Params: { emisorId: string } }>('/proveedor/clientes/:emisorId/estructura', async (request, reply) => {
    ...
    const [{ data: ests }, { data: puntos }, { data: cuentaRel }, { data: sub }, { count: docs }, { count: docsComp }, { data: pagos }] = await Promise.all([
      ...
      supabase.from('suscripciones').select('id,plan_id,estado,modalidad,fecha_inicio,proximo_vencimiento,precio_contratado,fecha_cambio_plan,prueba_usada_at,ultimo_pago_at,dias_gracia,renovacion_automatica,planes_suscripcion(codigo,nombre,precio_mensual,max_documentos_mes)').eq('emisor_id',emisorId).maybeSingle(),
      supabase.from('comprobantes').select('id',{count:'exact',head:true}).eq('emisor_id',emisorId).eq('estado','autorizado').gte('created_at',mesDesde),
      supabase.from('documentos_sri_borrador').select('id',{count:'exact',head:true}).eq('emisor_id',emisorId).eq('estado','autorizado').gte('created_at',mesDesde),
      supabase.from('pagos_suscripcion').select('id,monto,fecha_pago,periodo_desde,periodo_hasta,metodo,referencia,plan_codigo').eq('emisor_id',emisorId).order('fecha_pago',{ascending:false}).limit(10),
    ]);
