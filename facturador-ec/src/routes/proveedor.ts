...
function generarPasswordTemporal(): string {
  ...
  app.get<{ Params: { ruc: string } }>('/proveedor/ruc/:ruc', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    const ruc = String(request.params.ruc ?? '').replace(/\D/g, '');
    if (!RUC_REGEX.test(ruc)) return reply.status(400).send({ error: 'El RUC debe contener 13 dígitos.' });
    try {
      const url = `https://srienlinea.sri.gob.ec/facturacion-internet/consultas/publico/ruc-datos2.jspa?ruc=${ruc}`;
      const resp = await fetch(url, { headers: { 'User-Agent': 'CONTSERTRIB/9.7.5' } });
      const html = await resp.text();
      if (!resp.ok || /No se encontr|no existe|RUC no válido/i.test(html)) return reply.status(404).send({ error: 'No se encontró información pública del RUC en SRI.' });
      const clean = html.replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<[^>]+>/g,' ').replace(/&nbsp;/gi,' ').replace(/\s+/g,' ').trim();
      const pick=(label:string)=>{ const re=new RegExp(label.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'\\s*:?[ ]*([^:]+?)(?=\\s+(?:RUC|Nombre Comercial|Estado del Contribuyente|Clase de Contribuyente|Tipo de Contribuyente|Obligado a llevar Contabilidad|Actividad Económica Principal|Fecha de inicio de actividades|Fecha actualización|Fecha de cese de actividades|Fecha reinicio de actividades|Categoria Mi PYMES)\\s*:|$)','i'); const m=clean.match(re); return m?.[1]?.trim()||''; };
      const razon = pick('Razón Social');
      const nombre = pick('Nombre Comercial');
      const estado = pick('Estado del Contribuyente en el RUC');
      const tipo = pick('Tipo de Contribuyente');
      const obligado = pick('Obligado a llevar Contabilidad');
      const actividad = pick('Actividad Económica Principal');
      const inicio = pick('Fecha de inicio de actividades');
      if (!razon && !clean.includes(ruc)) return reply.status(404).send({ error: 'El SRI no devolvió datos utilizables para ese RUC.' });
      return reply.send({ ok:true, ruc, razonSocial: razon, nombreComercial:nombre, estado, tipoContribuyente:tipo, obligadoContabilidad:obligado, actividadPrincipal:actividad, fechaInicioActividades:inicio, fuente:'Consulta pública RUC SRI' });
    } catch (e) { return reply.status(502).send({ error:'No fue posible consultar el RUC en este momento.', detalle:e instanceof Error?e.message:String(e) }); }
  });
...
export async function registrarRutasProveedor(app: FastifyInstance) {
  ...
  app.patch<{ Params: { emisorId: string }; Body: { planCodigo?: string; nota?: string } }>('/proveedor/clientes/:emisorId/plan', async (request, reply) => {
    ...
    if (!codigo) return reply.status(400).send({ error:'Debe indicar el nuevo plan.' });
    const { data: plan } = await supabase.from('planes_suscripcion').select('*').eq('codigo',codigo).eq('activo',true).maybeSingle();
    if (!plan) return reply.status(404).send({ error:`El plan ${codigo} no existe o está inactivo.` });
    ...
    const { data: sub } = await supabase.from('suscripciones').select('*').eq('emisor_id',request.params.emisorId).maybeSingle();
    if (!sub) return reply.status(404).send({ error:'El cliente no tiene suscripción.' });
    const cuentaId=rel?.cuenta_id ?? sub.cuenta_id ?? null;
    const anterior=sub.plan_id;
    if (anterior===plan.id) return reply.send({ok:true,mensaje:'El cliente ya tiene ese plan.',suscripcion:sub});
    const { data: updated, error } = await supabase.from('suscripciones').update({ plan_id:plan.id, updated_at:new Date().toISOString() }).eq('id',sub.id).select('*').single();
    if(error) return reply.status(500).send({error:error.message});
    if(cuentaId) await supabase.from('cuentas_cliente_saas').update({plan_id:plan.id,updated_at:new Date().toISOString()}).eq('id',cuentaId);
    if(cuentaId) await supabase.from('suscripciones').update({plan_id:plan.id,updated_at:new Date().toISOString()}).eq('cuenta_id',cuentaId);
    await supabase.from('historial_suscripciones').insert({suscripcion_id:sub.id,emisor_id:request.params.emisorId,cuenta_id:cuentaId,accion:'CAMBIO_PLAN',plan_anterior_id:anterior,plan_nuevo_id:plan.id,estado_anterior:sub.estado,estado_nuevo:updated.estado,fecha_anterior:sub.proximo_vencimiento,fecha_nueva:updated.proximo_vencimiento,nota:String(request.body?.nota??'').trim()||null});
    return reply.send({ok:true,suscripcion:updated,plan});
  });
  ...
  app.get<{ Params: { emisorId: string } }>('/proveedor/clientes/:emisorId/consumo', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    const hoy=hoyEcuadorIso(), desde=`${hoy.slice(0,7)}-01`;
    const [{count:docs},{count:contrib},{count:est},{count:ptos},{data:sub}]=await Promise.all([
      supabase.from('comprobantes').select('id',{count:'exact',head:true}).eq('emisor_id',request.params.emisorId).eq('estado','autorizado').gte('created_at',`${desde}T00:00:00-05:00`),
      supabase.from('contribuyentes_cliente_saas').select('id',{count:'exact',head:true}).eq('emisor_id',request.params.emisorId).eq('activo',true),
      supabase.from('establecimientos_emisor').select('id',{count:'exact',head:true}).eq('emisor_id',request.params.emisorId).eq('activo',true),
      supabase.from('puntos_emision').select('id',{count:'exact',head:true}).eq('emisor_id',request.params.emisorId).eq('activo',true),
      supabase.from('suscripciones').select('plan_id,proximo_vencimiento,estado,planes_suscripcion(max_documentos_mes,max_contribuyentes,max_establecimientos,max_puntos_emision)').eq('emisor_id',request.params.emisorId).maybeSingle()
    ]);
    return reply.send({periodo:desde.slice(0,7),documentos:docs??0,contribuyentes:contrib??0,establecimientos:est??0,puntos:ptos??0,suscripcion:sub??null});
  });
  app.get<{ Params: { emisorId: string } }>('/proveedor/clientes/:emisorId/historial-suscripcion', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    const {data,error}=await supabase.from('historial_suscripciones').select('*').eq('emisor_id',request.params.emisorId).order('created_at',{ascending:false}).limit(100);
    if(error) return reply.status(500).send({error:error.message});
    return reply.send(data??[]);
  });
