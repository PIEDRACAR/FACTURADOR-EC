...
export async function registrarRutasProveedor(app: FastifyInstance) {
  ...
  app.post<{ Body: { ruc?: string; razonSocial?: string; nombreComercial?: string; direccionMatriz?: string; emailAdmin?: string; passwordTemporal?: string; planCodigo?: string; ambiente?: 'pruebas'|'produccion'; diasIniciales?: number } }>('/proveedor/clientes', async (request, reply) => {
    ...
    if (emisor) {
      ...
      const { data: nuevoEmisor, error: eError } = await supabase.from('emisores').insert({ ruc, razon_social: razonSocial, nombre_comercial: String(b.nombreComercial ?? '').trim() || null, direccion_matriz: direccion, obligado_contabilidad: sri.obligadoContabilidad ?? false, ambiente }).select('id').single();
  ...
  app.patch<{ Params: { emisorId: string }; Body: { estado?: Estado; dias?: number; sumarDias?: number; nota?: string; planId?: string } }>('/proveedor/clientes/:emisorId/suscripcion', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    const b = request.body ?? {};
    ...
    if (b.sumarDias !== undefined) {
      const sumar = Math.max(1, Math.min(3650, Number(b.sumarDias)));
      ...
      const base = actual?.proximo_vencimiento && actual.proximo_vencimiento >= hoyEcuadorIso() ? actual.proximo_vencimiento : hoyEcuadorIso();
      const [y,m,d] = base.split('-').map(Number);
      const fecha = new Date(Date.UTC(y,m-1,d+sumar,12,0,0));
      cambios.proximo_vencimiento = fecha.toISOString().slice(0,10);
      cambios.estado = 'activa';
    } else if (b.dias !== undefined) {
    ...
    let planAnteriorId: string | null = null;
    if (b.planId !== undefined) {
      ...
      cuentaId = rel?.cuenta_id ?? null;
      const { data: actual } = await supabase.from('suscripciones').select('plan_id').eq('emisor_id', request.params.emisorId).maybeSingle();
      planAnteriorId = actual?.plan_id ?? null;
      cambios.plan_id = plan.id;
      if (cuentaId) {
        await supabase.from('cuentas_cliente_saas').update({ plan_id: plan.id, updated_at: new Date().toISOString() }).eq('id', cuentaId);
        // Un plan pertenece a la cuenta comercial: todos sus RUC deben usar el mismo plan.
        await supabase.from('suscripciones').update({ plan_id: plan.id, updated_at: new Date().toISOString() }).eq('cuenta_id', cuentaId);
      }
    }
    const { data: anterior } = await supabase.from('suscripciones').select('id,plan_id,cuenta_id').eq('emisor_id', request.params.emisorId).maybeSingle();
    const { data, error } = await supabase.from('suscripciones').update(cambios).eq('emisor_id', request.params.emisorId).select('*').single();
    if (error) return reply.status(500).send({ error: error.message });
    if (b.planId && (planAnteriorId || anterior?.plan_id) !== b.planId) {
      await supabase.from('historial_planes_saas').insert({cuenta_id:cuentaId ?? anterior?.cuenta_id ?? null,emisor_id:request.params.emisorId,plan_anterior_id:planAnteriorId ?? anterior?.plan_id ?? null,plan_nuevo_id:b.planId,nota:String(b.nota??'Cambio de plan desde panel').trim()||null});
    }
    return reply.send(data);
  });
