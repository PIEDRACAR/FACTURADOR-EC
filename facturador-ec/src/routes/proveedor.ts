...
export async function registrarRutasProveedor(app: FastifyInstance) {
  ...
  app.get('/proveedor/clientes', async (request, reply) => {
    ...
    return reply.send((emisores ?? []).map(e => { const c:any=contribMap.get(e.id); return { ...e, suscripcion: subMap.get(e.id) ?? null, certificado: certMap.get(e.id) ?? null, configuracionPendiente: !certMap.has(e.id), cuenta: c?.cuentas_cliente_saas ?? null, contribuyentes: c?.cuenta_id ? countMap.get(c.cuenta_id) ?? 1 : 1 }; }));
  });
  ...
  app.post<{ Body: { ruc?: string; razonSocial?: string; nombreComercial?: string; direccionMatriz?: string; emailAdmin?: string; passwordTemporal?: string; planCodigo?: string; ambiente?: 'pruebas'|'produccion'; diasIniciales?: number } }>('/proveedor/clientes', async (request, reply) => {
    ...
    await supabase.from('contribuyentes_cliente_saas').insert({ cuenta_id: cuenta.id, emisor_id: emisor.id, activo: true });
    const { data: sub, error: sError } = await supabase.from('suscripciones').insert({ emisor_id: emisor.id, cuenta_id: cuenta.id, plan_id: plan.id, estado: 'activa', fecha_inicio: inicio, proximo_vencimiento: vencimiento }).select('*').single();
  ...
  app.delete<{ Params: { emisorId: string } }>('/proveedor/clientes/:emisorId', async (request, reply) => {
    ...
    return reply.send({ok:true,mensaje:'Cuenta desactivada. Los documentos tributarios históricos no fueron borrados.'});
  });
