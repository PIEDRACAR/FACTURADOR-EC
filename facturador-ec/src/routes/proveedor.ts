...
export async function registrarRutasProveedor(app: FastifyInstance) {
  ...
  app.get('/proveedor/clientes', async (request, reply) => {
    ...
    const { data: contribs } = ids.length ? await supabase.from('contribuyentes_cliente_saas').select('emisor_id,cuenta_id,activo,ticket_pos_habilitado,ticket_pos_updated_at,cuentas_cliente_saas(id,nombre,email_admin,plan_id,estado,planes_suscripcion(codigo,nombre,precio_mensual,max_documentos_mes,max_contribuyentes,max_establecimientos,max_puntos_emision,max_usuarios,incluye_inventario,incluye_ats,incluye_carga_electronica,incluye_reportes_avanzados,incluye_ticket_pos))').in('emisor_id', ids) : { data: [] as any[] };
  ...
  app.get<{ Querystring: { ruc?: string } }>('/proveedor/ruc/consultar', async (request, reply) => {
    ...
    try {
      ...
      const r = await fetch(u,{headers:{Accept:'application/json','User-Agent':'CONTSERTRIB/9.7.5'}});
      if (!r.ok) return reply.status(502).send({error:`SRI respondió HTTP ${r.status}.`});
      const raw:any = await r.json();
      const item:any = Array.isArray(raw) ? raw[0] : raw?.data?.[0] ?? raw?.data ?? raw;
      if (!item) return reply.status(404).send({error:'No se encontró información para ese RUC en el catastro consultado.'});
      const datos:any = {
        ruc: item.numeroRuc ?? item.ruc ?? ruc,
        ...
        estado: item.estadoContribuyenteRuc ?? item.estadoContribuyente ?? item.estado ?? '',
        tipoContribuyente: item.tipoContribuyente ?? item.tipo_contribuyente ?? '',
        subtipoContribuyente: item.subtipoContribuyente ?? item.subtipo_contribuyente ?? '',
        claseContribuyente: item.claseContribuyente ?? item.clase_contribuyente ?? '',
        ...
        fechaInicioActividades: item.fechaInicioActividades ?? item.fecha_inicio_actividades ?? null,
        fechaActualizacion: item.fechaActualizacion ?? item.fecha_actualizacion ?? null,
        ...
        agenteRetencion: item.agenteRetencion ?? item.agente_retencion ?? null,
        contribuyenteEspecial: item.contribuyenteEspecial ?? item.contribuyente_especial ?? null,
        establecimientos: item.establecimientos ?? item.listaEstablecimientos ?? item.establecimiento ?? [],
        fuente:'SRI'
      };
      return reply.send({ok:true,encontrado:true,datos,raw});
    } catch (e) { request.log.error({err:e},'Consulta RUC SRI falló'); return reply.status(502).send({error:'No fue posible consultar el catastro del SRI en este momento.'}); }
  });
  ...
  app.post<{ Body: { ruc?: string; razonSocial?: string; nombreComercial?: string; direccionMatriz?: string; emailAdmin?: string; passwordTemporal?: string; planCodigo?: string; ambiente?: 'pruebas'|'produccion'; diasIniciales?: number } }>('/proveedor/clientes', async (request, reply) => {
    ...
    const { data: cuenta, error: cuentaError } = await supabase.from('cuentas_cliente_saas').insert({ nombre: razonSocial, email_admin: email, admin_user_id: userId, plan_id: plan.id, estado: 'activa' }).select('*').single();
  ...
  app.patch<{ Params: { emisorId: string }; Body: { estado?: Estado; dias?: number; nota?: string; planCodigo?: string } }>('/proveedor/clientes/:emisorId/suscripcion', async (request, reply) => {
    if (!await exigirProveedor(request, reply)) return;
    const b = request.body ?? {};
    ...
    if (b.planCodigo !== undefined) {
      const codigo=String(b.planCodigo).trim().toUpperCase();
      const {data:plan,error:pErr}=await supabase.from('planes_suscripcion').select('id,codigo,nombre,precio_mensual').eq('codigo',codigo).eq('activo',true).maybeSingle();
      if(pErr||!plan) return reply.status(400).send({error:`El plan ${codigo} no existe o está inactivo.`});
      cambios.plan_id=plan.id;
      const {data:rel}=await supabase.from('contribuyentes_cliente_saas').select('cuenta_id').eq('emisor_id',request.params.emisorId).eq('activo',true).maybeSingle();
  ...
  app.patch<{ Params: { emisorId: string }; Body: { habilitado?: boolean; motivo?: string } }>('/proveedor/clientes/:emisorId/ticket-pos', async (request, reply) => {
    const auth = await exigirProveedor(request, reply);
    if (!auth) return;
    const emisorId = request.params.emisorId;
    const habilitado = request.body?.habilitado === true;
    const motivo = String(request.body?.motivo ?? '').trim().slice(0, 500) || null;

    const { data: rel, error: relError } = await supabase.from('contribuyentes_cliente_saas')
      .select('id,cuenta_id,activo,ticket_pos_habilitado').eq('emisor_id', emisorId).maybeSingle();
    if (relError) return reply.status(500).send({ error: relError.message });
    if (!rel) return reply.status(404).send({ error: 'El contribuyente no está registrado en el esquema SaaS.' });
    if (!rel.activo) return reply.status(409).send({ error: 'El contribuyente está inactivo en el Panel Maestro.' });

    const { data: cuenta, error: cuentaError } = await supabase.from('cuentas_cliente_saas')
      .select('estado,planes_suscripcion(incluye_ticket_pos,nombre)').eq('id', rel.cuenta_id).maybeSingle();
    if (cuentaError) return reply.status(500).send({ error: cuentaError.message });
    if (habilitado && cuenta?.planes_suscripcion && (cuenta.planes_suscripcion as any).incluye_ticket_pos !== true) {
      return reply.status(409).send({ error: `No se puede activar Ticket/POS: el plan ${(cuenta.planes_suscripcion as any).nombre ?? ''} no incluye esta función. Activa primero Ticket/POS en el plan desde Planes y límites comerciales.` });
    }

    const anterior = rel.ticket_pos_habilitado === true;
    const { data: actualizado, error } = await supabase.from('contribuyentes_cliente_saas')
      .update({ ticket_pos_habilitado: habilitado, ticket_pos_updated_at: new Date().toISOString(), ticket_pos_updated_by: auth.userId, ticket_pos_motivo: motivo })
      .eq('id', rel.id)
      .select('id,emisor_id,ticket_pos_habilitado,ticket_pos_updated_at,ticket_pos_updated_by,ticket_pos_motivo').single();
    if (error) return reply.status(500).send({ error: error.message });

    const { error: auditError } = await supabase.from('auditoria_ticket_pos').insert({
      cuenta_id: rel.cuenta_id, emisor_id: emisorId, habilitado, estado_anterior: anterior,
      motivo, realizado_por: auth.userId,
    });
    if (auditError) request.log.warn({ err: auditError }, 'No se pudo registrar la auditoría Ticket/POS');

    return reply.send({ ok: true, ticketPos: actualizado, mensaje: habilitado ? 'Ticket/POS habilitado por ROOT para este contribuyente.' : 'Ticket/POS deshabilitado por ROOT para este contribuyente.' });
  });
