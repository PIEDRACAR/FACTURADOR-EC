...
import { contabilizarMovimientoInventario } from '../services/motorContable.js';
...
export async function registrarRutasInventario(app: FastifyInstance) {
  ...
  app.post<{
    ...
    let contabilidad: any = {estado:'pendiente'};
    ...
    try { contabilidad = await contabilizarMovimientoInventario(resultado.movimiento_id, request.usuarioSesion?.userId); }
    catch (accountingError) { request.log.error({err:accountingError,movimientoId:resultado.movimiento_id},'Entrada de inventario registrada pero pendiente de contabilización automática.'); contabilidad={estado:'pendiente',advertencia:accountingError instanceof Error?accountingError.message:String(accountingError)}; }
  ...
  app.post<{
    ...
    let contabilidad:any={estado:'pendiente'};
    try {
      const mov=await supabase.from('movimientos_inventario').select('id').eq('producto_id',productoId).eq('tipo','ajuste').eq('referencia_tipo','ajuste_manual').order('created_at',{ascending:false}).limit(1).maybeSingle();
      if(mov.error||!mov.data) throw new Error(mov.error?.message||'No se pudo localizar el movimiento del ajuste.');
      contabilidad=await contabilizarMovimientoInventario(mov.data.id,request.usuarioSesion?.userId);
    }
    catch(accountingError){ request.log.error({err:accountingError},'Ajuste de inventario registrado pero pendiente de contabilización automática.'); contabilidad={estado:'pendiente',advertencia:accountingError instanceof Error?accountingError.message:String(accountingError)}; }
    return reply.status(201).send({ stockResultante: data[0].stock_resultante, contabilidad });
  });
  ...
  app.get<{ Querystring: { emisorId?: string; tipo?: string; formato?: string; productoId?: string } }>(
    '/inventario/exportar',
    async (request, reply) => {
      ...
      if (!['kardex', 'movimientos', 'semaforo', 'valorizado'].includes(tipo)) return reply.status(400).send({ error: 'Tipo de exportación inválido.' });
      if (!['excel', 'pdf'].includes(formato)) return reply.status(400).send({ error: 'formato debe ser excel o pdf.' });

      type Col = { clave: string; etiqueta: string; ancho: number; alinearDerecha?: boolean };
      let titulo = '';
      let columnas: Col[] = [];
      ...
      try {
        if (tipo === 'kardex' || tipo === 'movimientos') {
          ...
          filas = (data ?? []).map((m) => {
            const p = m.productos as unknown as { codigo_principal: string; descripcion: string } | null;
            return {
