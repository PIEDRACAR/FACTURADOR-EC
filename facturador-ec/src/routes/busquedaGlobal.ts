...
import { obtenerPermisosUsuario } from '../auth/permisos.js';
...
export async function registrarRutaBusquedaGlobal(app: FastifyInstance) {
  app.get<{ Querystring: { emisorId?: string; q?: string } }>('/buscar-global', async (request, reply) => {
    ...
    const patron = `%${q}%`;
    const permisos = await obtenerPermisosUsuario(request.usuarioSesion?.userId ?? '', emisorId, request.usuarioSesion?.rol ?? '');
    const [facturasR, asientosR, clientesR, productosR] = await Promise.all([
      ...
      permisos.has('contabilidad') ? supabase.from('asientos_contables')
        ...
        .order('fecha', { ascending: false }).limit(10) : Promise.resolve({ data: [], error: null } as any),
      permisos.has('clientes') ? supabase.from('clientes')
        ...
        .order('razon_social', { ascending: true }).limit(10) : Promise.resolve({ data: [], error: null } as any),
      permisos.has('productos') ? supabase.from('productos')
        .select('id,codigo_principal,descripcion,precio_venta')
        .eq('emisor_id', emisorId)
        .or(`codigo_principal.ilike.${patron},descripcion.ilike.${patron}`)
        .order('descripcion', { ascending: true }).limit(10) : Promise.resolve({ data: [], error: null } as any),
    ]);
