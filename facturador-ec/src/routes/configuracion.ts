...
import { obtenerMotorResumen } from '../services/motorTributario.js';
...
export async function registrarRutasConfiguracion(app: FastifyInstance) {
  ...
  app.get<{ Querystring:{emisorId?:string} }>('/configuracion/motor-tributario', async (request, reply) => {
    const emisorId=request.query.emisorId; if(!emisorId) return reply.status(400).send({error:'Falta emisorId.'});
    try{return reply.send(await obtenerMotorResumen(emisorId));}catch(e){return reply.status(500).send({error:e instanceof Error?e.message:String(e)});}
  });

  app.patch<{ Body:{emisorId?:string; esTuristico?:boolean; registroTurismo?:boolean; numeroRegistroTurismo?:string; luafVigente?:boolean; fechaVencimientoLuaf?:string; actividadEconomicaPrincipal?:string; codigoCiiu?:string} }>('/configuracion/motor-tributario', async (request, reply) => {
    const b=request.body??{}; if(!b.emisorId) return reply.status(400).send({error:'Falta emisorId.'});
    const {data,error}=await supabase.from('perfil_tributario_emisor').upsert({emisor_id:b.emisorId,es_turistico:!!b.esTuristico,registro_turismo:!!b.registroTurismo,numero_registro_turismo:String(b.numeroRegistroTurismo??'').trim()||null,luaf_vigente:!!b.luafVigente,fecha_vencimiento_luaf:b.fechaVencimientoLuaf||null,actividad_economica_principal:String(b.actividadEconomicaPrincipal??'').trim()||null,codigo_ciiu:String(b.codigoCiiu??'').trim()||null,updated_at:new Date().toISOString(),fuente:'manual'},{onConflict:'emisor_id'}).select('*').single();
