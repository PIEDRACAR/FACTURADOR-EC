...
export async function registrarRutasConfiguracion(app: FastifyInstance) {
  ...
  /**
   * Firma electrónica: cada contribuyente configura su propio certificado .p12/.pfx.
   * El archivo y la contraseña se cifran antes de almacenarse; nunca se devuelven al navegador.
   */
  app.get<{ Querystring: { emisorId?: string } }>('/configuracion/firma', async (request, reply) => {
    const emisorId = String(request.query.emisorId ?? '').trim();
    if (!emisorId) return reply.status(400).send({ error:'Falta emisorId.' });
    ...
    const rol=await obtenerRolEnNegocio(sesion.userId,emisorId); if(!rol)return reply.status(403).send({error:'No tienes acceso a este negocio.'});
    const {data,error}=await supabase.from('certificados').select('id,alias,fecha_expiracion,activo,created_at,updated_at').eq('emisor_id',emisorId).eq('alias','PRINCIPAL').maybeSingle();
    if(error)return reply.status(500).send({error:error.message});
    return reply.send({configurado:Boolean(data), certificado:data?{id:data.id,alias:data.alias,fecha_expiracion:data.fecha_expiracion,activo:data.activo,created_at:data.created_at,updated_at:data.updated_at}:null});
  });

  app.post<{ Body: { emisorId?:string; alias?:string; archivoBase64?:string; nombre?:string; password?:string; fechaExpiracion?:string } }>('/configuracion/firma', async (request, reply) => {
    const b=request.body??{}; const emisorId=String(b.emisorId??'').trim(); const alias='PRINCIPAL';
    const token=request.cookies?.sesion; const sesion=token?await obtenerSesion(token):null;
    if(!sesion)return reply.status(401).send({error:'Sesión no válida.'});
    const rol=await obtenerRolEnNegocio(sesion.userId,emisorId); if(!rol)return reply.status(403).send({error:'No tienes acceso a este negocio.'});
    const raw=String(b.archivoBase64??'').trim(); const pass=String(b.password??''); const fecha=String(b.fechaExpiracion??'').trim();
    if(!emisorId||!raw||!pass||!fecha)return reply.status(400).send({error:'Contribuyente, archivo .p12/.pfx, contraseña y fecha de expiración son obligatorios.'});
    if(!/^\d{4}-\d{2}-\d{2}$/.test(fecha))return reply.status(400).send({error:'La fecha de expiración debe tener formato AAAA-MM-DD.'});
    if(new Date(`${fecha}T23:59:59-05:00`).getTime()<Date.now())return reply.status(400).send({error:'El certificado está vencido. Selecciona la fecha real de expiración.'});
    const base64=raw.includes(',')&&raw.startsWith('data:')?raw.slice(raw.indexOf(',')+1):raw;
    let buffer:Buffer; try{buffer=Buffer.from(base64,'base64')}catch{return reply.status(400).send({error:'El archivo de firma no es válido.'});}
    if(!buffer.length)return reply.status(400).send({error:'El archivo de firma está vacío.'});
    if(buffer.length>5*1024*1024)return reply.status(413).send({error:'El certificado no puede superar 5 MB.'});
    // PKCS#12 normalmente comienza con una estructura ASN.1 DER SEQUENCE (30 xx).
    if(buffer[0]!==0x30)return reply.status(400).send({error:'El archivo no parece ser un certificado PKCS#12 válido. Usa .p12 o .pfx.'});
    const {data:emisor}=await supabase.from('emisores').select('id').eq('id',emisorId).maybeSingle(); if(!emisor)return reply.status(404).send({error:'Contribuyente no encontrado.'});
    const payload={
      emisor_id:emisorId, alias, fecha_expiracion:fecha, activo:true,
      p12_cifrado:bufferAPgBytea(cifrar(buffer)),
      p12_password_cifrado:bufferAPgBytea(cifrarTexto(pass)),
      referencia_almacenamiento:String(b.nombre??'certificado.p12').slice(0,120),
      updated_at:new Date().toISOString()
    } as any;
    const {data,error}=await supabase.from('certificados').upsert(payload,{onConflict:'emisor_id,alias'}).select('id,alias,fecha_expiracion,activo').single();
    if(error)return reply.status(500).send({error:`No se pudo guardar la firma electrónica: ${error.message}`});
    return reply.send({ok:true,certificado:data,mensaje:'Firma electrónica configurada correctamente. El archivo y la contraseña quedaron almacenados cifrados.'});
  });

  app.delete<{ Querystring:{emisorId?:string} }>('/configuracion/firma', async (request, reply) => {
    const emisorId=String(request.query.emisorId??'').trim(); const token=request.cookies?.sesion; const sesion=token?await obtenerSesion(token):null;
    if(!sesion)return reply.status(401).send({error:'Sesión no válida.'});
    const rol=await obtenerRolEnNegocio(sesion.userId,emisorId); if(!rol)return reply.status(403).send({error:'No tienes acceso a este negocio.'});
    const {error}=await supabase.from('certificados').update({activo:false,updated_at:new Date().toISOString()}).eq('emisor_id',emisorId).eq('alias','PRINCIPAL');
    if(error)return reply.status(500).send({error:error.message}); return reply.send({ok:true,mensaje:'Firma electrónica desactivada. El certificado almacenado no se elimina para conservar trazabilidad.'});
  });
