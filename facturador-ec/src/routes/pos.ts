...
function normalizarTarifaIva(valor: unknown, cfg = DEFAULT_IVA): { tarifa: string; codigo: string; tasa: number; grupo: '0'|'5'|'8'|'general'|'otros' } {
  ...
  if (Math.abs(n-cfg.general)<0.0001 || raw === '15' || raw === '13') return { tarifa:String(cfg.general), codigo:cfg.codigoGeneral, tasa:cfg.general/100, grupo:'general' };
  // 15% es la tarifa general vigente del sistema.
...
export async function registrarRutasPos(app: FastifyInstance) {
  app.get<{ Querystring: { emisorId?: string } }>('/pos/configuracion-iva', async (request, reply) => {
    ...
    const { data, error } = await supabase.from('configuracion_iva')
      .select('tarifa_general,codigo_general,tarifa_reducida,codigo_reducida,tarifa_turismo,codigo_turismo,turismo_habilitado,registro_turismo,luaf,activo')
    ...
    return reply.send({ ...(data ?? { tarifa_general:15, codigo_general:'4', tarifa_reducida:5, codigo_reducida:'5', tarifa_turismo:8, codigo_turismo:'8', turismo_habilitado:false, activo:true }), turismo_vigente: Boolean(turismoVigencia?.length), turismo_automatico: Boolean(data?.turismo_habilitado && turismoVigencia?.length) });
  });
  ...
  app.post<{
    ...
    if (b?.codigoPrincipal && String(b.codigoPrincipal).length > 25) return reply.status(400).send({ error: 'El código principal no puede superar 25 caracteres porque el SRI lo limita a esa longitud.' });
  ...
  app.patch<{
    ...
    if (b.codigoPrincipal !== undefined) {
      if (String(b.codigoPrincipal).length > 25) return reply.status(400).send({ error: 'El código principal no puede superar 25 caracteres por requisito XSD del SRI.' });
      cambios.codigo_principal = String(b.codigoPrincipal).trim();
    }
  ...
  app.post<{ Body: VentaBody }>('/pos/venta', async (request, reply) => {
    ...
    const { data: cfgIvaRow } = await supabase.from('configuracion_iva').select('tarifa_general,codigo_general,tarifa_reducida,codigo_reducida,tarifa_turismo,codigo_turismo,turismo_habilitado,registro_turismo,luaf,activo').eq('emisor_id', body.emisorId).maybeSingle();
    const cfgIva = cfgIvaRow ? {
      general: Number(cfgIvaRow.tarifa_general ?? 15), codigoGeneral: String(cfgIvaRow.codigo_general ?? '4'),
    ...
    const fechaVenta = fechaIsoEcuador();
    const { data: reglaIvaEspecial } = await supabase.from('catalogo_iva_sri')
      .select('id,codigo_porcentaje,tarifa,sector,fecha_desde,fecha_hasta,base_legal,aplicacion_automatica,requiere_turismo')
      .eq('codigo_porcentaje', String(cfgIva.codigoTurismo))
      .eq('activo', true)
      .eq('aplicacion_automatica', true)
      .lte('fecha_desde', fechaVenta)
      .or(`fecha_hasta.is.null,fecha_hasta.gte.${fechaVenta}`)
      .limit(1).maybeSingle();
    const turismoAutomatico = Boolean((cfgIvaRow as any)?.turismo_habilitado && reglaIvaEspecial);
    ...
    for (const item of body.items) {
      ...
      if (item.productoId) {
        ...
        if (String(producto.codigo_principal).length > 25) {
          return reply.status(409).send({ error: `El producto ${producto.descripcion} tiene un código principal de más de 25 caracteres. Corrígelo antes de facturar.` });
        }
        codigoParaFactura = producto.codigo_principal;
        ...
        if (item.precioUnitario !== undefined && Math.abs(precioUnitario - precioCatalogo) > 0.001) {
          if (!['admin','contador'].includes(String(request.usuarioSesion?.rol ?? ''))) {
            return reply.status(403).send({ error: 'Solo un administrador o contador puede autorizar un precio manual distinto al catálogo.' });
          }
        ...
        const tarifaSolicitada = normalizarTarifaIva(item.tarifaIva ?? producto.tarifa_iva, cfgIva);
        tarifaIva = (turismoAutomatico && (tarifaSolicitada.grupo === 'general')) ? String(cfgIva.turismo) : tarifaSolicitada.tarifa;
        ...
        const tarifaSolicitada = normalizarTarifaIva(item.tarifaIvaLibre ?? String(cfgIva.general), cfgIva);
        tarifaIva = (turismoAutomatico && tarifaSolicitada.grupo === 'general') ? String(cfgIva.turismo) : tarifaSolicitada.tarifa;
        costoUnitario = 0;
      }
      ...
      if (tax.codigo === '8') {
        ...
        if (!Boolean((cfgIvaRow as any)?.turismo_habilitado)) {
          return reply.status(400).send({
            error: 'El emisor no está habilitado por ROOT para aplicar la tarifa turística 8%. Registra la elegibilidad tributaria desde el Panel Maestro.'
          });
        }
      }
    ...
    if (modo === 'factura') {
      ...
      const { data: cliente, error: errorCliente } = await supabase
        ...
        .select('id')
        .single();
      if (errorCliente || !cliente) return reply.status(500).send({ error: errorCliente?.message ?? 'No se pudo resolver el cliente.' });
      clienteId = cliente.id;

      // Validación tributaria únicamente para factura electrónica. Un ticket
      // interno nunca se firma ni se transmite al SRI.
      validarFacturaAntesDeGuardar({
        ...
        detalles: detallesFactura.map((d) => ({
          codigoPrincipal: String(d.codigoPrincipal), descripcion: String(d.descripcion),
          cantidad: Number(d.cantidad), precioUnitario: Number(d.precioUnitario),
          descuento: Number(d.descuento), precioTotalSinImpuesto: Number(d.precioTotalSinImpuesto),
        })),
        pagos: body.pagos.map((p) => ({ formaPago: p.formaPagoCodigo, total: Number(p.valor) })),
        importeTotal,
      });
    }
    ...
    if (modo === 'ticket') {
      // Ticket interno: queda claramente separado del flujo SRI. No genera XML,
      // clave de acceso, RIDE ni correo. Sí descuenta inventario y queda listo
      // para caja, reportes y contabilidad.
      const { data: ticketSeq, error: ticketSeqError } = await supabase.rpc('increment_secuencial', {
        p_emisor_id: body.emisorId,
        p_establecimiento: puntoEmision.establecimiento,
        p_punto_emision: puntoEmision.punto_emision,
        p_columna: 'secuencial_ticket',
      });
      if (ticketSeqError) return reply.status(500).send({ error: `No se pudo obtener el número del ticket: ${ticketSeqError.message}` });
      const ticketNumero = String(ticketSeq).padStart(9, '0');
      const { error: estadoTicket } = await supabase.from('comprobantes').update({ estado: 'registrado', secuencial: ticketNumero }).eq('id', comprobanteId);
      if (estadoTicket) return reply.status(500).send({ error: `No se pudo finalizar el ticket: ${estadoTicket.message}` });
      let contabilidad: any = null;
      try { contabilidad = await contabilizarVenta(comprobante.id, request.usuarioSesion?.userId); }
      catch (contErr) { request.log.error(contErr, 'No se pudo contabilizar automáticamente el ticket'); }
      return reply.status(201).send({
        comprobanteId: comprobante.id, estado: 'TICKET_REGISTRADO', tipo: 'ticket',
        importeTotal, secuencial: ticketNumero,
        contabilidad: contabilidad?.estado ?? 'PENDIENTE',
        mensaje: 'Ticket interno registrado. No es un comprobante electrónico SRI y no reemplaza la factura cuando esta sea obligatoria.'
      });
    }
    ...
    // emitidas. Esto es lo que evita inconsistencias entre 15%, 8%, 5%, 0%,
