...
export async function registrarRutasPos(app: FastifyInstance) {
  /** Lista los productos activos de un emisor, para el buscador del carrito. */
  app.get<{ Querystring: { emisorId?: string } }>('/productos', async (request, reply) => {
    ...
    const { data, error } = await supabase
      .from('productos')
      .select('id, codigo_principal, descripcion, precio_venta, tarifa_iva, stock_actual')
