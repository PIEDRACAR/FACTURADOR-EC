...
export async function registrarRutasPos(app: FastifyInstance) {
  app.get<{ Querystring: { emisorId?: string } }>('/pos/configuracion-iva', async (request, reply) => {
    ...
    return reply.send({ ...(data ?? { tarifa_general:15, codigo_general:'4', tarifa_reducida:5, codigo_reducida:'5', tarifa_turismo:8, codigo_turismo:'8', activo:true }), turismo_vigente: Boolean(turismoVigencia?.length) });
  });
