...
import { comprobarLimiteDocumentos, comprobarCaracteristica } from '../services/saas.js';
import { resolverIva } from '../services/motorTributario.js';
/**
...
// El catálogo tributario vive en Supabase. Estos códigos se usan solo como respaldo
// para instalaciones que todavía no ejecutaron la migración v9.7.5.
const CODIGO_PORCENTAJE_IVA: Record<string,string> = { '0':'0','5':'5','8':'8','15':'4','general':'4',exento:'7',no_objeto:'6' };
const TASA_POR_TARIFA: Record<string,number> = { '0':0,'5':.05,'8':.08,'15':.15,general:.15,exento:0,no_objeto:0 };
...
export async function registrarRutasPos(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { emisorId?: string } }>('/pos/puntos-emision', async (request, reply) => {
    ...
    const accesoTicketPos = await comprobarCaracteristica(emisorId, 'ticket_pos');
    if (!accesoTicketPos.ok) return reply.status(403).send({ error: accesoTicketPos.mensaje });
    const { data, error } = await supabase
      .from('puntos_emision')
  ...
  app.post<{ Body: VentaBody }>('/pos/venta', async (request, reply) => {
    ...
    const accesoTicketPos = await comprobarCaracteristica(body.emisorId, 'ticket_pos');
    if (!accesoTicketPos.ok) return reply.status(403).send({ error: accesoTicketPos.mensaje });
    ...
    const itemsParaGuardar: Array<{
      ...
      regla_tributaria_id: string | null;
    }> = [];
    ...
    let subtotal15 = 0;
    let subtotalOtros = 0;
    const desgloseIva: Record<string,{tarifa:number,base:number,valor:number,codigoPorcentaje:string}> = {};
    ...
    for (const item of body.items) {
      ...
      if (item.productoId) {
        ...
        tarifaIva = producto.tarifa_iva || 'general';
        ...
        tarifaIva = item.tarifaIvaLibre ?? 'general';
        costoUnitario = 0;
      }

      const precioTotalSinImpuesto = redondear(item.cantidad * precioUnitario - descuento);
      const regla = await resolverIva(body.emisorId, tarifaIva, fechaEmisionEcuador());
      const codigoPorcentaje = regla.codigoPorcentaje || CODIGO_PORCENTAJE_IVA[tarifaIva] || '';
      const porcentajeIva = regla.porcentaje;
      const valorIva = redondear(precioTotalSinImpuesto * porcentajeIva);
      const tarifaResuelta = regla.tarifa;

      if (tarifaResuelta === '15') subtotal15 = redondear(subtotal15 + precioTotalSinImpuesto);
      else if (tarifaResuelta === '8') subtotal8 = redondear(subtotal8 + precioTotalSinImpuesto);
      else if (tarifaResuelta === '5') subtotal5 = redondear(subtotal5 + precioTotalSinImpuesto);
      else if (porcentajeIva > 0) subtotalOtros = redondear(subtotalOtros + precioTotalSinImpuesto);
      else subtotal0 = redondear(subtotal0 + precioTotalSinImpuesto);
      const clave = tarifaResuelta;
      desgloseIva[clave] = desgloseIva[clave] || { tarifa:Number(tarifaResuelta), base:0, valor:0, codigoPorcentaje };
      desgloseIva[clave].base = redondear(desgloseIva[clave].base + precioTotalSinImpuesto);
      desgloseIva[clave].valor = redondear(desgloseIva[clave].valor + valorIva);
      ...
      itemsParaGuardar.push({
        ...
        tarifa_iva: tarifaResuelta,
        valor_iva: valorIva,
        codigo_porcentaje_iva: codigoPorcentaje,
        regla_tributaria_id: regla.reglaId,
      });
    ...
    const totalSinImpuestos = redondear(subtotal0 + subtotal5 + subtotal8 + subtotal15 + subtotalOtros);
    ...
    const { data: comprobanteId, error: errorVenta } = await supabase.rpc('crear_venta_dinamica', {
    ...
    if (subtotal8 > 0) {
      totalConImpuestos.push({
        codigo: '2',
        codigoPorcentaje: '8',
        baseImponible: subtotal8,
        valor: redondear(subtotal8 * 0.08),
      });
    }
    if (subtotal15 > 0) {
      totalConImpuestos.push({
