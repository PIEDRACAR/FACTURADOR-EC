...
import { resolverIva, perfilDesdeTarifa, porcentajeDecimal, codigoSRI, redondearImpuesto, type PerfilIva } from '../services/impuestos.js';
/**
...
interface ItemCarritoBody {
  ...
  tarifaIvaLibre?: '0' | '5' | '8' | '15' | 'exento' | 'no_objeto';
  perfilIvaLibre?: PerfilIva;
  cantidad: number;
  descuento?: number;
}
...
export async function registrarRutasPos(app: FastifyInstance) {
  ...
  app.patch<{
    ...
    if (b.perfilIva !== undefined) cambios.perfil_iva = b.perfilIva;
  ...
  app.post<{ Body: VentaBody }>('/pos/venta', async (request, reply) => {
    ...
    const productosPorId = new Map<
      string,
      { codigo_principal: string; descripcion: string; precio_venta: number; tarifa_iva: string; perfil_iva?: PerfilIva | null; costo_promedio: number; stock_actual: number }
    >();
    ...
    for (const item of body.items) {
      ...
      if (item.productoId) {
        ...
        perfilIva = (producto.perfil_iva as PerfilIva | null) || perfilDesdeTarifa(producto.tarifa_iva);
      ...
      if (reglaIva.tipo === 'GENERAL') subtotal15 = redondear(subtotal15 + precioTotalSinImpuesto);
      else if (tarifaAplicada === 8) subtotal8 = redondear(subtotal8 + precioTotalSinImpuesto);
      else if (tarifaAplicada === 5) subtotal5 = redondear(subtotal5 + precioTotalSinImpuesto);
    ...
    const impuestosAgrupados = new Map<string, { codigo: string; tarifa: number; base: number; valor: number }>();
    ...
    const totalConImpuestos: TotalTax[] = [...impuestosAgrupados.entries()].map(([key, x]) => {
