...
export async function registrarRutasPos(app: FastifyInstance) {
  ...
  app.post<{ Body: VentaBody }>('/pos/venta', async (request, reply) => {
    ...
    for (const item of body.items) {
      ...
      else if (tarifaIva === '8') subtotal8 = redondear(subtotal8 + precioTotalSinImpuesto);
    ...
    try {
      ...
      const { data: confProveedor } = await supabase.from('configuracion_sistema').select('ruc_proveedor_facturacion').eq('emisor_id', emisorId).maybeSingle();
