...
export async function registrarRutasProformas(app: FastifyInstance) {
  ...
  app.post<{ Params: { id: string } }>('/proformas/:id/convertir', async (request, reply) => {
    ...
    const cliente = proforma.clientes as { tipo_identificacion: string; identificacion: string; razon_social: string; direccion: string | null; email?: string | null };
    ...
    try {
      ...
      // Una conversión de proforma aceptada sigue el mismo estándar documental
      // que una venta normal: si el SRI autoriza, el XML y el RIDE deben quedar
      // archivados y el cliente debe recibirlos automáticamente por correo.
      let archivos: Array<{ tipo: string; nombre_archivo: string; url: string | null }> = [];
      let emailEnviado = false;
      let emailError: string | null = null;
      if (resultado.estado === 'AUTORIZADO' && resultado.xmlFirmado && resultado.claveAcceso) {
        try {
          // Reintento seguro: si el primer archivado dentro de emitirFactura falló
          // por Storage o por un índice temporal, aquí se recupera sin alterar la
          // autorización del SRI. La operación es idempotente (upsert).
          await archivarComprobanteAutorizado({
            comprobanteId: comprobanteId as string,
            emisorId: proforma.emisor_id,
            claveAcceso: resultado.claveAcceso,
            xmlFirmado: resultado.xmlFirmado,
            secuencial: resultado.secuencial,
          });
        } catch (archiveError) {
          emailError = `No se pudo completar el archivo permanente: ${archiveError instanceof Error ? archiveError.message : String(archiveError)}`;
        }

        try {
          archivos = await obtenerArchivosComprobante(comprobanteId as string);
        } catch (archiveIndexError) {
          if (!emailError) emailError = `No se pudo consultar el archivo documental: ${archiveIndexError instanceof Error ? archiveIndexError.message : String(archiveIndexError)}`;
        }

        const emailCliente = String(cliente.email ?? '').trim();
        if (emailCliente) {
          try {
            const ride = await generarRidePdf(comprobanteId as string);
            await enviarComprobantePorCorreo({
              to: emailCliente,
              subject: `Factura electrónica CONTSERTRIB ${resultado.secuencial ?? ''}`.trim(),
              html: `<div style=\"font-family:Arial,sans-serif;color:#172033;max-width:640px;margin:auto\"><h2 style=\"color:#1d4ed8\">Factura electrónica autorizada</h2><p>Estimado/a <strong>${String(cliente.razon_social ?? '').replace(/[<>]/g,'')}</strong>:</p><p>Su proforma fue convertida en venta y la factura electrónica fue <strong>AUTORIZADA por el SRI</strong>.</p><p>Adjuntamos el <strong>RIDE en PDF</strong> y el <strong>XML firmado</strong>.</p><p style=\"font-size:12px;color:#64748b\">Clave de acceso: ${resultado.claveAcceso}<br>Número de autorización: ${resultado.numeroAutorizacion ?? '—'}</p></div>`,
              attachments: [
                { filename: `factura-${resultado.secuencial ?? comprobanteId}.pdf`, content: ride.toString('base64'), type: 'application/pdf' },
                { filename: `${resultado.claveAcceso}.xml`, content: Buffer.from(resultado.xmlFirmado, 'utf8').toString('base64'), type: 'application/xml' },
              ],
            });
            emailEnviado = true;
          } catch (mailError) {
            emailError = `${emailError ? emailError + ' | ' : ''}No se pudo enviar la factura al cliente: ${mailError instanceof Error ? mailError.message : String(mailError)}`;
          }
        } else {
          emailError = `${emailError ? emailError + ' | ' : ''}El cliente no tiene un correo electrónico registrado.`;
        }
      }
      ...
      return reply.status(201).send({
        ...
        importeTotal,
        emailEnviado,
        emailDestinatario: cliente.email ?? null,
        emailError,
        archivos,
      });
    } catch (err) {
