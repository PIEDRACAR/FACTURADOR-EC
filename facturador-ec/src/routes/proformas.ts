...
const CODIGO_PORCENTAJE_IVA: Record<string, string> = { '0': '0', '5': '5', '8': '8', '15': '4', exento: '7', no_objeto: '6' };
const TASA_POR_TARIFA: Record<string, number> = { '0': 0, '5': 0.05, '8': 0.08, '15': 0.15, general:0.15, exento: 0, no_objeto: 0 };
...
export async function registrarRutasProformas(app: FastifyInstance) {
  ...
  app.post<{ Params: { id: string } }>('/proformas/:id/convertir', async (request, reply) => {
    ...
    for (const item of proforma.proforma_items as Array<{
      ...
      const tarifaBase = item.producto_id ? tarifasPorProducto.get(item.producto_id) ?? 'general' : 'general';
      const base = redondear(item.cantidad * item.precio_unitario - item.descuento);
      const regla = await resolverIva(proforma.emisor_id, tarifaBase);
      const tarifaIva = regla.tarifa;
      const codigoPorcentaje = regla.codigoPorcentaje || CODIGO_PORCENTAJE_IVA[tarifaBase] || '4';
      ...
      else if (porcentaje > 0) subtotalOtros = redondear(subtotalOtros + base);
      else subtotal0 = redondear(subtotal0 + base);
      const clave=tarifaIva;
      desgloseIva[clave]=desgloseIva[clave]||{tarifa:Number(tarifaIva),base:0,valor:0,codigoPorcentaje};
      desgloseIva[clave].base=redondear(desgloseIva[clave].base+base);
    ...
    if (subtotal8 > 0) totalConImpuestos.push({ codigo: '2', codigoPorcentaje: '8', baseImponible: subtotal8, valor: redondear(subtotal8 * 0.08) });
