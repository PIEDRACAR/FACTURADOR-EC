...
export async function registrarRutasProformas(app: FastifyInstance) {
  ...
  app.post<{ Body: CrearProformaBody }>('/proformas', async (request, reply) => {
    ...
    const productosPorId = new Map<string, { descripcion: string; precio_venta: number; tarifa_iva: string; perfil_iva?: string | null }>();
  ...
  app.post<{ Params: { id: string } }>('/proformas/:id/convertir', async (request, reply) => {
    ...
    const perfilesPorProducto = new Map<string, any>();
    ...
    for (const item of proforma.proforma_items as Array<{
      ...
      if (regla.tipo === 'GENERAL') subtotal15 = redondear(subtotal15 + base);
      else if (tarifaAplicada === 8) subtotal8 = redondear(subtotal8 + base);
      else if (tarifaAplicada === 5) subtotal5 = redondear(subtotal5 + base);
    ...
    const impuestosAgrupados = new Map<string, { codigo: string; codigoPorcentaje: string; tarifa: number; base: number; valor: number }>();
    for (const d of detallesFactura) for (const imp of d.impuestos ?? []) {
      ...
      x.base = redondear(x.base + Number(imp.baseImponible)); x.valor = redondear(x.valor + Number(imp.valor)); impuestosAgrupados.set(key,x);
    }
    const totalConImpuestos: TotalTax[] = [...impuestosAgrupados.values()].map(x => ({ codigo:x.codigo, codigoPorcentaje:x.codigoPorcentaje, tarifa:x.tarifa, baseImponible:x.base, valor:x.valor }));
