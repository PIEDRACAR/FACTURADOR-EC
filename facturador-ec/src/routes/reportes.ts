...
async function obtenerListadoVentas(emisorId: string, desde?: string, hasta?: string, qTexto?: string, estadoFiltro?: string, tipoFiltro?: string) {
  ...
  // Evitamos consultas gigantes que pueden dejar la pantalla esperando indefinidamente.
  // El listado es un reporte interactivo: devolvemos una cantidad razonable y estable de filas.
  const { data, error } = await consulta.limit(3000);
  if (error) throw new Error(error.message);
