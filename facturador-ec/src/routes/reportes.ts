...
import { archivarComprobanteAutorizado } from '../services/archivoComprobante.js';
...
async function obtenerListadoVentas(emisorId: string, desde?: string, hasta?: string, qTexto?: string, estadoFiltro?: string, tipoFiltro?: string) {
  ...
  // IMPORTANTE: no hacemos el JOIN anidado `clientes(...)` en la consulta principal.
  // Ese JOIN puede volver muy pesada la respuesta de PostgREST cuando el emisor tiene
  // muchos comprobantes y es la principal causa de que la pantalla quede esperando.
  // Primero traemos solo comprobantes (consulta cubierta por índices) y luego resolvemos
  // los clientes en lotes pequeños.
  let consulta = supabase
    .from('comprobantes')
    .select('id, cliente_id, tipo, secuencial, created_at, importe_total, total_iva, subtotal_0, subtotal_5, subtotal_8, subtotal_15, estado, clave_acceso, numero_autorizacion')
  ...
  // 1.000 filas ofrecen una respuesta rápida y estable en pantalla.
  // Excel/PDF siguen teniendo su propia ruta de exportación y no pierden información.
  const { data, error } = await consulta.limit(1000);
  if (error) throw new Error(error.message);

  const filas = (data ?? []) as any[];
  const clienteIds = [...new Set(filas.map(v => v.cliente_id).filter(Boolean))] as string[];
  const clientesPorId = new Map<string, { razon_social?: string; email?: string; identificacion?: string }>();

  // Supabase/PostgREST maneja mejor lotes de tamaño moderado que un IN enorme.
  for (let i = 0; i < clienteIds.length; i += 250) {
    const lote = clienteIds.slice(i, i + 250);
    const { data: clientes, error: clientesError } = await supabase
      .from('clientes')
      .select('id, razon_social, email, identificacion')
      .in('id', lote);
    if (clientesError) throw new Error(`No se pudieron cargar los clientes del listado de ventas: ${clientesError.message}`);
    for (const c of clientes ?? []) clientesPorId.set(c.id, c);
  }

  const q = String(qTexto ?? '').trim().toLowerCase();
  const ventas = filas.map((v: any) => {
...
// ============================================================
// DESCARGA MASIVA DE XML / RIDE PDF
// ============================================================
// ZIP sin compresión: prioriza velocidad y bajo consumo de CPU. Los XML/PDF
// ya vienen comprimidos de forma eficiente cuando corresponde y el proceso
// se ejecuta en lotes para no saturar Railway.
...
function crearZipSinCompresion(entradas: Array<{ nombre: string; contenido: Buffer }>): Buffer {
  const locales: Buffer[] = [];
  const centrales: Buffer[] = [];
  let offset = 0;
  for (const entrada of entradas) {
    const nombre = Buffer.from(entrada.nombre.replace(/\\/g, '/'), 'utf8');
    const contenido = entrada.contenido;
    const crc = crc32(contenido);
    const local = Buffer.alloc(30 + nombre.length + contenido.length);
    ...
    contenido.copy(local, 30 + nombre.length);
    locales.push(local);
...
async function descargarArchivosMasivos(opts: {
  emisorId: string;
  desde?: string;
  hasta?: string;
  tipo?: string;
  formato?: 'xml' | 'pdf' | 'ambos';
  ids?: string[];
}) {
  const { fechaDesde, fechaHasta, inicioUtc, finUtc } = rangoFechas(opts.desde, opts.hasta);
  let consulta = supabase.from('comprobantes')
    .select('id,emisor_id,tipo,secuencial,clave_acceso,estado,created_at,xml_firmado')
    .eq('emisor_id', opts.emisorId).eq('estado', 'autorizado')
    .gte('created_at', inicioUtc).lt('created_at', finUtc)
    .order('created_at', { ascending: true });
  if (opts.tipo) consulta = consulta.eq('tipo', opts.tipo);
  if (opts.ids?.length) consulta = consulta.in('id', opts.ids);
  const { data: comprobantes, error } = await consulta.limit(5000);
  if (error) throw new Error(`No se pudieron consultar los comprobantes autorizados: ${error.message}`);

  const docs = (comprobantes ?? []) as any[];
  if (!docs.length) throw new Error('No existen comprobantes autorizados que coincidan con los filtros seleccionados.');

  // Garantiza que documentos autorizados antiguos también tengan XML/PDF archivados.
  const ids = docs.map(d => d.id);
  const archivos: any[] = [];
  for (let i = 0; i < ids.length; i += 500) {
    const lote = ids.slice(i, i + 500);
    const { data, error: ae } = await supabase.from('comprobante_archivos')
      .select('comprobante_id,tipo,bucket,storage_path,nombre_archivo,mime_type')
      .in('comprobante_id', lote);
    if (ae) throw new Error(`No se pudo consultar el archivo documental: ${ae.message}`);
    archivos.push(...(data ?? []));
  }
  const porComprobante = new Map<string, any[]>();
  for (const a of archivos) {
    const lista = porComprobante.get(a.comprobante_id) ?? [];
    lista.push(a); porComprobante.set(a.comprobante_id, lista);
  }

  const entradas: Array<{ nombre: string; contenido: Buffer }> = [];
  const manifest: string[] = ['fecha,tipo,secuencial,clave_acceso,xml,pdf,detalle'];
  const usados = new Set<string>();
  const formato = opts.formato ?? 'ambos';
  let xmlOk = 0, pdfOk = 0, omitidos = 0;

  const nombreUnico = (base: string) => {
    let n = base, i = 2;
    while (usados.has(n)) { const punto = base.lastIndexOf('.'); const raiz = punto > 0 ? base.slice(0, punto) : base; const ext = punto > 0 ? base.slice(punto) : ''; n = `${raiz}_${i++}${ext}`; }
    usados.add(n); return n;
  };

  for (const doc of docs) {
    let lista = porComprobante.get(doc.id) ?? [];
    if (lista.length < 2 && doc.xml_firmado && doc.clave_acceso) {
      try {
        await archivarComprobanteAutorizado({ comprobanteId: doc.id, emisorId: doc.emisor_id, claveAcceso: doc.clave_acceso, xmlFirmado: doc.xml_firmado, secuencial: doc.secuencial });
        const { data } = await supabase.from('comprobante_archivos').select('comprobante_id,tipo,bucket,storage_path,nombre_archivo,mime_type').eq('comprobante_id', doc.id);
        lista = data ?? [];
      } catch (e) {
        manifest.push(`${fechaIsoEcuador(new Date(doc.created_at))},${doc.tipo},${doc.secuencial},${doc.clave_acceso},NO,NO,"${String(e instanceof Error ? e.message : e).replace(/"/g,'""')}"`);
        omitidos++; continue;
      }
    }
    let xml = false, pdf = false;
    for (const a of lista) {
      const esXml = a.tipo === 'xml_firmado';
      const esPdf = a.tipo === 'ride_pdf';
      if ((esXml && formato === 'pdf') || (esPdf && formato === 'xml') || (!esXml && !esPdf)) continue;
      try {
        const { data, error: de } = await supabase.storage.from(a.bucket).download(a.storage_path);
        if (de || !data) throw new Error(de?.message || 'No se pudo descargar desde Storage');
        const contenido = Buffer.from(await data.arrayBuffer());
        const ext = esXml ? 'xml' : 'pdf';
        const carpeta = esXml ? 'XML' : 'PDF';
        const base = a.nombre_archivo || `${doc.clave_acceso || doc.id}.${ext}`;
        entradas.push({ nombre: nombreUnico(`${carpeta}/${base}`), contenido });
        if (esXml) { xml = true; xmlOk++; } else { pdf = true; pdfOk++; }
      } catch (e) {
        manifest.push(`${fechaIsoEcuador(new Date(doc.created_at))},${doc.tipo},${doc.secuencial},${doc.clave_acceso},${xml?'SI':'NO'},${pdf?'SI':'NO'},"${String(e instanceof Error ? e.message : e).replace(/"/g,'""')}"`);
      }
    }
    manifest.push(`${fechaIsoEcuador(new Date(doc.created_at))},${doc.tipo},${doc.secuencial},${doc.clave_acceso},${xml?'SI':'NO'},${pdf?'SI':'NO'},${xml && pdf ? 'OK' : 'Faltan archivos'}`);
    if (!xml && !pdf) omitidos++;
  }

  entradas.push({ nombre: 'MANIFEST.csv', contenido: Buffer.from(manifest.join('\n'), 'utf8') });
  entradas.push({ nombre: 'README.txt', contenido: Buffer.from(`CONTSERTRIB — Descarga masiva de comprobantes electrónicos\nPeríodo: ${fechaDesde} a ${fechaHasta}\nDocumentos consultados: ${docs.length}\nXML incluidos: ${xmlOk}\nPDF/RIDE incluidos: ${pdfOk}\nDocumentos con incidencias: ${omitidos}\nLos archivos corresponden a comprobantes autorizados por el SRI y archivados en CONTSERTRIB.\n`, 'utf8') });
  return { zip: crearZipSinCompresion(entradas), fechaDesde, fechaHasta, cantidadDocumentos: docs.length, xmlOk, pdfOk, omitidos };
}
...
async function obtenerMovimientosCajaDetallados(emisorId: string, desde?: string, hasta?: string, qTexto?: string) {
  ...
  const {data:pagos,error:pe}=await supabase.from('comprobantes').select('id,secuencial,tipo,created_at,importe_total,clientes(razon_social),comprobante_formas_pago(forma_pago_codigo,valor)').eq('emisor_id',emisorId).in('estado',['autorizado','registrado']).gte('created_at',inicioUtc).lt('created_at',finUtc).order('created_at',{ascending:false});
  if(pe) throw new Error(pe.message);
...
export async function registrarRutasReportes(app: FastifyInstance) {
  ...
  /** Descarga masiva de XML/RIDE de comprobantes autorizados. Procesa hasta 5.000 documentos por solicitud. */
  app.get<{ Querystring: { emisorId?: string; desde?: string; hasta?: string; tipo?: string; formato?: string; ids?: string } }>('/reportes/descarga-masiva', async (request, reply) => {
    const { emisorId, desde, hasta, tipo, formato, ids } = request.query;
    if (!emisorId) return reply.status(400).send({ error: 'Falta el parámetro emisorId.' });
    if (formato !== 'xml' && formato !== 'pdf' && formato !== 'ambos') return reply.status(400).send({ error: 'formato debe ser xml, pdf o ambos.' });
    try {
      const resultado = await descargarArchivosMasivos({
        emisorId, desde, hasta, tipo, formato: formato as 'xml'|'pdf'|'ambos',
        ids: ids ? ids.split(',').map(x => x.trim()).filter(Boolean).slice(0, 5000) : undefined,
      });
      const nombre = `CONTSERTRIB_COMPROBANTES_${resultado.fechaDesde}_${resultado.fechaHasta}.zip`;
      return reply.header('Cache-Control','no-store').header('Content-Type','application/zip').header('Content-Disposition',`attachment; filename="${nombre}"`).send(resultado.zip);
    } catch (err) {
