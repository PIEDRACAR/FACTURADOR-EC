...
async function obtenerMovimientosCajaDetallados(emisorId: string, desde?: string, hasta?: string, qTexto?: string) {
  ...
  const {data:movs,error:me}=ids.length?await supabase.from('movimientos_caja').select('id,caja_id,tipo,concepto,monto,forma_pago,created_at').in('caja_id',ids).gte('created_at',inicioUtc).lt('created_at',finUtc).order('created_at',{ascending:false}):{data:[],error:null};
