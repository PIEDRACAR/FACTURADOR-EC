...
async function obtenerReporteProveedores(emisorId: string, desde?: string, hasta?: string) {
  ...
  const { data, error } = await supabase.from('movimientos_inventario').select('cantidad,costo_unitario,proveedor_id').eq('emisor_id',emisorId).eq('tipo','entrada').not('proveedor_id','is',null).gte('created_at',fechaDesde).lte('created_at',hastaFinDia);
  if(error) throw new Error(error.message);
