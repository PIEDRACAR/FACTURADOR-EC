...
// ============================================================
async function obtenerListadoVentas(emisorId: string, desde?: string, hasta?: string, limite = 1000) {
  const { fechaDesde, fechaHasta, hastaFinDia } = rangoFechas(desde, hasta);

  // El listado debe ser una consulta ligera y predecible. No necesitamos traer
  // miles de filas para pintar la pantalla; las exportaciones siguen teniendo
  // su propia ruta. El índice compuesto emisor_id + created_at permite que
  // Supabase encuentre primero las ventas del contribuyente y las entregue ya
  // ordenadas.
  ...
  // Nunca dejar la interfaz esperando indefinidamente por una consulta de BD.
  const timeout = new Promise<never>((_, reject) =>
    setTimeout(() => reject(new Error('La consulta del listado de ventas excedió el tiempo permitido.')), 8000)
  );
  const resultado = await Promise.race([consulta, timeout]) as { data: any[] | null; error: any; };
    ...
    async (request, reply) => {
      ...
      try {
        switch (tipo) {
          ...
          case 'ventas': {
            const r = await obtenerListadoVentas(emisorId, desde, hasta, 5000);
            config = {
