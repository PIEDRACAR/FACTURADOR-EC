...
interface RegistrarEmisorBody {
  ...
  emailAdmin: string; // correo del dueño/administrador — se crea como usuario con acceso total
...
export async function registrarRutasEmisores(app: FastifyInstance) {
  ...
  app.post<{ Body: RegistrarEmisorBody }>('/emisores/registrar', async (request, reply) => {
    ...
    if (!body?.emailAdmin) faltantes.push('emailAdmin');
    if (!body?.passwordAdmin) faltantes.push('passwordAdmin');
