...
export async function registrarRutasEmisores(app: FastifyInstance) {
  // Webhook público de Payphone: solo procesa referencias de cobro creadas por CONTSERTRIB.
  app.post('/pagos/payphone/notificacion', async (request, reply) => {
    try {
      const resultado = await procesarNotificacionPayphone(request.body ?? {});
      return reply.send({ ok: true, recibido: true, resultado: { pagoId: resultado.pago?.id ?? null, duplicado: Boolean(resultado.duplicado) } });
    } catch (err) {
      request.log.error({ err }, 'Notificación Payphone no procesada');
      return reply.status(400).send({ ok: false, recibido: false, error: err instanceof Error ? err.message : String(err) });
    }
  });
