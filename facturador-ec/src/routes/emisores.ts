...
import { crearOEncontrarUsuario, crearUsuarioParaAlta, agregarUsuarioANegocio } from '../auth/sesiones.js';
...
export async function registrarRutasEmisores(app: FastifyInstance) {
  ...
  app.post<{ Body: SolicitudRegistroBody }>('/emisores/registrar', async (request, reply) => {
    ...
    try {
      ...
      const altaUsuario = await crearUsuarioParaAlta(email, password);
      const userId = altaUsuario.userId;
      ...
      else await supabase.from('suscripciones').insert({ emisor_id:emisorIdFinal, cuenta_id:cuentaId, plan_id:plan.id, estado:'activa', fecha_inicio:inicio, proximo_vencimiento:vence });
