...
export async function registrarRutasClientes(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { emisorId?: string; q?: string } }>('/clientes/buscar', async (request, reply) => {
    const emisorId = request.query.emisorId;
    const q = String(request.query.q ?? '').trim().replace(/[^0-9A-Za-zÁÉÍÓÚÜÑáéíóúüñ .-]/g, '');
    if (!emisorId) return reply.status(400).send({ error: 'Falta el parámetro emisorId.' });
    if (q.length < 3) return reply.send({ items: [], total: 0, minimo: 3 });
  ...
  app.get<{ Querystring: { tipo?: string; identificacion?: string; emisorId?: string } }>('/clientes/consultar', async (request, reply) => {
    ...
    try {
      ...
      if (mapped) return reply.send({ encontrado: true, fuente: 'SRI', datos: mapped, identificacionValida: true });
      if (tipo === '05') return reply.send({ encontrado: false, fuente: 'SRI', identificacionValida: true, mensaje: 'La cédula no tiene un RUC consultable en el catastro del SRI.' });
