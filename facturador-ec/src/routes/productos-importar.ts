...
const TARIFAS_VALIDAS = new Set(['general', '0', '5', '8', '15', 'exento', 'no_objeto']);
...
const COLUMNAS_PLANTILLA = [
  ...
  { clave: 'tarifaIva', etiqueta: 'IVA (general, 0, 5, 8, 15, exento o no_objeto)' },
...
export async function registrarRutasImportacionProductos(app: FastifyInstance) {
  ...
  app.post<{ Body: { emisorId?: string; archivoBase64?: string } }>('/productos/importar', async (request, reply) => {
    ...
    filas.forEach((fila, indice) => {
      ...
      const tarifaIva = (fila.tarifaIva || fila['IVA (general, 0, 5, 8, 15, exento o no_objeto)'] || '15').trim();
      ...
      if (!TARIFAS_VALIDAS.has(tarifaIva)) { errores.push(`Fila ${numeroFila}: tarifa de IVA "${tarifaIva}" no es válida (usa 0, 5, 8, 15, exento o no_objeto).`); return; }
