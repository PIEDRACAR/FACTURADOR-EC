...
const COOKIE_OPTS = { path: '/', httpOnly: true, sameSite: 'lax' as const, maxAge: 60 * 60 * 24 * 3650 };
...
export async function registrarRutasAuth(app: FastifyInstance) {
  ...
  app.post<{ Body: { email?: string; password?: string } }>('/auth/root-login', async (request, reply) => {
    ...
    try {
      ...
      // Segunda barrera: el usuario ROOT también debe estar autorizado como
      // administrador del proveedor, mediante registro activo o lista segura.
      const { data: admin } = await supabase
        .from('proveedores_admin')
        .select('user_id,activo')
        .eq('user_id', usuario.userId)
        .eq('activo', true)
        .maybeSingle();

      const lista = (env.proveedorAdminEmails ?? '')
        .split(',')
        .map(x => x.trim().toLowerCase())
        .filter(Boolean);

      if (!admin?.user_id && !lista.includes(correo)) {
        return reply.status(403).send({ error: 'El usuario ROOT no está autorizado como administrador del proveedor.' });
      }
