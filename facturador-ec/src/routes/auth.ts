...
const COOKIE_OPTS = { path: '/', httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax' as const, maxAge: 60 * 60 * 24 * 3650 };
...
export async function registrarRutasAuth(app: FastifyInstance) {
  ...
  app.post<{ Body: { emisorId?: string; email?: string; password?: string; rol?: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        const userId = await crearOEncontrarUsuario(email, password, { actualizarPasswordExistente: true });
