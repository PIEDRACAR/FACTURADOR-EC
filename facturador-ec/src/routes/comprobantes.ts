...
export async function registrarRutasComprobantes(app: FastifyInstance) {
  ...
  app.post<{ Body: EmitirFacturaBody }>('/comprobantes/factura/emitir', async (request, reply) => {
    ...
    try {
      ...
      let emailStatus: 'enviado' | 'error' | 'no_aplica' = 'no_aplica';
      ...
      if (resultado.estado === 'AUTORIZADO' || String(resultado.estado).toLowerCase() === 'autorizado') {
        ...
        // Regla operativa CONTSERTRIB: una factura solo se envía al cliente
        // después de que el SRI haya devuelto AUTORIZADO. Nunca se envía un
        // comprobante rechazado/devuelto como si tuviera validez tributaria.
        try {
          const { data: comprobante, error: comprobanteError } = await supabase
            .from('comprobantes')
            .select('id,secuencial,clave_acceso,numero_autorizacion,xml_firmado,clientes(razon_social,email),emisores(razon_social,nombre_comercial)')
            .eq('id', comprobanteId)
            .eq('emisor_id', emisorId)
            .single();

          if (comprobanteError || !comprobante) {
            throw new Error(`No se pudo recuperar la factura autorizada para notificación: ${comprobanteError?.message ?? 'comprobante no encontrado'}`);
          }

          const cliente = comprobante.clientes as unknown as { razon_social?: string; email?: string } | null;
          const destinatario = String((facturaData as unknown as { emailComprador?: string; correoElectronico?: string }).emailComprador ?? (facturaData as unknown as { correoElectronico?: string }).correoElectronico ?? cliente?.email ?? '').trim().toLowerCase();
          if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(destinatario)) {
            throw new Error('El cliente no tiene un correo electrónico válido para recibir el comprobante autorizado.');
          }
          if (!comprobante.xml_firmado) throw new Error('La factura fue autorizada, pero no tiene XML firmado almacenado para el envío.');

          const ride = await generarRidePdf(comprobante.id);
          const numero = comprobante.secuencial ?? resultado.secuencial;
          const mail = await enviarComprobantePorCorreo({
            to: destinatario,
            subject: `Factura electrónica ${numero} — autorizada por el SRI`,
            html: `<div style=\"font-family:Arial,sans-serif;color:#172033;max-width:680px;margin:auto\"><h2 style=\"color:#0f2747\">Factura electrónica autorizada</h2><p>Estimado/a <strong>${String(cliente?.razon_social ?? 'Cliente').replace(/[<>]/g,'')}</strong>:</p><p>Su factura electrónica <strong>${String(numero)}</strong> fue autorizada por el SRI.</p><p>Adjuntamos el <strong>RIDE en PDF</strong> y el <strong>XML firmado</strong>.</p><p style=\"font-size:12px;color:#64748b\">Clave de acceso: ${String(comprobante.clave_acceso ?? resultado.claveAcceso ?? '—')}<br>Número de autorización: ${String(comprobante.numero_autorizacion ?? resultado.numeroAutorizacion ?? '—')}</p></div>`,
            attachments: [
              { filename: `factura-${numero}.pdf`, content: ride.toString('base64'), type: 'application/pdf' },
          ...
          emailStatus = 'enviado';
          emailDetalle = `Envío automático después de autorización SRI. ID Resend: ${mail?.id ?? '—'}`;
          await supabase.from('email_envios').insert({
            comprobante_id: comprobante.id,
            destinatario,
            estado: 'enviado',
            detalle: emailDetalle,
          });
        } catch (emailError) {
          emailStatus = 'error';
          ...
          await supabase.from('email_envios').insert({
            ...
            destinatario: String((facturaData as unknown as { emailComprador?: string; correoElectronico?: string }).emailComprador ?? (facturaData as unknown as { correoElectronico?: string }).correoElectronico ?? '').trim().toLowerCase() || 'SIN_CORREO',
            estado: 'error',
            detalle: `Factura AUTORIZADA por el SRI; falló la notificación automática: ${emailDetalle}`,
          });
          request.log.error({ err: emailError, comprobanteId }, 'Factura autorizada por SRI, pero falló el envío automático al cliente.');
        }
      }
      return reply.send({
  ...
  app.get<{ Params: { id: string } }>('/comprobantes/:id/ticket', async (request, reply) => {
    ...
    const html = `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Ticket ${esc(c.secuencial)}</title><style>
      ...
      <div class="line"></div><div class="center"><b>FACTURA</b><br>No. ${esc(punto?.establecimiento)}-${esc(punto?.punto_emision)}-${esc(c.secuencial)}<br><span class="sri">Autorización: ${esc(c.numero_autorizacion || c.clave_acceso)}</span><br><span class="sri">Emisión: ${esc(new Date(c.created_at).toLocaleString('es-EC',{timeZone:'America/Guayaquil'}))}</span><br><span class="sri">Autorización: ${esc(c.fecha_autorizacion ? new Date(c.fecha_autorizacion).toLocaleString('es-EC',{timeZone:'America/Guayaquil'}) : 'Pendiente')}</span></div>
