...
export async function registrarRutasComprobantes(app: FastifyInstance) {
   ...
   *
   * La Resolución NAC-DGERCGC25-00000014 permite anular en línea hasta el
   * día 10 del mes siguiente; después de ese plazo la factura se corrige
   * mediante nota de crédito dentro de los 12 meses. El SaaS no falsifica
  ...
  app.post<{ Params: { id: string }; Body: { motivo?: string; emisorId?: string } }>('/comprobantes/:id/anulacion', async (request, reply) => {
    ...
    const finMesSiguiente = new Date(fechaEmision.getFullYear(), fechaEmision.getMonth() + 1, 10, 23, 59, 59, 999);
    const limiteDoceMeses = new Date(fechaEmision.getFullYear(), fechaEmision.getMonth() + 12, fechaEmision.getDate(), 23, 59, 59, 999);
