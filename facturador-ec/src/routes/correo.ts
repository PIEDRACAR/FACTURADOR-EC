...
import { enviarComprobantePorCorreo, diagnosticoResend } from '../services/email.js';

export async function registrarRutasCorreo(app: FastifyInstance) {
  app.get('/correo-prueba/health', async (_request, reply) => {
    const d = await diagnosticoResend();
    return reply.send({ ...d, timestamp: new Date().toISOString() });
  });
  ...
  app.post<{ Body: { emisorId?: string; to?: string } }>('/correo-prueba/enviar', async (request, reply) => {
    ...
    try {
      ...
      return reply.send({ ok: true, provider: 'resend', emailId: resultado?.id ?? null, destinatario: to, from: env.emailFrom, mensaje: 'Resend aceptó el correo. Revisa bandeja de entrada, Spam y el panel de Resend.' });
    } catch (err) {
