...
export async function registrarRutasDocumentos(app: FastifyInstance) {
  ...
  app.get<{ Querystring:{emisorId?:string} }>('/api/documentos/sustentos', async (request, reply) => {
    ...
    return reply.send((data ?? []).map((c: any) => ({
      ...
      items: (c.comprobante_items||[]).map((i:any)=>({codigo:i.codigo_principal,descripcion:i.descripcion,cantidad:Number(i.cantidad||0),precio:Number(i.precio_unitario||0),descuento:Number(i.descuento||0),ivaCodigo:Number(i.valor_iva||0)>0?'4':'0'})),
    })));
  });
