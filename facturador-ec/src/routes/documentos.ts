  ...
  app.post<{Params:{id:string}}>('/api/documentos/:id/emitir', async (request, reply) => {
    ...
    try {
      ...
      const result:any = await emitirDocumentoSri(doc.emisor_id, doc.tipo, datosNormalizados, async (preparado) => {
        const { error: persistError } = await supabase.from('documentos_sri_borrador').update({
        ...
        if (persistError) throw new Error(`No se pudo guardar la clave de acceso antes de transmitir: ${persistError.message}. El documento NO fue transmitido al SRI.`);
      }, async () => {
        transmisionIniciada = true;
        await supabase.from('documentos_sri_borrador').update({recepcion_sri:'TRANSMISION_INICIADA'}).eq('id', doc.id);
      });
      ...
      await supabase.from('documentos_sri_borrador').update({estado:'rechazado',motivo_error:mensaje,ultimo_error:mensaje,recepcion_sri:'NO TRANSMITIDO'}).eq('id',doc.id);
