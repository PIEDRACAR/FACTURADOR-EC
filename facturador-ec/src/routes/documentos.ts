...
export async function registrarRutasDocumentos(app: FastifyInstance) {
  ...
  app.post<{ Params:{id:string} }>('/api/documentos/:id/consultar-sri', async (request, reply) => {
    ...
    if (!doc.clave_acceso) return reply.status(409).send({error:'Este documento quedó procesando pero no tiene clave de acceso guardada para consultar al SRI. No se debe retransmitir automáticamente.'});
    const r = await consultarAutorizacionDocumentoSri(doc.emisor_id, doc.clave_acceso);
