...
export async function registrarRutasDocumentos(app: FastifyInstance) {
  ...
  app.post<{ Params:{id:string} }>('/api/documentos/:id/consultar-sri', async (request, reply) => {
    ...
    // Recuperación segura para documentos antiguos: si la clave no quedó en su
    // columna pero sí está dentro del XML original, la recuperamos sin volver a
    // transmitir el comprobante.
    if (!claveAcceso && doc.xml_original) {
      const m = String(doc.xml_original).match(/<claveAcceso>\s*(\d{49})\s*<\/claveAcceso>/i);
      if (m?.[1]) {
        claveAcceso = m[1];
        await supabase.from('documentos_sri_borrador').update({clave_acceso:claveAcceso}).eq('id',doc.id);
      }
    }
    if (!/^\d{49}$/.test(claveAcceso)) return reply.status(409).send({error:'Este documento quedó procesando y no se pudo recuperar una clave de acceso válida. No se debe retransmitir automáticamente; revisa el XML original o corrige el borrador y emítelo manualmente.'});
    ...
    await supabase.from('documentos_sri_borrador').update(patch).eq('id',doc.id);
