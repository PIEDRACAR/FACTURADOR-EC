...
import { contabilizarDocumentoSri } from '../services/motorContable.js';
...
export async function registrarRutasDocumentos(app: FastifyInstance) {
  ...
  app.get<{ Querystring:{emisorId?:string} }>('/api/documentos/sustentos', async (request, reply) => {
    ...
    const facturas = (data ?? []).map((c: any) => {
      const base = Number(c.subtotal_0 ?? 0) + Number(c.subtotal_5 ?? 0) + Number(c.subtotal_8 ?? 0) + Number(c.subtotal_15 ?? 0);
      const iva = Number(c.importe_total ?? 0) - base;
      return {
        id: c.id, tipoOrigen:'factura', codDoc:'01',
        numero: c.puntos_emision ? `${c.puntos_emision.establecimiento}-${c.puntos_emision.punto_emision}-${String(c.secuencial ?? '').padStart(9,'0')}` : String(c.secuencial ?? ''),
        claveAcceso: c.clave_acceso, fecha: c.created_at, total: Number(c.importe_total ?? 0), base, iva:Number(iva.toFixed(2)),
        razonSocial: c.clientes?.razon_social ?? '', identificacion: c.clientes?.identificacion ?? '', tipoIdentificacion: c.clientes?.tipo_identificacion ?? '04', direccion: c.clientes?.direccion ?? '',
    ...
    // También ofrecemos documentos electrónicos autorizados que pueden aparecer como sustento en retenciones.
    const { data: docsExtra, error: docsExtraError } = await supabase.from('documentos_sri_borrador')
      .select('id,tipo,created_at,datos,clave_acceso,secuencial')
      .eq('emisor_id', emisorId).eq('estado','autorizado')
      .in('tipo',['liquidacion_compra','nota_credito','nota_debito'])
      .order('created_at',{ascending:false}).limit(150);
    if (docsExtraError && !/relation .*does not exist|schema cache/i.test(docsExtraError.message)) return reply.status(500).send({error:docsExtraError.message});
    const extras=(docsExtra||[]).map((c:any)=>{
      const d=c.datos||{}; const base=Number(d.totalSinImpuestos||0); const total=Number(d.importeTotal??d.valorModificacion??d.valorTotal??0); const iva=Math.max(0,Number((total-base).toFixed(2)));
      return {id:c.id,tipoOrigen:c.tipo,codDoc:c.tipo==='liquidacion_compra'?'03':c.tipo==='nota_credito'?'04':'05',numero:String(d.numDocModificado||c.secuencial||''),claveAcceso:c.clave_acceso||'',fecha:c.created_at,total,base,iva,razonSocial:d.razonSocialProveedor||d.razonSocialComprador||'',identificacion:d.identificacionProveedor||d.identificacionComprador||'',tipoIdentificacion:d.tipoIdentificacionProveedor||d.tipoIdentificacionComprador||'04',direccion:d.direccionProv||'',items:Array.isArray(d.detalles)?d.detalles.map((i:any)=>({productoId:null,codigo:i.codigoPrincipal||'',descripcion:i.descripcion||'',cantidad:Number(i.cantidad||0),precio:Number(i.precioUnitario||0),descuento:Number(i.descuento||0),ivaCodigo:Number(i?.impuestos?.[0]?.valor||0)>0?'4':'0'})):[]};
    });
    return reply.send([...facturas,...extras]);
  });
  ...
  app.post<{Body:{emisorId?:string;tipo?:string;clienteId?:string;comprobanteSustentoId?:string;datos?:Record<string,unknown>}}>('/api/documentos', async (request,reply)=>{
    ...
    if (b.comprobanteSustentoId) { const { data:sustento } = await supabase.from('comprobantes').select('id,emisor_id,estado').eq('id',b.comprobanteSustentoId).eq('emisor_id',b.emisorId).maybeSingle(); if (!sustento) return reply.status(400).send({error:'El documento de sustento no pertenece a esta empresa o no existe.'}); if(sustento.estado!=='autorizado'&&sustento.estado!=='AUTORIZADO') return reply.status(400).send({error:'El documento de sustento debe estar autorizado por el SRI.'}); }
  ...
  app.post<{Params:{id:string}}>('/api/documentos/:id/emitir', async (request, reply) => {
    ...
    try {
      ...
      if ((doc.tipo === 'nota_credito' || doc.tipo === 'nota_debito') && doc.comprobante_sustento_id) {
        ...
        if (String(datosNormalizados.identificacionComprador ?? '') === '9999999999999' && doc.tipo === 'nota_credito') throw new Error('SRI: una factura emitida a CONSUMIDOR FINAL no puede ser anulada ni modificada mediante nota de crédito una vez transmitida.');
      ...
      let contabilidadEstado = estado === 'autorizado' ? 'PENDIENTE' : 'NO_APLICA';
      let contabilidadAsientoId: string | null = null;
      let contabilidadError: string | null = null;
      if (estado === 'autorizado') {
        try {
          const cr:any = await contabilizarDocumentoSri(doc.id, request.usuarioSesion?.userId);
          contabilidadEstado = String(cr.estado || 'CONTABILIZADO');
          contabilidadAsientoId = cr.id ?? null;
        } catch (accountingError) {
          contabilidadEstado = 'PENDIENTE';
          contabilidadError = accountingError instanceof Error ? accountingError.message : String(accountingError);
          request.log.error({ err: accountingError, documentoId: doc.id }, 'Documento SRI autorizado pero pendiente de contabilización automática.');
        }
        await actualizarBorradorSeguro(doc.id, {contabilidad_estado:contabilidadEstado,contabilidad_asiento_id:contabilidadAsientoId,contabilidad_error:contabilidadError});
      }
      ...
      return reply.send({ok:estado==='autorizado',estado,...result,emailEstado,emailDetalle,contabilidadEstado,contabilidadAsientoId,contabilidadError,mensaje:result.mensaje});
    } catch (e) {
  ...
  app.post<{ Params:{id:string} }>('/api/documentos/:id/consultar-sri', async (request, reply) => {
    ...
    if (estado === 'autorizado') {
      try {
        ...
        contabilidadError = accountingError instanceof Error ? accountingError.message : String(accountingError);
        request.log.error({ err: accountingError, documentoId: doc.id }, 'Documento SRI autorizado en reconsulta pero pendiente de contabilización.');
      }
      await actualizarBorradorSeguro(doc.id, {contabilidad_estado:contabilidadEstado,contabilidad_asiento_id:contabilidadAsientoId,contabilidad_error:contabilidadError});
    }
    return reply.send({ok:estado==='autorizado',estado,numeroAutorizacion:r.numeroAutorizacion,fechaAutorizacion:r.fechaAutorizacion,contabilidadEstado,contabilidadAsientoId,contabilidadError,mensaje:r.mensaje});
  });
