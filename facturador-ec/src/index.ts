...
app.setErrorHandler(async (error, request, reply) => {
  ...
  const path = request.url.split('?')[0];
  const esApi = path.startsWith('/api/') || path.startsWith('/pos/') || path.startsWith('/comprobantes/') || path.startsWith('/documentos/');
  if (esApi) return reply.status((error as any).statusCode || 500).send({ ok: false, error: 'Error interno del servidor.', detalle: error.message });
  return reply.status((error as any).statusCode || 500).send('Error interno del servidor.');
});
