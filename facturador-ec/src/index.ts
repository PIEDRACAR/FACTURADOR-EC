...
import { registrarRutasBodegas } from './routes/bodegas.js';
...
import { registrarRutasBancos } from './routes/bancos.js';
import { registrarRutasImportacionDatos } from './routes/importacionDatos.js';
...
import { registrarRutasContabilidad } from './routes/contabilidad.js';
import { registrarRutaBusquedaGlobal } from './routes/busquedaGlobal.js';
...
import { registrarProteccionCsrf } from './auth/seguridadWeb.js';
...
app.setNotFoundHandler(async (request, reply) => {
  ...
  if (esApi) return reply.status(404).send({ ok: false, error: 'Ruta de API no encontrada.', ruta: path, metodo: request.method });
...
function obtenerStatusCodeError(error: unknown): number {
...
app.setErrorHandler(async (error, request, reply) => {
  ...
  if (esApi) {
    const payload: Record<string, unknown> = { ok: false, error: 'Error interno del servidor.' };
    if (process.env.NODE_ENV !== 'production') payload.detalle = detalle;
...
// La protección global se registra ANTES de las páginas y APIs protegidas.
// Las excepciones públicas (incluido /root-login y /auth/root-login) están
// declaradas explícitamente en src/auth/proteccion.ts.
registrarProteccionSesion(app);
...
app.get('/app.css', async (_request, reply) => {
  ...
  return reply.header('Cache-Control', 'no-store, max-age=0, must-revalidate').type('text/css; charset=utf-8').send(css);
});
app.get('/app-shell.js', async (_request, reply) => {
  const js = readFileSync(join(__dirname, '..', 'public', 'app-shell.js'), 'utf-8');
  return reply.header('Cache-Control', 'no-store, max-age=0, must-revalidate').type('application/javascript; charset=utf-8').send(js);
});
app.get('/security.js', async (_request, reply) => {
  const js = readFileSync(join(__dirname, '..', 'public', 'security.js'), 'utf-8');
  return reply.header('Cache-Control', 'no-store, max-age=0, must-revalidate').type('application/javascript; charset=utf-8').send(js);
});
...
function servirPagina(nombreArchivo: string, protegida = true) {
  return async (_request: unknown, reply: { type: (t: string) => { send: (s: string) => void } }) => {
    ...
    if (html.includes('</head>') && !html.includes('/security.js')) html = html.replace('</head>', '<script src="/security.js"></script></head>');
...
app.get('/planes', servirPagina('planes.html', false));
app.get('/registro', servirPagina('registro.html', false));
app.get('/restablecer-password', servirPagina('restablecer-password.html', false));
...
// El acceso ROOT es una pantalla totalmente independiente: no debe cargar el shell,
// la selección de negocio, emisorId ni la protección visual de páginas operativas.
app.get('/root-login', servirPagina('root-login.html', false));
app.get('/ats', servirPagina('ats.html'));
app.get('/contabilidad', servirPagina('contabilidad.html'));
...
await registrarRutasBodegas(app);
...
await registrarRutasBancos(app);
await registrarRutasImportacionDatos(app);
...
await registrarRutasContabilidad(app);
await registrarRutaBusquedaGlobal(app);
