  ...
  app.addHook('preHandler', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!emisorId) {
      ...
      if (negocios.length === 1) {
        ...
        emisorId = negocios[0].emisorId;
        reply.setCookie(NEGOCIO_COOKIE, emisorId, { path: '/', httpOnly: true, sameSite: 'lax', maxAge: 60 * 60 * 24 * 30 });
        if (metodo === 'GET' && PAGINAS_HTML.has(patronRuta)) {
          return reply.redirect(`${patronRuta}?emisorId=${encodeURIComponent(emisorId)}`);
        }
        asignarEmisorEnPeticion(request, emisorId);
    ...
    if (emisorId) reply.setCookie(NEGOCIO_COOKIE, emisorId, { path: '/', httpOnly: true, sameSite: 'lax', maxAge: 60 * 60 * 24 * 30 });
