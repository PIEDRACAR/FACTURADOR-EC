...
export function registrarProteccionSesion(app: FastifyInstance) {
  app.addHook('preHandler', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    // El Panel Maestro solo se puede abrir con la identidad ROOT autorizada.
    if (patronRuta === '/admin-proveedor') {
      const { data: rootAdmin } = await supabase
        ...
        .eq('activo', true)
        .maybeSingle();
      const { data: authUser } = await supabase.auth.admin.getUserById(sesion.userId);
      const email = authUser.user?.email?.trim().toLowerCase() ?? '';
      const permitidos = (env.proveedorAdminEmails ?? '').split(',').map(x => x.trim().toLowerCase()).filter(Boolean);
      if (email !== env.rootAdminEmail || (!rootAdmin?.user_id && !permitidos.includes(email))) {
        if (metodo === 'GET') return reply.redirect('/root-login');
