...
const RUTAS_PUBLICAS = new Set([
  'GET /', 'GET /login', 'GET /root-login', 'GET /planes', 'GET /registro', 'GET /salud',
  'POST /auth/login', 'POST /auth/root-login', 'GET /auth/yo', 'GET /emisores/buscar', 'POST /emisores/registrar', 'POST /pagos/payphone/notificacion',
  ...
  app.addHook('preHandler', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!emisorId) {
      ...
      if (negocios.length === 1) {
        ...
        emisorId = negocios[0].emisorId;
        reply.setCookie(NEGOCIO_COOKIE, emisorId, { path: '/', httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 60 * 60 * 24 * 3650 });
        if (metodo === 'GET' && PAGINAS_HTML.has(patronRuta)) {
          return reply.redirect(`${patronRuta}?emisorId=${encodeURIComponent(emisorId)}`);
        }
        asignarEmisorEnPeticion(request, emisorId);
    ...
    if (emisorId) reply.setCookie(NEGOCIO_COOKIE, emisorId, { path: '/', httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 60 * 60 * 24 * 3650 });
