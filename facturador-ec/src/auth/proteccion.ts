...
const PAGINAS_HTML = new Set([
  ...
  '/correo-prueba', '/admin-proveedor', '/suscripcion', '/ats', '/impuestos', '/impuestos-',
]);
...
export function registrarProteccionSesion(app: FastifyInstance) {
  app.addHook('preHandler', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!emisorId) {
      ...
      // La pantalla de impuestos tiene selector propio cuando el usuario administra
      // varios RUC. No bloqueamos la carga de la página con un 409.
      if (metodo === 'GET' && (patronRuta === '/impuestos' || patronRuta === '/impuestos-')) return;
