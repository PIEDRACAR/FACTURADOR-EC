...
export const CATALOGO_PERMISOS = [
  ...
  { clave: 'contabilidad', nombre: 'Contabilidad', descripcion: 'Plan de cuentas, asientos, estados financieros y nómina.' },
] as const;

export type Permiso = typeof CATALOGO_PERMISOS[number]['clave'];
...
export const PERMISOS_POR_ROL: Record<string, Permiso[]> = {
  ...
  contador: ['facturar','clientes','productos','inventario','proveedores','caja','cxc','cxp','proformas','reportes','correo','notificaciones','documentos','configuracion','contabilidad'],
  cajero: ['facturar','clientes','notificaciones','correo'],
};

export async function obtenerPermisosUsuario(userId: string, emisorId: string, rol: string): Promise<Set<Permiso>> {
  if (rol === 'admin') return new Set(PERMISOS_POR_ROL.admin);
  const { data, error } = await supabase
    ...
    .eq('activo', true);
  if (!error && (data?.length ?? 0) > 0) return new Set((data ?? []).map(x => x.permiso as Permiso));
  return new Set(PERMISOS_POR_ROL[rol] ?? []);
}

export async function guardarPermisosUsuario(userId: string, emisorId: string, permisos: string[]): Promise<void> {
  const unicos: Permiso[] = [...new Set(permisos.filter(esPermiso))];
...
export function permisoParaRuta(method: string, patron: string): Permiso | null {
  if (patron.startsWith('/pos') || patron.startsWith('/comprobantes')) return 'facturar';
  if (patron.startsWith('/clientes')) return 'clientes';
  if (patron.startsWith('/productos')) return 'productos';
  if (patron.startsWith('/inventario')) return 'inventario';
  if (patron.startsWith('/proveedores')) return 'proveedores';
  if (patron.startsWith('/caja')) return 'caja';
  if (patron.startsWith('/cuentas-por-cobrar')) return 'cxc';
  if (patron.startsWith('/cuentas-por-pagar')) return 'cxp';
  if (patron.startsWith('/proformas')) return 'proformas';
  if (patron.startsWith('/reportes') || patron.startsWith('/dashboard')) return 'reportes';
  if (patron.startsWith('/correo-prueba')) return 'correo';
  if (patron.startsWith('/notificaciones')) return 'notificaciones';
  if (patron.startsWith('/documentos') || patron.startsWith('/api/documentos')) return 'documentos';
  if (patron.startsWith('/configuracion')) return 'configuracion';
  if (patron.startsWith('/auth/usuarios') || patron.startsWith('/usuarios-admin')) return 'usuarios';
  if (patron.startsWith('/contabilidad')) return 'contabilidad';
  if (patron.startsWith('/ats')) return 'documentos';
  return null;
}
