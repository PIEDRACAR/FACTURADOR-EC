...
export async function verificarCredenciales(email: string, password: string): Promise<UsuarioAutenticado> {
  const respuesta = await fetch(`${env.supabaseUrl}/auth/v1/token?grant_type=password`, {
    ...
    headers: {
      ...
      apikey: String(env.supabaseServiceRoleKey),
      Authorization: `Bearer ${String(env.supabaseServiceRoleKey)}`,
    },
...
export async function obtenerUsuarioAuthPorEmail(email: string): Promise<{ id: string; email: string } | null> {
  const respuesta = await fetch(`${env.supabaseUrl}/auth/v1/admin/users?email=${encodeURIComponent(email.trim().toLowerCase())}`, {
    headers: { apikey: env.supabaseServiceRoleKey, Authorization: `Bearer ${env.supabaseServiceRoleKey}` },
  });
  if (!respuesta.ok) return null;
  const datos = (await respuesta.json()) as { users?: Array<{ id: string; email: string }> };
  return datos.users?.find((u) => u.email?.toLowerCase() === email.trim().toLowerCase()) ?? null;
}

export async function crearOEncontrarUsuario(email: string, password: string, opciones: { actualizarPasswordExistente?: boolean; verificarPasswordExistente?: boolean } = {}): Promise<string> {
  ...
  if (busqueda.ok) {
    ...
    if (existente) {
      // Seguridad: una alta pública puede reutilizar una identidad existente
      // solo si demuestra conocimiento de su contraseña. Nunca se resetea la
      // contraseña desde un flujo público.
      if (opciones.verificarPasswordExistente) {
        const ok = await fetch(`${env.supabaseUrl}/auth/v1/token?grant_type=password`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', apikey: env.supabaseServiceRoleKey, Authorization: `Bearer ${env.supabaseServiceRoleKey}` },
          body: JSON.stringify({ email: email.trim().toLowerCase(), password }),
        });
        if (!ok.ok) throw new Error('No se pudo validar la identidad del correo registrado.');
      }
      if (opciones.actualizarPasswordExistente) {
        ...
        if (!actualizado.ok) {
          const detalleActualizacion = await actualizado.text();
          throw new Error(`El usuario ya existe pero no se pudo actualizar su contraseña: ${detalleActualizacion}`);
        }
      }
      return existente.id;
    }
