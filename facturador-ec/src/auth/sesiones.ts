...
// Política absoluta: tanto el registro backend como la cookie expiran a los siete días.
...
export async function crearOEncontrarUsuario(email: string, password: string, opciones: { verificarPasswordExistente?: boolean } = {}): Promise<string> {
  ...
  if (busqueda.ok) {
    ...
    if (existente) {
      ...
      // La identidad Auth es global: asociarla a otro emisor nunca cambia credenciales.
      return existente.id;
    }
  }
