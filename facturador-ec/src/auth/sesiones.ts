...
export async function crearOEncontrarUsuario(email: string, password: string): Promise<string> {
  ...
  if (busqueda.ok) {
    ...
    if (existente) {
      // Si el correo ya existe, sincronizamos la contraseña entregada en el
      // registro con Supabase Auth. Así las credenciales mostradas al cliente
      // son exactamente las que funcionan en el siguiente inicio de sesión.
