...
export const env = {
  ...
  // Clave pública de Supabase para el flujo de inicio de sesión.
  // Si no existe, se usa la service role como compatibilidad temporal.
  supabaseAnonKey: process.env.SUPABASE_ANON_KEY ?? process.env.SUPABASE_PUBLISHABLE_KEY ?? process.env.SUPABASE_SERVICE_ROLE_KEY,
  ...
  sriAmbiente: (process.env.SRI_AMBIENTE ?? '1') as '1' | '2',
  ...
  payphoneLinkExpireHours: Math.max(1, Math.min(168, Number(process.env.PAYPHONE_LINK_EXPIRE_HOURS ?? 24))),
  // Acceso exclusivo al Panel Maestro. La contraseña NO se guarda aquí:
  // la gestiona Supabase Auth para este usuario.
  rootAdminEmail: (process.env.ROOT_ADMIN_EMAIL ?? 'ecfacturador@gmail.com').trim().toLowerCase(),
};
