...
async function ensureBucket(): Promise<void> {
  if (!bucketReady) {
    bucketReady = (async () => {
      ...
      if (listError) throw new Error(`No se pudo consultar Supabase Storage: ${listError.message}`);
      if (buckets?.some((b) => b.name === COMPROBANTES_BUCKET)) return;
...
export async function archivarComprobanteAutorizado(input: {
  ...
  if (!input.xmlFirmado) throw new Error('No existe XML firmado para archivar.');
  ...
  if (xmlUpload.error) throw new Error(`No se pudo guardar el XML en Storage: ${xmlUpload.error.message}`);
  if (pdfUpload.error) throw new Error(`No se pudo guardar el RIDE PDF en Storage: ${pdfUpload.error.message}`);
...
export async function obtenerArchivosComprobante(comprobanteId: string) {
  ...
  if (error) throw new Error(`No se pudo consultar el archivo documental: ${error.message}`);
