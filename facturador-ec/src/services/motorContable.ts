...
export const CUENTAS_BASE = {
  ...
  PROPINAS_POR_PAGAR: '2.1.05.01', MEDIOS_ELECTRONICOS: '1.1.01.03'
} as const;
...
export async function contabilizarVenta(comprobanteId:string,userId?:string){
  ...
  if(!pagosQ.error){
    for(const p of pagosQ.data||[]){
      ...
      lineas.push({codigo:cuentaParaFormaPago(codigo),descripcion:`Cobro de ${esTicket ? 'ticket' : 'factura'} (${codigo||'forma de pago'})`,debe:v,haber:0});
    }
  }
  ...
  if(porCobrar>0){
    ...
    if(existingCxc.error) throw new Error(`No se pudo consultar la cuenta por cobrar de la factura: ${existingCxc.error.message}`);
    ...
    lineas.push({codigo:CUENTAS_BASE.CLIENTES,descripcion:'Saldo pendiente de factura',debe:porCobrar,haber:0});
  }
  ...
  if(propina>0) lineas.push({codigo:CUENTAS_BASE.PROPINAS_POR_PAGAR,descripcion:'Propina cobrada pendiente de entrega',debe:0,haber:propina});
  ...
  if(costo>0){
    lineas.push({codigo:CUENTAS_BASE.COSTO_VENTAS,descripcion:'Costo de ventas automático',debe:costo,haber:0});
    lineas.push({codigo:CUENTAS_BASE.INVENTARIO,descripcion:'Salida de inventario por venta',debe:0,haber:costo});
  }
...
export async function contabilizarMovimientoInventario(movimientoId:string,userId?:string){
  ...
  if(tipo==='entrada'){
    const lineas:any[]=[{codigo:CUENTAS_BASE.INVENTARIO,descripcion:'Entrada de inventario',debe:valor,haber:0}];
    if(m.proveedor_id){
      ...
      lineas.push({codigo:CUENTAS_BASE.PROVEEDORES,descripcion:'Obligación por compra de inventario',debe:0,haber:valor,terceroTipo:'PROVEEDOR',terceroId:m.proveedor_id});
    }else{
      lineas.push({codigo:CUENTAS_BASE.CAJA,descripcion:'Compra de inventario pagada al contado',debe:0,haber:valor});
    }
  ...
  if(tipo==='salida'){
    const lineas=[{codigo:CUENTAS_BASE.COSTO_VENTAS,descripcion:'Costo de salida de inventario',debe:valor,haber:0},{codigo:CUENTAS_BASE.INVENTARIO,descripcion:'Salida de inventario',debe:0,haber:valor}];
  ...
  if(tipo==='ajuste'){
    ...
    if(delta>0){const lineas=[{codigo:CUENTAS_BASE.INVENTARIO,descripcion:'Ajuste positivo de inventario',debe:valor,haber:0},{codigo:CUENTAS_BASE.OTROS_INGRESOS,descripcion:'Contrapartida ajuste positivo de inventario',debe:0,haber:valor}];const id=await asientoAtomico({emisorId:m.emisor_id,fecha,tipo:'AJUSTE_INVENTARIO',concepto:`Ajuste positivo de inventario ${m.id}`,referencia:m.referencia_id||m.id,origenTipo:'MOVIMIENTO_INVENTARIO',origenId:m.id,userId,lineas});return {id,estado:'CONTABILIZADO'};}
    if(delta<0){const lineas=[{codigo:CUENTAS_BASE.GASTOS_ADMIN,descripcion:'Pérdida por ajuste negativo de inventario',debe:valor,haber:0},{codigo:CUENTAS_BASE.INVENTARIO,descripcion:'Disminución por ajuste de inventario',debe:0,haber:valor}];const id=await asientoAtomico({emisorId:m.emisor_id,fecha,tipo:'AJUSTE_INVENTARIO',concepto:`Ajuste negativo de inventario ${m.id}`,referencia:m.referencia_id||m.id,origenTipo:'MOVIMIENTO_INVENTARIO',origenId:m.id,userId,lineas});return {id,estado:'CONTABILIZADO'};}
  }
...
export async function contabilizarPagoCxP(cuentaId:string,pagoId:string,userId?:string){
  ...
  const codigo=String(p.forma_pago_codigo||'20'); const cuentaBanco=cuentaParaFormaPago(codigo);
...
export async function contabilizarCxC(cuentaId:string,userId?:string){
  ...
  const id=await asientoAtomico({emisorId:c.emisor_id,fecha:c.fecha_emision,tipo:'CUENTA_POR_COBRAR',concepto:c.concepto,origenTipo:'CUENTA_POR_COBRAR',origenId:c.id,userId,lineas:[{codigo:CUENTAS_BASE.CLIENTES,descripcion:c.concepto,debe:Number(c.monto_total),haber:0,terceroTipo:'CLIENTE',terceroId:c.cliente_id},{codigo:CUENTAS_BASE.VENTAS,descripcion:'Ingreso pendiente de cobro',debe:0,haber:Number(c.monto_total)}]});
  return {id,estado:'CONTABILIZADO'};
}
export async function contabilizarCobroCxC(cuentaId:string,pagoId:string,userId?:string){
  ...
  const codigo=String(p.forma_pago_codigo||'01'); const cuentaBanco=cuentaParaFormaPago(codigo);
...
export async function contabilizarDocumentoSri(documentoId:string,userId?:string){
  ...
  if(doc.tipo==='nota_credito'){
    lineas=[...(baseSegura?[{codigo:CUENTAS_BASE.VENTAS,descripcion:'Reversión de venta por nota de crédito',debe:baseSegura,haber:0}]:[]),...(ivaSegura?[{codigo:CUENTAS_BASE.IVA_VENTAS,descripcion:'Reversión del IVA por nota de crédito',debe:ivaSegura,haber:0}]:[]),{codigo:CUENTAS_BASE.CLIENTES,descripcion:'Disminución de cuenta por cobrar por nota de crédito',debe:0,haber:total,terceroTipo:'CLIENTE',terceroId:doc.cliente_id||undefined}];
  ...
  if(doc.tipo==='nota_debito'){
    lineas=[{codigo:CUENTAS_BASE.CLIENTES,descripcion:'Incremento de cuenta por cobrar por nota de débito',debe:total,haber:0,terceroTipo:'CLIENTE',terceroId:doc.cliente_id||undefined},...(baseSegura?[{codigo:CUENTAS_BASE.VENTAS,descripcion:'Ingreso adicional por nota de débito',debe:0,haber:baseSegura}]:[]),...(ivaSegura?[{codigo:CUENTAS_BASE.IVA_VENTAS,descripcion:'IVA adicional por nota de débito',debe:0,haber:ivaSegura}]:[])];
  ...
  if(doc.tipo==='liquidacion_compra'){
    ...
    lineas=[...(baseSegura?[{codigo:CUENTAS_BASE.GASTOS_ADMIN,descripcion:'Adquisición registrada en liquidación de compra',debe:baseSegura,haber:0}]:[]),...(ivaSegura?[{codigo:CUENTAS_BASE.IVA_COMPRAS,descripcion:'IVA crédito tributario de liquidación de compra',debe:ivaSegura,haber:0}]:[]),{codigo:CUENTAS_BASE.PROVEEDORES,descripcion:'Obligación derivada de liquidación de compra',debe:0,haber:total,terceroTipo:'PROVEEDOR',terceroId:proveedorId||undefined}];
  ...
  if(doc.tipo==='retencion'){
    ...
    const retDetalles=Array.isArray(d.impuestos)?d.impuestos:Array.isArray(d.retenciones)?d.retenciones:[]; const totalRet=money(retDetalles.reduce((s:number,x:any)=>s+Number(x.valorRetenido??x.valor??0),0)||Number(d.valorRetenidoTotal||0));
    ...
    lineas=[{codigo:CUENTAS_BASE.PROVEEDORES,descripcion:'Aplicación de retención a proveedor',debe:totalRet,haber:0,terceroTipo:'PROVEEDOR',terceroId:cxp.proveedor_id},{codigo:CUENTAS_BASE.IMPUESTOS,descripcion:'Retenciones e impuestos por pagar al SRI',debe:0,haber:totalRet}];
