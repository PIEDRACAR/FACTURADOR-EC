...
export async function contabilizarVenta(comprobanteId:string,userId?:string){
  ...
  if(ce||!c) throw new Error('Comprobante no encontrado.'); if(c.estado!=='autorizado') throw new Error('Solo se contabilizan comprobantes autorizados.');
  ...
  const id=await asientoAtomico({emisorId:c.emisor_id,fecha:String(c.created_at).slice(0,10),tipo:'VENTA',concepto:`Factura ${c.secuencial||c.clave_acceso||c.id}`,referencia:c.clave_acceso,origenTipo:'COMPROBANTE_VENTA',origenId:c.id,userId,lineas});
...
export async function contabilizarCobroCxC(cuentaId:string,pagoId:string,userId?:string){
  ...
  const id=await asientoAtomico({emisorId:c.emisor_id,fecha:String(p.created_at).slice(0,10),tipo:'COBRO_CLIENTE',concepto:`Cobro: ${c.concepto}`,origenTipo:'PAGO_CXC',origenId:p.id,userId,lineas:[{codigo:cuentaBanco,descripcion:'Ingreso de cobro',debe:Number(p.monto),haber:0},{codigo:CUENTAS_BASE.CLIENTES,descripcion:'Aplicación de cobro',debe:0,haber:Number(p.monto),terceroTipo:'CLIENTE',terceroId:c.cliente_id}]});return {id};
}

export async function obtenerBalanza(emisorId:string,desde?:string,hasta?:string){await asegurarPlanBase(emisorId);let q=supabase.from('asiento_lineas_contables').select('debe,haber,plan_cuentas_contables!inner(id,codigo,nombre,tipo,naturaleza),asientos_contables!inner(emisor_id,fecha,estado)').eq('asientos_contables.emisor_id',emisorId).eq('asientos_contables.estado','CONTABILIZADO');if(desde)q=q.gte('asientos_contables.fecha',desde);if(hasta)q=q.lte('asientos_contables.fecha',hasta);const {data,error}=await q;if(error)throw new Error(error.message);const map=new Map<string,any>();for(const r of data||[]){const c:any=r.plan_cuentas_contables;const x=map.get(c.id)||{id:c.id,codigo:c.codigo,nombre:c.nombre,tipo:c.tipo,naturaleza:c.naturaleza,debe:0,haber:0};x.debe+=Number(r.debe||0);x.haber+=Number(r.haber||0);map.set(c.id,x);}return Array.from(map.values()).map(x=>({...x,debe:money(x.debe),haber:money(x.haber),saldo:money(x.debe-x.haber)})).sort((a,b)=>a.codigo.localeCompare(b.codigo));}
