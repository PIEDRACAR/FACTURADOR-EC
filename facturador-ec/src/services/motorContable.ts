...
export async function contabilizarVenta(comprobanteId:string,userId?:string){
  const {data:c,error:ce}=await supabase.from('comprobantes').select('id,emisor_id,estado,created_at,importe_total,subtotal_0,subtotal_5,subtotal_8,subtotal_15,total_iva,propina,secuencial,clave_acceso').eq('id',comprobanteId).single();
  if(ce||!c) throw new Error(`Comprobante no encontrado: ${ce?.message||''}`.trim());
  if(String(c.estado||'').toLowerCase()!=='autorizado') throw new Error(`Solo se contabilizan comprobantes autorizados. Estado actual: ${c.estado}`);
  ...
  if(!pagosQ.error){
    for(const p of pagosQ.data||[]){
      ...
      lineas.push({codigo:cuentaParaFormaPago(codigo),descripcion:`Cobro de factura (${codigo||'forma de pago'})`,debe:v,haber:0});
    }
  }
  ...
  if(porCobrar>0) lineas.push({codigo:CUENTAS_BASE.CLIENTES,descripcion:'Saldo pendiente de factura',debe:porCobrar,haber:0});
  ...
  const id=await asientoAtomico({emisorId:c.emisor_id,fecha,tipo:'VENTA',concepto:`Factura ${c.secuencial||c.clave_acceso||c.id}`,referencia:c.clave_acceso,origenTipo:'COMPROBANTE_VENTA',origenId:c.id,userId,lineas});
