...
export async function obtenerEmailsAdminSaas(): Promise<string[]> {
  ...
  const configurados = (env.proveedorAdminEmails ?? '').split(',').map(x => x.trim().toLowerCase()).filter(Boolean);
  return configurados.length ? configurados : (env.rootAdminEmail ? [env.rootAdminEmail] : []);
}
...
export async function notificarNuevaSolicitudSaas(solicitud: {
  ...
  paymentLink?: string | null; paymentAmount?: number | null; paymentError?: string | null;
}) {
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:680px;margin:auto;color:#172033">`+
    ...
    `</table>`+
    (solicitud.paymentLink ? `<div style="margin:20px 0;padding:16px;background:#ecfdf5;border:1px solid #86efac;border-radius:10px"><b>💳 Pago PayPhone generado</b><br><span>Total: $${Number(solicitud.paymentAmount||0).toFixed(2)} USD</span><br><a href="${esc(solicitud.paymentLink)}" style="display:inline-block;margin-top:10px;background:#0f2747;color:#fff;text-decoration:none;padding:12px 18px;border-radius:9px;font-weight:bold">ABRIR LINK DE PAGO</a></div>` : (solicitud.paymentError ? `<div style="margin:20px 0;padding:16px;background:#fff7ed;border:1px solid #fdba74;border-radius:10px"><b>⚠ PayPhone pendiente</b><br>${esc(solicitud.paymentError)}</div>` : ''))+
    `<p style="margin-top:22px"><a href="${esc(panelUrl)}" style="display:inline-block;background:#0f2747;color:#fff;text-decoration:none;padding:12px 18px;border-radius:9px;font-weight:bold">Ver solicitud en Panel ROOT</a></p>`+
    `<p style="font-size:12px;color:#64748b">La activación definitiva se realiza después del pago aprobado. Si existe una promoción vigente, el alta puede ser inmediata.</p></div></div>`;
