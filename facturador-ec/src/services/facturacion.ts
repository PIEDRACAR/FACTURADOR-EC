...
type ResultadoFacturacion = {
  // Tipos propios del dominio: la librería 1.0.1 tiene uniones demasiado
  // restrictivas (p. ej. DEVUELTO/Ambiente) que no representan literalmente
  // las respuestas del SRI (DEVUELTA/pruebas/produccion).
  estado: string;
...
type RespuestaSriConMensajes = {
  ...
  comprobantes?: {
    comprobante?:
      | { mensajes?: MensajeSri[] }
      | Array<{ mensajes?: MensajeSri[] }>;
  } | null;
};

function extraerMensajesSri(respuesta: unknown): MensajeSri[] {
  const r = (respuesta ?? {}) as RespuestaSriConMensajes;
...
function normalizarEstadoSri(estado: unknown): string {
  const valor = String(estado ?? '').trim().toUpperCase();
  return valor === 'DEVUELTO' ? 'DEVUELTA' : valor;
}

function formatearMensajesSri(respuesta: unknown): string {
...
async function construirFacturadorParaEmisor(
  ...
  return {
    fe: new FacturacionElectronicaEC({
      ...
      p12Password,
      sequenceProvider,
      validateXsd: false, // La validación se ejecuta explícitamente después de insertar RUC Proveedor; evita el error opaco [object Object] del SDK.
    }),
    ruc: emisor.ruc,
...
function insertarInfoAdicionalRucProveedor(xmlSinFirmar: string, rucProveedor: string): string {
  // El XSD permite UNA sola sección infoAdicional. La versión anterior
  // insertaba siempre una nueva sección y, si la librería ya había generado
  // infoAdicional por datos del comprobante, el XML quedaba inválido.
  const campo =
    `<campoAdicional nombre="${NOMBRE_CAMPO_RUC_PROVEEDOR}">` +
    `${escapeXmlBasico(rucProveedor)}</campoAdicional>`;
  const reInfo = /<infoAdicional>([\s\S]*?)<\/infoAdicional>/i;
  const existente = xmlSinFirmar.match(reInfo);

  if (existente) {
    const contenido = existente[1] ?? '';
    const reCampoProveedor = new RegExp(
      `<campoAdicional\\s+[^>]*nombre=["']${NOMBRE_CAMPO_RUC_PROVEEDOR.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}["'][^>]*>[\\s\\S]*?<\/campoAdicional>`,
      'i',
    );
    const nuevoContenido = reCampoProveedor.test(contenido)
      ? contenido.replace(reCampoProveedor, campo)
      : contenido + campo;
    return xmlSinFirmar.replace(reInfo, `<infoAdicional>${nuevoContenido}</infoAdicional>`);
  }
  ...
  return xmlSinFirmar.slice(0, posicion) +
    `<infoAdicional>${campo}</infoAdicional>` +
    xmlSinFirmar.slice(posicion);
}

function formatearErroresXsd(errores: unknown[]): string[] {
  return (errores ?? []).map((e: unknown) => {
    ...
    if (e && typeof e === 'object') {
      ...
      const partes = [
        ...
        linea != null ? `línea ${String(linea)}` : '',
        columna != null ? `columna ${String(columna)}` : '',
      ].filter(Boolean);
...
async function emitirFacturaConCampoAdicional({
  ...
  sequenceProvider,
  rucProveedor,
}: ParametrosCampoAdicional): Promise<ResultadoFacturacion> {
  ...
  for (let intento = 0; intento <= MAX_ERROR_70_RETRIES; intento++) {
    ...
    if (!validacion.valid) {
      const erroresXsd = formatearErroresXsd(validacion.errors as unknown[]);
      throw new Error(
        `El XML no pasó la validación XSD local: ${erroresXsd.join('; ') || 'Error de estructura XML sin detalle de la librería.'}`
      );
    }
    ...
    const recepcionSri = recepcion as unknown as RespuestaSriConMensajes & { estado?: string };
...
function construirResultadoDesdeAutorizacion(
  autorizacion: RespuestaSriConMensajes & {
    estado: string;
    numeroAutorizacion: string | null;
...
export async function emitirFactura({
  ...
  comprobanteId,
  facturaData,
}: EmitirFacturaInput): Promise<ResultadoFacturacion> {
  ...
  let resultado: ResultadoFacturacion;
  try {
    ...
    if (mensaje.includes('[object Object]')) {
      // El SDK 1.0.1 puede convertir los errores XSD internos a [object Object]
      // cuando validateXsd está activo dentro de buildXml. Desde v9.9.76 la
      // validación XSD se ejecuta explícitamente después de insertar el RUC del
      // proveedor, por lo que este bloque solo queda como red de seguridad para
      // instalaciones que aún tengan una combinación antigua de dependencias.
      mensaje = mensaje.replace(/(?:\s*[;,:-]?\s*)?\[object Object\]/g, '').trim();
      mensaje +=
        ' — El SDK devolvió un error XSD sin detalle. La validación detallada se ejecuta ahora antes de firmar; ' +
        'revise código principal (máx. 25), descripción (máx. 300), cantidades, precios, fechas y estructura de impuestos.';
    }
