...
function construirResultadoDesdeAutorizacion(
  autorizacion: { estado: string; numeroAutorizacion: string | null; fechaAutorizacion: string | null; mensaje?: string; mensajes?: Array<{ identificador?: string; mensaje?: string; informacionAdicional?: string }> },
  claveAcceso: string,
  secuencial: string,
  xmlOriginal: string,
  xmlFirmado: string,
  ...
  return {
    ...
    mensaje: autorizacion.mensaje ?? autorizacion.mensajes?.map((m) => `${m.identificador ?? ''}: ${m.mensaje ?? ''}${m.informacionAdicional ? ' — ' + m.informacionAdicional : ''}`).filter(Boolean).join('; ') ?? undefined,
  } as unknown as EmissionResult;
}
