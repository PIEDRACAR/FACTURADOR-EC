...
type ResultadoFacturacion = EmissionResult & {
  mensaje?: string;
  mensajes?: MensajeSri[];
};
...
async function emitirFacturaConCampoAdicional({
  ...
  for (let intento = 0; intento <= MAX_ERROR_70_RETRIES; intento++) {
    ...
    const mensajesRecepcion = extraerMensajesSri(recepcion);
    const tieneCodigo70 = recepcion.estado === 'DEVUELTA' && mensajesRecepcion.some((m) => m.identificador === '70');
