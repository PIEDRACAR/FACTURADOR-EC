...
// Por defecto queda desactivada para evitar bloqueos de XSD empaquetados/errores opacos.
...
async function emitirFacturaConCampoAdicional({
  ...
  for (let intento = 0; intento <= MAX_ERROR_70_RETRIES; intento++) {
    ...
    // XSD local opcional; el SRI es la autoridad definitiva.
    if (VALIDAR_XSD_LOCAL) {
      ...
      if (!validacion.valid) {
        ...
        throw new Error(`El XML no pasó la validación XSD local: ${erroresXsd.join('; ') || 'Error de estructura XML sin detalle.'}`);
      }
    }
...
export async function emitirFactura({
  ...
  try {
    ...
    if (mensaje.includes('[object Object]')) {
      ...
      if (errAny?.code) detalles.push(`código=${String(errAny.code)}`);
      if (Array.isArray(errAny?.errors)) detalles.push(...formatearErroresXsd(errAny.errors));
      if (Array.isArray(errAny?.sriMensajes)) detalles.push(formatearMensajesSri({ mensajes: errAny.sriMensajes }));
      mensaje = detalles.filter(Boolean).join('; ') || 'La librería devolvió un objeto de error sin serializar. Revise el detalle técnico del servidor.';
    }
