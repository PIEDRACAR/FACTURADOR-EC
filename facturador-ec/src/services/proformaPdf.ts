...
const TIPO_IDENTIFICACION: Record<string,string> = { '04':'RUC', '05':'Cédula', '06':'Pasaporte', '07':'Consumidor final', '08':'Identificación del exterior' };
...
export async function generarProformaPdf(id:string):Promise<Buffer>{
 ...
 y=section(doc,'Datos del cliente',y);doc.fillColor(C.ink).font('Helvetica-Bold').fontSize(8.2).text(txt(c.razon_social)||'Sin cliente',page.l,y,{width:265});const tipoId = TIPO_IDENTIFICACION[String(c.tipo_identificacion ?? '').trim()] || 'Identificación';
doc.font('Helvetica').text(`${tipoId}: ${txt(c.identificacion)}`,page.l+275,y,{width:115});doc.text(txt(c.direccion),page.l,y+12,{width:360});doc.text(txt(c.email),page.l+370,y+12,{width:158});y+=31;
