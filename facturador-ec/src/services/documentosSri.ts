...
import { validarRucProveedor, validarXmlSRI } from './sriPreflight.js';
import { buscarRetencionSRI } from './retencionesSri.js';
...
// La validación XSD local es opcional; la validación definitiva la realiza el SRI.
const VALIDAR_XSD_LOCAL = /^true$/i.test(process.env.VALIDAR_XSD_LOCAL?.trim() ?? '');
...
function addProviderRuc(xml: string, root: string, ruc: string): string {
  ...
  const re = /<infoAdicional>([\s\S]*?)<\/infoAdicional>/i;
  if (re.test(xml)) {
    return xml.replace(re, (whole, contenido: string) => {
      const providerRe = /<campoAdicional\s+[^>]*nombre=["']RUC\s+Proveedor["'][^>]*>[\s\S]*?<\/campoAdicional>/i;
      if (providerRe.test(contenido)) return whole.replace(providerRe, field);
      const cantidad = (contenido.match(/<campoAdicional\s+[^>]*>/gi) ?? []).length;
      if (cantidad >= 15) throw new Error('El XML ya contiene 15 camposAdicionales; no es posible agregar RUC Proveedor sin incumplir el límite del SRI.');
      return whole.replace('</infoAdicional>', `${field}</infoAdicional>`);
    });
  }
  return xml.replace(`</${root}>`, `${info}</${root}>`);
}
...
async function construir(emisorId: string) {
  ...
  const direccionMatriz = String(emisor.direccion_matriz ?? '').trim();
  const direccionEstablecimiento = String(punto.direccion ?? '').trim();
  if (!direccionMatriz) throw new Error('La dirección de la matriz está vacía. Configure los datos de la empresa antes de emitir.');
  if (!direccionEstablecimiento) throw new Error(`El establecimiento ${punto.establecimiento} no tiene dirección válida. Configure la dirección antes de emitir.`);
  if (direccionMatriz.length > 300 || direccionEstablecimiento.length > 300) throw new Error('La dirección de matriz o establecimiento supera los 300 caracteres permitidos por el SRI.');
  ...
  const fe = new FacturacionElectronicaEC({
    emisor: { ruc: emisor.ruc, razonSocial: emisor.razon_social, nombreComercial: emisor.nombre_comercial ?? undefined,
      dirMatriz: direccionMatriz, establecimiento: punto.establecimiento, puntoEmision: punto.punto_emision,
      direccionEstablecimiento, contribuyenteEspecial: emisor.contribuyente_especial ?? undefined,
      obligadoContabilidad: emisor.obligado_contabilidad, ambiente, agenteRetencion: emisor.agente_retencion ? 'SI' : undefined },
    p12, p12Password: pass, sequenceProvider: seq, validateXsd: false,
...
export async function emitirDocumentoSri(
  emisorId: string,
  tipo: string,
  datos: Record<string, unknown>,
  onPreparado?: (preparado: { claveAcceso: string; secuencial: string; xmlOriginal: string }) => Promise<void>,
  onAntesDeTransmitir?: () => Promise<void>,
): Promise<ResultadoDocumentoSri> {
  const cfg = MAP[tipo];
  ...
  validarRucProveedor(rucProveedor);
  ...
  validarXmlSRI(xml, { rootTag: cfg.root, requireDirEstablecimiento: tipo !== 'guia_remision' });
  if (VALIDAR_XSD_LOCAL) {
    const xsd = await validateXmlAgainstXsd(cfg.sdk, xml);
    if (!xsd.valid) {
      const erroresXsd = (xsd.errors ?? []).map((e: unknown) => {
        if (typeof e === 'string') return e;
        if (e && typeof e === 'object') {
          const o = e as Record<string, unknown>;
          const codigo = o.code ?? o.codigo ?? o.identificador;
          const mensaje = o.message ?? o.mensaje ?? o.reason ?? o.detalle;
          const linea = o.line ?? o.lineNumber ?? o.linea;
          const columna = o.column ?? o.columnNumber ?? o.columna;
          const partes = [codigo != null ? String(codigo) : '', mensaje != null ? String(mensaje) : '', linea != null ? `línea ${String(linea)}` : '', columna != null ? `columna ${String(columna)}` : ''].filter(Boolean);
      ...
      return {
        ...
        mensaje: `XML rechazado por validación XSD local: ${erroresXsd.join('; ') || 'Error de estructura XML.'}. No fue firmado ni transmitido al SRI. Puedes corregir el borrador y volver a emitirlo.`,
        mensajes: erroresXsd.map((mensaje) => ({ tipo: 'XSD', mensaje })),
      };
    }
  }

  // Persistimos la identidad del comprobante ANTES de firmar/transmitir.
  // Si el proceso se interrumpe después, la clave de acceso y el XML original
  // siguen disponibles para consultar al SRI sin retransmitir el documento.
  if (onPreparado) await onPreparado({ claveAcceso: clave, secuencial, xmlOriginal: xml });

  const firmado = await fe.signXml(xml, cfg.sdk);
  if (onAntesDeTransmitir) await onAntesDeTransmitir();
...
export function validarDatosDocumento(tipo: string, d: Record<string, unknown>): void {
  ...
  if (tipo === 'nota_credito') {
    ...
    exigir(Array.isArray(d.detalles) && d.detalles.length > 0, 'detalles');
    if (Array.isArray(d.detalles)) {
      exigir(casiIgual(d.detalles.reduce((s:any,x:any)=>s+Number(x?.precioTotalSinImpuesto||0),0), Number(d.totalSinImpuestos||0)), 'detalle vs totalSinImpuestos');
      d.detalles.forEach((x:any,i:number)=>{ exigir(!!texto(x,'descripcion') && texto(x,'descripcion').length<=300, `detalles[${i}].descripcion obligatoria y máximo 300 caracteres`); });
    }
...
export function normalizarDatosDocumento(tipo: string, datos: Record<string, unknown>): Record<string, unknown> {
  ...
  if (tipo === 'liquidacion_compra') {
    ...
    if (Array.isArray(d.detalles)) {
      d.detalles = d.detalles.map((x:any, i:number) => ({
        ...x,
        codigoPrincipal: String(x?.codigoPrincipal ?? '').trim().slice(0,25) || `ITEM-${i+1}`,
        descripcion: (String(x?.descripcion ?? '').trim() || 'Bien o servicio adquirido').slice(0,300),
      }));
    }
  }
  if (tipo === 'guia_remision' && Array.isArray(d.destinatarios)) {
    d.destinatarios = d.destinatarios.map((x:any, i:number) => ({
      ...x,
      descripcion: x.descripcion,
      detalles: Array.isArray(x.detalles) ? x.detalles.map((z:any,j:number)=>({
        ...z, codigoPrincipal: String(z?.codigoPrincipal ?? '').trim().slice(0,25) || `ITEM-${i+1}-${j+1}`,
        descripcion: (String(z?.descripcion ?? '').trim() || 'Bien transportado').slice(0,300),
      })) : x.detalles,
    }));
  }
  if (tipo === 'retencion' && Array.isArray(d.docsSustento)) {
    d.docsSustento = d.docsSustento.map((doc:any)=>({
      ...doc,
      retenciones: Array.isArray(doc.retenciones) ? doc.retenciones.map((r:any)=>{
        const impuesto = String(r?.codigo ?? '') === '2' ? 'IVA' : 'RENTA';
        const cat = buscarRetencionSRI(String(r?.codigoRetencion ?? '').trim(), impuesto);
        if (!cat) return r;
        const base = Number(r?.baseImponible ?? 0);
        const valor = Number.isFinite(base) ? Number((base * cat.porcentaje / 100).toFixed(2)) : Number(r?.valorRetenido ?? 0);
        return {...r, codigoRetencion:cat.codigo, porcentajeRetener:cat.porcentaje, valorRetenido:valor};
      }) : doc.retenciones,
    }));
  }
  if (tipo === 'nota_credito') {
