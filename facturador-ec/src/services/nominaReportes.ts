...
export async function generarExcelRoles(opts:{empresa:any;periodo:string;rows:Row[];individual?:boolean}):Promise<Buffer>{
 ...
 ws.autoFilter={from:'A3',to:`N${Math.max(3,ws.rowCount)}`}; ws.views=[{state:'frozen',ySplit:3}]; ws.pageSetup={orientation:'landscape',fitToPage:true,fitToWidth:1,fitToHeight:0,paperSize:9}; ws.pageMargins={left:.3,right:.3,top:.5,bottom:.5,header:.2,footer:.2};
