...
export async function procesarNotificacionPayphone(body:any) {
  ...
  const approved = status === 'approved' || statusCode === 3;
  ...
  // Idempotencia fuerte: una notificación repetida nunca vuelve a activar ni a cobrar.
  ...
  if (approved) {
    ...
    await supabase.from('pagos_solicitud_saas').update({ respuesta_proveedor: { ...body, contsertrib: { activado:true, activacion } }, updated_at:new Date().toISOString(), notificacion_admin_estado:'enviado' }).eq('id',pago.id);
    await notificarPagoAroot(actualizado, pago.solicitudes_registro_saas, activacion);
...
async function activarPlanPagado(pago:any, solicitud:any) {
  ...
  if (!emisorId) {
    const ins = await supabase.from('emisores').insert({ruc,razon_social:razonSocial,nombre_comercial:solicitud?.nombre_comercial??null,direccion_matriz:direccion||'Sin dirección registrada',obligado_contabilidad:false,ambiente:solicitud?.ambiente==='pruebas'?'pruebas':'produccion'}).select('id').single();
  ...
  const dir = direccion || 'Sin dirección registrada';
  ...
  const punto = await supabase.from('puntos_emision').upsert({emisor_id:emisorId,establecimiento:'001',punto_emision:'001',direccion:dir,activo:true},{onConflict:'emisor_id,establecimiento,punto_emision'}).select('id').single();
  if (punto.error) throw new Error(`No se pudo activar el punto de emisión: ${punto.error.message}`);
