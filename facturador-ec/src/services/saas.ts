...
export async function comprobarCaracteristica(emisorId: string, caracteristica: 'ats' | 'inventario' | 'ticket_pos'): Promise<{ ok: boolean; plan?: PlanSaas; mensaje?: string }> {
  ...
  if (caracteristica === 'ticket_pos') {
    if (!plan.incluye_ticket_pos) return { ok: false, plan, mensaje: `Ticket/POS no está incluido en el plan ${plan.nombre}.` };
    const { data: permiso, error } = await supabase
      .from('contribuyentes_cliente_saas')
      .select('ticket_pos_habilitado,activo')
      .eq('emisor_id', emisorId)
      .maybeSingle();
    if (error) throw new Error(`No se pudo verificar la autorización Ticket/POS: ${error.message}`);
    if (!permiso?.activo || permiso.ticket_pos_habilitado !== true) {
      return { ok: false, plan, mensaje: 'Ticket/POS no está habilitado por el administrador ROOT para este contribuyente.' };
    }
    return { ok: true, plan };
  }
