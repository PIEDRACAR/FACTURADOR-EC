...
export type PlanSaas = {
  ...
  modulos_config?: Record<string, boolean> | null;
};
...
export async function comprobarCaracteristica(emisorId: string, caracteristica: 'ats' | 'inventario' | 'ticket_pos'): Promise<{ ok: boolean; plan?: PlanSaas; mensaje?: string }> {
  ...
  const cfg = plan.modulos_config && typeof plan.modulos_config === 'object' ? plan.modulos_config : {};
  const explicito = Object.prototype.hasOwnProperty.call(cfg, caracteristica);
  const incluidaEnPlan = explicito
    ? cfg[caracteristica] !== false
    : caracteristica === 'ats'
      ? plan.incluye_ats
      : caracteristica === 'inventario'
        ? plan.incluye_inventario
        : plan.incluye_ticket_pos !== false;
  if (!incluidaEnPlan) return { ok: false, plan, mensaje: `La función ${caracteristica === 'ticket_pos' ? 'TICKET / POS' : caracteristica.toUpperCase()} no está incluida en el plan ${plan.nombre}.` };
  if (caracteristica === 'ticket_pos') {
    const { data, error } = await supabase.from('emisores').select('ticket_pos_habilitado').eq('id', emisorId).maybeSingle();
    if (error) throw new Error(`No se pudo consultar la habilitación individual de Ticket POS: ${error.message}`);
    if (data && data.ticket_pos_habilitado === false) return { ok: false, plan, mensaje: 'Ticket / Venta POS no está habilitado para este contribuyente. Solicita su activación al administrador.' };
  }
  return { ok: true, plan };
}

const LEGACY_MODULES: Record<string, keyof PlanSaas | null> = {
  inventario: 'incluye_inventario',
  ats: 'incluye_ats',
  carga_electronica: 'incluye_carga_electronica',
  reportes_avanzados: 'incluye_reportes_avanzados',
  ticket_pos: 'incluye_ticket_pos',
};

export async function comprobarModuloPlan(emisorId: string, modulo: string): Promise<{ ok: boolean; plan?: PlanSaas; mensaje?: string }> {
  const plan = await obtenerPlanSaas(emisorId);
  if (!plan) return { ok: true };
  const cfg = plan.modulos_config && typeof plan.modulos_config === 'object' ? plan.modulos_config : {};
  let habilitado: boolean;
  if (Object.prototype.hasOwnProperty.call(cfg, modulo)) {
    habilitado = cfg[modulo] !== false;
  } else {
    const legacy = LEGACY_MODULES[modulo];
    habilitado = legacy ? plan[legacy] !== false : true;
  }
  if (!habilitado) return { ok: false, plan, mensaje: `El módulo ${modulo.replace(/_/g, ' ').toUpperCase()} no está habilitado en el plan ${plan.nombre}.` };
  return { ok: true, plan };
}

export async function comprobarTicketPosGlobal(): Promise<{ ok: boolean; mensaje?: string }> {
  // Se conserva por compatibilidad con versiones anteriores. La autorización real
  // de Ticket POS es ahora individual por contribuyente y por plan.
  return { ok: true };
}
