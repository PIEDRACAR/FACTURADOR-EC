...
export async function obtenerDocumentoSriPdf(id: string): Promise<{buffer: Buffer; filename: string; tipo: string; secuencial: string; email: string; estado: string}> {
  ...
  // El encabezado ocupa una sola zona compacta; el resto del RIDE empieza debajo
  // de la línea de seguridad para evitar solapamientos y desperdicio vertical.
  doc.y=212;
  ...
  // La clave de acceso y su código de barras ya están en la caja superior derecha,
  // como en el modelo RIDE de referencia. No se repiten al pie del documento.
...
async function renderHeader(doc:any,o:any){
  ...
  // Zona reservada al logo: idéntica para los seis comprobantes. El logo nunca
  // invade la caja del documento ni modifica el área de datos del emisor.
  doc.roundedRect(left+10,22,250,58,4).fill(C.blueSoft).strokeColor(C.line).stroke();
  if(hasLogo){ try { doc.image(Buffer.from(o.logoBase64,'base64'),left+18,28,{fit:[234,46],valign:'center'}); } catch {} }
  else { doc.fillColor(C.blue).font('Helvetica-Bold').fontSize(11).text('CONTSERTRIB',left+20,43,{width:230,align:'center'}); doc.font('Helvetica').fontSize(7).fillColor(C.muted).text('FACTURACIÓN ELECTRÓNICA',left+20,59,{width:230,align:'center'}); }
  doc.roundedRect(left,86,leftW,116,4).lineWidth(.8).strokeColor(C.line).stroke();
  doc.font('Helvetica-Bold').fontSize(10.2).fillColor(C.navy).text(text(o.emisor?.razon_social||o.emisor?.nombre_comercial||'CONTSERTRIB FACTURACIÓN'),left+10,96,{width:leftW-20,height:23});
  doc.font('Helvetica-Bold').fontSize(7.2).fillColor(C.ink).text(`R.U.C.: ${text(o.emisor?.ruc)}`,left+10,122,{width:leftW-20});
  doc.font('Helvetica').fontSize(6.8).fillColor(C.muted).text(text(o.emisor?.direccion_matriz),left+10,136,{width:leftW-20,height:22});
  doc.font('Helvetica-Bold').fontSize(6.7).fillColor(C.navy).text(o.esAutorizado?'DOCUMENTO ELECTRÓNICO':'BORRADOR PARA REVISIÓN',left+10,184,{width:leftW-20,align:'center'});
  ...
  if(o.clave){ try { const barras=await generarCodigoBarras(String(o.clave)); doc.image(barras,boxX+14,fy+41,{width:boxW-28,height:26}); } catch {} }
  doc.font('Helvetica-Bold').fontSize(6.0).fillColor(C.muted).text('CLAVE DE ACCESO',boxX+10,fy+69,{width:boxW-20,align:'center'});
  doc.font('Helvetica').fontSize(5.4).fillColor(C.ink).text(text(o.clave),boxX+8,fy+79,{width:boxW-16,height:9,align:'center',characterSpacing:.08,ellipsis:true});
  if(!o.esAutorizado){ doc.roundedRect(left+8,188,leftW-16,10,2).fill('#FEF3C7'); doc.fillColor(C.amber).font('Helvetica-Bold').fontSize(6.1).text('BORRADOR · SIN VALIDEZ TRIBUTARIA',left+10,190,{width:leftW-20,align:'center'}); }
