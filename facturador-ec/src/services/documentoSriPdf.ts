...
const PAGE = { left:38, right:38, top:34, bottom:48 };
...
function heading(doc:any,title:string, y?:number){ if(y!==undefined) doc.y=y; const x=PAGE.left,w=doc.page.width-PAGE.left-PAGE.right; doc.roundedRect(x,doc.y,w,22,4).fill(C.soft); doc.fillColor(C.navy).font('Helvetica-Bold').fontSize(8.5).text(title.toUpperCase(),x+9,doc.y+7,{width:w-18}); doc.y+=29; doc.fillColor(C.ink); }
function labelValue(doc:any,label:string,value:unknown,x:number,y:number,w:number){ doc.font('Helvetica-Bold').fontSize(7.3).fillColor(C.muted).text(label.toUpperCase(),x,y,{width:w}); doc.font('Helvetica').fontSize(8.7).fillColor(C.ink).text(text(value),x,y+10,{width:w,height:25}); }
...
function tableHeader(doc:any, y:number, cols:Array<{title:string,width:number,align?:'left'|'right'|'center'}>){ let x=PAGE.left; doc.rect(PAGE.left,y,doc.page.width-PAGE.left-PAGE.right,23).fill(C.navy); doc.font('Helvetica-Bold').fontSize(7).fillColor(C.white); for(const c of cols){ doc.text(c.title,x+5,y+8,{width:c.width-10,align:c.align??'left'}); x+=c.width; } return y+23; }
function tableRow(doc:any,y:number,values:string[],cols:Array<{title:string,width:number,align?:'left'|'right'|'center'}>,h=24){ if(y+h>doc.page.height-PAGE.bottom){doc.addPage(); y=PAGE.top; y=tableHeader(doc,y,cols);} let x=PAGE.left; doc.rect(PAGE.left,y,doc.page.width-PAGE.left-PAGE.right,h).fill(y%2?C.white:C.soft); doc.font('Helvetica').fontSize(7.3).fillColor(C.ink); values.forEach((v,i)=>{const c=cols[i];doc.text(v,x+5,y+7,{width:c.width-10,height:h-8,align:c.align??'left',ellipsis:true});x+=c.width;}); return y+h; }
...
export async function obtenerDocumentoSriPdf(id: string): Promise<{buffer: Buffer; filename: string; tipo: string; secuencial: string; email: string; estado: string}> {
  ...
  labelValue(doc,'Fecha de emisión',datos.fechaEmision??d.created_at,cols[2],doc.y,170); doc.y+=43;
  ...
  labelValue(doc,'Período fiscal',datos.periodoFiscal,cols[2],doc.y,170); doc.y+=40;
...
function renderLiquidacion(doc:any,d:any){
  ...
  heading(doc,'Impuestos'); const imps=Array.isArray(d.totalConImpuestos)?d.totalConImpuestos:[]; const colsI=[{title:'Código / tarifa',width:150},{title:'Base imponible',width:150,align:'right' as const},{title:'Valor impuesto',width:150,align:'right' as const}]; let yi=tableHeader(doc,doc.y,colsI); for(const imp of imps){const code=text(imp.codigoPorcentaje??'0');const tarifa=imp.tarifa!==undefined?`${money(imp.tarifa)}%`:code==='4'?'15%':code==='8'?'8%':code==='5'?'5%':'0%'; yi=tableRow(doc,yi,[`IVA · ${tarifa}`,`$${money(imp.baseImponible)}`,`$${money(imp.valor)}`],colsI,25);} doc.y=yi+12; renderTotals(doc,[['Total sin impuestos',d.totalSinImpuestos],['Descuento total',d.totalDescuento],['Importe total',d.importeTotal]],true); heading(doc,'Formas de pago'); const pagos=Array.isArray(d.pagos)?d.pagos:[]; const colsP=[{title:'Forma de pago',width:360},{title:'Valor',width:145,align:'right' as const}]; let yp=tableHeader(doc,doc.y,colsP); for(const p of pagos) yp=tableRow(doc,yp,[text(p.formaPago),`$${money(p.total)}`],colsP,25); if(!pagos.length) yp=tableRow(doc,yp,['No registrada','—'],colsP,25); doc.y=yp;
}
function renderGuia(doc:any,d:any){
  ...
  for(const x of dest){
    ensureSpace(doc,150);
    const boxY=doc.y;
    doc.roundedRect(PAGE.left,boxY,doc.page.width-PAGE.left-PAGE.right,55,5).fill(C.soft);
