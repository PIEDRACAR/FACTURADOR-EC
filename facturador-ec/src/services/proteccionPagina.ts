...
export function inyectarProteccionSesion(html: string): string {
  if (html.includes('</head>')) {
    const recursos = `<script>window.__FACTURADOR_EMISOR_ID__=(new URLSearchParams(location.search).get("emisorId")||localStorage.getItem("facturador_emisor_id")||"");</script><link rel="stylesheet" href="/app.css?v=9.9.4"><script src="/app-shell.js?v=9.9.4" defer></script>`;
