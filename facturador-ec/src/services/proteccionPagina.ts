...
export function inyectarProteccionSesion(html: string): string {
  if (html.includes('</head>')) {
    const tieneCss = /<link[^>]+href=["']\/app\.css(?:\?[^"']*)?["'][^>]*>/i.test(html);
    const tieneShell = /<script[^>]+src=["']\/app-shell\.js(?:\?[^"']*)?["'][^>]*><\/script>/i.test(html);
    const recursos = `<script>window.__FACTURADOR_EMISOR_ID__=(new URLSearchParams(location.search).get("emisorId")||localStorage.getItem("facturador_emisor_id")||"");</script>${tieneCss ? '' : '<link rel="stylesheet" href="/app.css?v=9.7.13">'}${tieneShell ? '' : '<script src="/app-shell.js?v=9.7.13" defer></script>'}`;
