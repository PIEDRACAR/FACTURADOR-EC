...
export async function renderizarRidePdf(c: DatosRide['comprobante']): Promise<Buffer> {
  ...
  if (c.logo_ride_base64 && c.logo_ride_mime) {
    try {
      doc.image(Buffer.from(c.logo_ride_base64, 'base64'), 36, 40, { fit: [84, 58], align: 'left', valign: 'center' });
    } catch { /* Logo inválido: se conserva el RIDE estándar. */ }
  }
  doc.fontSize(13).font('Helvetica-Bold').text(c.emisores.razon_social, encabezadoX, 40, { width: 190 });
  ...
  doc.fontSize(8.2).font('Helvetica').text(c.emisores.direccion_matriz, encabezadoX, doc.y + 2, { width: 190 })
  ...
  doc.roundedRect(cajaX, 36, cajaW, cajaH, 5).strokeColor('#64748b').stroke();
