...
export async function renderizarRidePdf(c: DatosRide['comprobante']): Promise<Buffer> {
  ...
  doc
    ...
    .fontSize(10.5)
    .font('Helvetica-Bold')
    .text('R.U.C.: ' + c.emisores.ruc, cajaX + 9, 45, { width: cajaW - 18 });
  doc
    .fontSize(9.5)
    .text('FACTURA', cajaX + 9, 61, { width: cajaW - 18 })
    .fontSize(10)
    .text(`No. ${c.puntos_emision.establecimiento}-${c.puntos_emision.punto_emision}-${c.secuencial ?? '—'}`, cajaX + 9, 76, { width: cajaW - 18 });
  doc
    .fontSize(7.2)
    .font('Helvetica-Bold')
    .text('NÚMERO DE AUTORIZACIÓN', cajaX + 9, 94, { width: cajaW - 18 });
  doc
    .font('Helvetica')
    .fontSize(7.1)
    .text(c.numero_autorizacion ?? '(pendiente)', cajaX + 9, 105, { width: cajaW - 18, lineBreak: false });
  doc
    .fontSize(7.1)
    .text(`FECHA/HORA DE AUTORIZACIÓN: ${c.fecha_autorizacion ? new Date(c.fecha_autorizacion).toLocaleString('es-EC',{timeZone:'America/Guayaquil'}) : 'PENDIENTE'}`, cajaX + 9, 116, { width: cajaW - 18 });
  doc
    .font('Helvetica-Bold')
    .fontSize(7.2)
    .text(`AMBIENTE: ${esPruebas ? 'PRUEBAS' : 'PRODUCCIÓN'}   ·   EMISIÓN: NORMAL`, cajaX + 9, 139, { width: cajaW - 18 });
  ...
  if (esPruebas) {
    doc
      ...
      .fontSize(8.5)
      .font('Helvetica-Bold')
      .text('AMBIENTE: PRUEBAS — este comprobante no tiene validez tributaria', 36, 168, { width: anchoUtil, align: 'center' })
      .fillColor('black');
  }

  let y = esPruebas ? 184 : 174;
