...
export async function renderizarRidePdf(c: DatosRide['comprobante']): Promise<Buffer> {
  ...
  doc
    ...
    .fontSize(7).text(`FECHA/HORA AUTORIZACIÓN: ${c.fecha_autorizacion ? new Date(c.fecha_autorizacion).toLocaleString('es-EC',{timeZone:'America/Guayaquil'}) : 'PENDIENTE'}`, cajaX + 8, 128, { width: anchoUtil - (cajaX - 36) - 16 });
    .fontSize(7.5)
