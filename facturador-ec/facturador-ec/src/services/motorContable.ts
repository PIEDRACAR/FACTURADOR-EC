...
export async function contabilizarVenta(comprobanteId:string,userId?:string){
  const {data:c,error:ce}=await supabase.from('comprobantes').select('id,emisor_id,estado,tipo,created_at,importe_total,subtotal_0,subtotal_5,subtotal_8,subtotal_15,total_iva,propina,secuencial,clave_acceso').eq('id',comprobanteId).single();
