...
function normalizarTarifaIva(valor: unknown, cfg = DEFAULT_IVA): { tarifa: string; codigo: string; tasa: number; grupo: '0'|'5'|'8'|'general'|'otros' } {
  ...
  if (Math.abs(n-cfg.general)<0.0001 || raw === '15') return { tarifa:String(cfg.general), codigo:cfg.codigoGeneral, tasa:cfg.general/100, grupo:'general' };
...
export async function registrarRutasPos(app: FastifyInstance) {
  ...
  app.post<{ Body: VentaBody }>('/pos/venta', async (request, reply) => {
    ...
    const cfgIva = cfgIvaRow ? {
      general: Number(cfgIvaRow.tarifa_general ?? 13), codigoGeneral: String(cfgIvaRow.codigo_general ?? '4'),
    ...
    for (const item of body.items) {
      ...
      if (item.productoId) {
        ...
        tarifaIva = normalizarTarifaIva(producto.tarifa_iva, cfgIva).tarifa;
