...
const TARIFAS_VALIDAS = new Set(['0', '5', '8', '13', '15', 'exento', 'no_objeto']);
...
export async function registrarRutasImportacionProductos(app: FastifyInstance) {
  ...
  app.post<{ Body: { emisorId?: string; archivoBase64?: string } }>('/productos/importar', async (request, reply) => {
    const { emisorId, archivoBase64 } = request.body ?? {};
    if (!emisorId) return reply.status(400).send({ error: 'Falta emisorId.' });
    if (!archivoBase64) return reply.status(400).send({ error: 'Falta el archivo.' });
    ...
    const productosValidos: Array<Record<string, unknown>> = [];
    ...
    filas.forEach((fila, indice) => {
      ...
      const tarifaIva = (fila.tarifaIva || fila['IVA (0, 5, 8, 15, exento o no_objeto)'] || '13').trim();
      ...
      if (!codigo) { errores.push(`Fila ${numeroFila}: falta el código.`); return; }
      if (!descripcion) { errores.push(`Fila ${numeroFila}: falta la descripción.`); return; }
      if (isNaN(stockCritico) || stockCritico < 0) { errores.push(`Fila ${numeroFila}: stock crítico / STOP inválido.`); return; }
      if (isNaN(stockMinimo) || stockMinimo < stockCritico) { errores.push(`Fila ${numeroFila}: el stock mínimo no puede ser menor que el stock crítico.`); return; }
      if (stockMaximo !== null && (isNaN(stockMaximo) || stockMaximo < stockMinimo)) { errores.push(`Fila ${numeroFila}: el stock máximo debe ser mayor o igual al mínimo.`); return; }
      if (isNaN(precio) || precio < 0) { errores.push(`Fila ${numeroFila}: precio de venta inválido ("${precioTexto}").`); return; }
      const tarifaNumerica = Number(tarifaIva); if (!(tarifaIva === 'exento' || tarifaIva === 'no_objeto' || (Number.isFinite(tarifaNumerica) && tarifaNumerica >= 0 && tarifaNumerica <= 100))) { errores.push(`Fila ${numeroFila}: tarifa de IVA "${tarifaIva}" no es válida.`); return; }
