# CONTSERTRIB FACTURACIÓN — sistema de facturación electrónica SRI Ecuador
...
## v9.6.0 — Panel maestro SaaS del proveedor
- Panel `/admin-proveedor` para administrar empresas clientes, planes, suscripciones y cobros.
- Alta asistida de clientes sin exigir el certificado en el primer paso.
- Creación del usuario administrador del cliente y punto de emisión 001-001.
- Planes configurables; valores iniciales provisionales: Básico $10, Profesional $20, Empresarial $35.
- Registro de pagos y extensión de vencimiento.
- Suspensión/activación comercial de suscripciones.
- Regeneración de contraseña y reenvío de acceso.
- Variable `PROVEEDOR_ADMIN_EMAILS` para autorizar el panel maestro inicialmente.
- Variable `APP_URL` para construir enlaces de acceso.
- Ejecutar `sql/migracion_saas_proveedor_v960.sql` antes de usar el panel.
- La firma electrónica del cliente sigue siendo responsabilidad del cliente y se configura desde su cuenta; no se envía la clave SRI al proveedor.


## v9.9.0 — Motor tributario y facturación automática SaaS
- Motor tributario configurable con tarifa general inicial 13%, IVA 0%, no objeto y exento separados.
- Historial y vigencia para futuras reformas.
- Cada cobro SaaS genera automáticamente su factura electrónica y la vincula al pago.
- Ver `README_V9_9_0_MOTOR_TRIBUTARIO_SAAS.md` y ejecutar `sql/migracion_v990_motor_tributario_saas.sql`.
