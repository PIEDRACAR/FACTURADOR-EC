...
function formatearMensajesDocumento(respuesta) {
...
function dateDdMmYyyy(v) {
    return normalizarFechaEmisionEcuador(v);
}
function addProviderRuc(xml, root, ruc) {
    ...
    if (p < 0)
        throw new Error(`No se encontró ${close} para insertar el RUC del proveedor.`);
    return xml.slice(0, p) + info + xml.slice(p);
}
async function construir(emisorId) {
    ...
    if (error || !cert)
        throw new Error('El emisor no tiene certificado electrónico activo.');
    if (new Date(cert.fecha_expiracion) < new Date())
        throw new Error(`El certificado está vencido (${cert.fecha_expiracion}).`);
    ...
    return { fe, seq, ambiente: ambiente, ruc: emisor.ruc, establecimiento: punto.establecimiento, puntoEmision: punto.punto_emision };
}
export async function emitirDocumentoSri(emisorId, tipo, datos) {
    const cfg = MAP[tipo];
    if (!cfg)
        throw new Error('Tipo de documento SRI no soportado.');
    ...
    if (!/^\d{13}$/.test(rucProveedor))
        throw new Error('Debe existir un RUC de proveedor de facturación válido de 13 dígitos.');
    ...
    for (let intento = 0; intento < 10; intento++) {
        try {
            ...
            ultimoError = null;
            const estado = normalizarEstadoSriDocumento(auth.estado);
...
function numero(d, clave) {
    const n = Number(d[clave]);
    return Number.isFinite(n) ? n : NaN;
}
export function validarDatosDocumento(tipo, d) {
    const faltan = [];
    const exigir = (ok, campo) => { if (!ok)
        faltan.push(campo); };
    const casiIgual = (a, b) => Math.abs(a - b) <= 0.02;
    const montoNoNegativo = (v) => Number.isFinite(Number(v)) && Number(v) >= 0;
    ...
    if (tipo === 'nota_credito') {
        ...
        if (Array.isArray(d.detalles)) {
            const base = d.detalles.reduce((s, x) => s + Number(x?.precioTotalSinImpuesto || 0), 0);
            const iva = d.detalles.reduce((s, x) => s + (Array.isArray(x?.impuestos) ? x.impuestos.reduce((a, t) => a + Number(t?.valor || 0), 0) : 0), 0);
        ...
        if (Array.isArray(d.motivos))
            exigir(casiIgual(d.motivos.reduce((s, x) => s + Number(x?.valor || 0), 0), Number(d.totalSinImpuestos || 0)), 'suma de motivos vs totalSinImpuestos');
        if (Array.isArray(d.impuestos))
            exigir(casiIgual(d.impuestos.reduce((s, x) => s + Number(x?.valor || 0), 0) + Number(d.totalSinImpuestos || 0), Number(d.valorTotal || 0)), 'impuestos vs valorTotal');
    }
        ...
        exigir(Array.isArray(d.detalles) && d.detalles.length > 0, 'detalles');
        if (Array.isArray(d.detalles))
            exigir(casiIgual(d.detalles.reduce((s, x) => s + Number(x?.precioTotalSinImpuesto || 0), 0), Number(d.totalSinImpuestos || 0)), 'detalle vs totalSinImpuestos');
        if (Array.isArray(d.pagos))
            exigir(casiIgual(d.pagos.reduce((s, x) => s + Number(x?.total || 0), 0), Number(d.importeTotal || 0)), 'pagos vs importeTotal');
    }
        ...
        if (Array.isArray(d.docsSustento))
            d.docsSustento.forEach((x, i) => { exigir(!!texto(x, 'codSustento'), `docsSustento[${i}].codSustento`); exigir(!!texto(x, 'codDocSustento'), `docsSustento[${i}].codDocSustento`); exigir(!!texto(x, 'numDocSustento'), `docsSustento[${i}].numDocSustento`); exigir(/^\d{2}\/\d{2}\/\d{4}$/.test(texto(x, 'fechaEmisionDocSustento')), `docsSustento[${i}].fechaEmisionDocSustento`); exigir(/^\d{49}$/.test(texto(x, 'numAutDocSustento')), `docsSustento[${i}].numAutDocSustento (49 dígitos)`); exigir(Array.isArray(x.retenciones) && x.retenciones.length > 0, `docsSustento[${i}].retenciones`); if (Array.isArray(x.retenciones))
                x.retenciones.forEach((r, j) => { exigir(!!texto(r, 'codigo'), `retenciones[${i}][${j}].codigo`); exigir(!!texto(r, 'codigoRetencion'), `retenciones[${i}][${j}].codigoRetencion`); exigir(Number(r.valorRetenido) > 0, `retenciones[${i}][${j}].valorRetenido`); }); });
    }
    if (faltan.length)
        throw new Error(`Datos incompletos o inconsistentes para ${tipo}: ${faltan.join(', ')}.`);
}
export function normalizarDatosDocumento(tipo, datos) {
    ...
    if (tipo === 'nota_credito' || tipo === 'nota_debito') {
        if (!d.numDocModificado && d.sustento)
            d.numDocModificado = d.sustento;
        if (!d.motivo && d.detalle)
            d.motivo = d.detalle;
        if (!d.razonSocialComprador && d.razonSocial)
            d.razonSocialComprador = d.razonSocial;
        if (!d.identificacionComprador && d.identificacion)
            d.identificacionComprador = d.identificacion;
        if (!d.tipoIdentificacionComprador)
            d.tipoIdentificacionComprador = /^\d{13}$/.test(String(d.identificacionComprador ?? '')) ? '04' : '05';
    }
    if (tipo === 'liquidacion_compra') {
        if (!d.razonSocialProveedor && d.razonSocial)
            d.razonSocialProveedor = d.razonSocial;
        if (!d.identificacionProveedor && d.identificacion)
            d.identificacionProveedor = d.identificacion;
    }
    if (tipo === 'retencion') {
        if (!d.razonSocialSujetoRetenido && d.razonSocial)
            d.razonSocialSujetoRetenido = d.razonSocial;
        if (!d.identificacionSujetoRetenido && d.identificacion)
            d.identificacionSujetoRetenido = d.identificacion;
        if (!d.tipoIdentificacionSujetoRetenido)
            d.tipoIdentificacionSujetoRetenido = /^\d{13}$/.test(String(d.identificacionSujetoRetenido ?? '')) ? '04' : '05';
    }
    if ('fechaEmision' in d)
        d.fechaEmision = dateDdMmYyyy(d.fechaEmision);
    for (const k of ['fechaEmisionDocSustento', 'fechaIniTransporte', 'fechaFinTransporte']) {
        if (k in d && d[k])
            d[k] = dateDdMmYyyy(d[k]);
    }
    if (tipo === 'retencion' && typeof d.periodoFiscal === 'string' && /^\d{4}-\d{2}$/.test(d.periodoFiscal))
        d.periodoFiscal = `${d.periodoFiscal.slice(5)}/${d.periodoFiscal.slice(0, 4)}`;
    if (tipo === 'guia_remision' && Array.isArray(d.destinatarios)) {
        d.destinatarios = d.destinatarios.map((x) => ({ ...x, fechaEmisionDocSustento: x?.fechaEmisionDocSustento ? dateDdMmYyyy(x.fechaEmisionDocSustento) : x?.fechaEmisionDocSustento }));
    }
    if (tipo === 'retencion' && Array.isArray(d.docsSustento)) {
        d.docsSustento = d.docsSustento.map((x) => ({ ...x, fechaEmisionDocSustento: dateDdMmYyyy(x.fechaEmisionDocSustento), fechaRegistroContable: x?.fechaRegistroContable ? dateDdMmYyyy(x.fechaRegistroContable) : x?.fechaRegistroContable }));
    }
    return d;
}
//# sourceMappingURL=documentosSri.js.map