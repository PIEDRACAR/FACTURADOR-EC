...
export async function emitirDocumentoSri(emisorId, tipo, datos) {
    ...
    if (recepcion.estado === 'DEVUELTA') {
        const mensajes = [
            ...(recepcion.mensajes ?? []),
            ...(recepcion.comprobantes?.comprobante?.mensajes ?? []),
        ];
        const msg = mensajes.map((m) => `${m.identificador ? `${m.identificador}: ` : ''}${m.mensaje ?? ''}${m.informacionAdicional ? ` — ${m.informacionAdicional}` : ''}`).filter(Boolean).join('; ');
