...
const NOMBRES_TIPO_IDENTIFICACION = {
    '04': 'RUC',
    '05': 'Cédula',
    '06': 'Pasaporte',
    '07': 'Consumidor Final',
...
async function obtenerDatosRide(comprobanteId) {
    const { data, error } = await supabase
        .from('comprobantes')
        .select(`*, emisores(*), puntos_emision(*), clientes(*),
    ...
    const { data: conf } = await supabase.from('configuracion_sistema').select('ruc_proveedor_facturacion').eq('emisor_id', data.emisor_id).maybeSingle();
    const rucProveedor = (process.env.RUC_PROVEEDOR_FACTURACION?.trim() || String(conf?.ruc_proveedor_facturacion ?? '').trim() || null);
    return { ...data, ruc_proveedor_facturacion: rucProveedor };
}
...
export async function generarRidePdf(comprobanteId) {
...
export async function renderizarRidePdf(c) {
    ...
    doc
        ...
        .fontSize(7.5)
        .text(`AMBIENTE: ${esPruebas ? 'PRUEBAS' : 'PRODUCCIÓN'} · EMISIÓN: NORMAL`, cajaX + 8, 118, { width: anchoUtil - (cajaX - 36) - 16 });
    if (esPruebas) {
        doc
            .fillColor('#b91c1c')
            .fontSize(9)
    ...
    const basesPorTarifa = new Map();
    for (const item of c.comprobante_items) {
        const tarifa = String(item.tarifa_iva ?? '0');
        basesPorTarifa.set(tarifa, Number(basesPorTarifa.get(tarifa) ?? 0) + Number(item.precio_total_sin_impuesto ?? 0));
    }
    const filasTotales = [];
    for (const tarifa of [...basesPorTarifa.keys()].sort((a, b) => Number(b) - Number(a))) {
        const base = Number(basesPorTarifa.get(tarifa) ?? 0);
        if (base <= 0)
            continue;
        const etiqueta = tarifa === '0' ? 'Subtotal 0%' : tarifa === 'exento' ? 'Subtotal exento' : tarifa === 'no_objeto' ? 'Subtotal no objeto' : `Subtotal ${tarifa}%`;
        filasTotales.push([etiqueta, base]);
    }
    filasTotales.push(['Descuento', c.total_descuento], ['IVA', c.total_iva]);
    if (c.propina > 0)
        filasTotales.push(['Propina', c.propina]);
    ...
    doc.fontSize(7).fillColor('#64748b').text('Documento generado electrónicamente. Consulta de validez: https://srienlinea.sri.gob.ec', 36, doc.page.height - 50, { width: anchoUtil, align: 'center' });
    doc.end();
    return listo;
}
