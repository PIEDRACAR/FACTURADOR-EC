...
export function inyectarProteccionSesion(html) {
    if (html.includes('</head>')) {
        const recursos = `<script>window.__FACTURADOR_EMISOR_ID__=(new URLSearchParams(location.search).get("emisorId")||localStorage.getItem("facturador_emisor_id")||"");</script><link rel="stylesheet" href="/app.css"><script src="/app-shell.js?v=9.4.7" defer></script>`;
        return html.replace('</head>', `${recursos}\n${SCRIPT_PROTECCION}\n</head>`);
    }
    // Respaldo por si alguna página no tiene </head> explícito.
    return SCRIPT_PROTECCION + html;
}
//# sourceMappingURL=proteccionPagina.js.map