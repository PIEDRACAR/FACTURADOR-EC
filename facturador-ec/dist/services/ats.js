...
const fechaDdmmyyyy = (v) => { const [y, m, d] = v.slice(0, 10).split('-'); return `${d}/${m}/${y}`; };
function tipoId(id) {
    const x = String(id ?? '');
    if (x === '9999999999999')
        return '07';
...
function bloqueVenta(v) {
    ...
    const formas = [...new Set(pagos.map((p) => String(p.forma_pago_codigo || '')).filter(Boolean))];
    return {
...
export async function generarAts(emisorId, anio, mes) {
    const feature = await comprobarCaracteristica(emisorId, 'ats');
    if (!feature.ok)
        throw new Error(feature.mensaje);
    if (!Number.isInteger(anio) || anio < 2002 || anio > 2100 || !Number.isInteger(mes) || mes < 1 || mes > 12)
        throw new Error('Período ATS no válido.');
    ...
    if (eErr || !emisor)
        throw new Error(eErr?.message || 'No se encontró el contribuyente.');
    if (vErr)
    ...
    if (saveError)
        throw new Error(`No se pudo guardar la generación ATS: ${saveError.message}`);
