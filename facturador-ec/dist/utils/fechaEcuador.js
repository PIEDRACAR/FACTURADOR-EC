...
function partesFechaEcuador(date = new Date()) {
...
export function fechaEmisionEcuador(date = new Date()) {
    const p = partesFechaEcuador(date);
    return `${p.day}/${p.month}/${p.year}`;
}
/** YYYY-MM-DD, útil para consultas y reportes. */
export function fechaIsoEcuador(date = new Date()) {
    const p = partesFechaEcuador(date);
 ...
 */
export function normalizarFechaEmisionEcuador(value) {
...
//# sourceMappingURL=fechaEcuador.js.map