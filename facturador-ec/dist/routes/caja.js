...
export async function registrarRutasCaja(app) {
    app.get('/caja/actual', async (req, reply) => {
        ...
        if (!caja)
            return reply.send({ abierta: false });
        ...
        for (const c of pagos ?? [])
            for (const p of c.comprobante_formas_pago ?? []) {
                const v = Number(p.valor);
                ingresos += v;
                if (p.forma_pago_codigo === '01')
                    efectivo += v;
                else if (['19', '20', '21', '22', '23'].includes(p.forma_pago_codigo))
                    transferencias += v;
                else
                    tarjetas += v;
            }
    ...
    app.post('/caja/abrir', async (req, reply) => {
        const b = req.body;
        if (!b?.emisorId)
            return reply.status(400).send({ error: 'Falta emisorId.' });
        const { data: abierta } = await supabase.from('cajas').select('id').eq('emisor_id', b.emisorId).eq('estado', 'abierta').maybeSingle();
        if (abierta)
            return reply.status(409).send({ error: 'Ya existe una caja abierta para este negocio.' });
    ...
    app.post('/caja/movimiento', async (req, reply) => {
        const b = req.body;
        if (!b?.cajaId || !b.concepto || !b.monto || b.monto <= 0)
            return reply.status(400).send({ error: 'Caja, concepto y monto válido son obligatorios.' });
        const { data: caja } = await supabase.from('cajas').select('id').eq('id', b.cajaId).eq('estado', 'abierta').maybeSingle();
        if (!caja)
            return reply.status(409).send({ error: 'La caja no está abierta.' });
    ...
    app.post('/caja/cerrar', async (req, reply) => {
        const b = req.body;
        if (!b?.cajaId || b.efectivoDeclarado === undefined)
            return reply.status(400).send({ error: 'Caja y efectivo declarado son obligatorios.' });
    ...
    app.get('/caja/historial', async (req, reply) => { if (!req.query.emisorId)
        return reply.status(400).send({ error: 'Falta emisorId.' }); const { data, error } = await supabase.from('cajas').select('*').eq('emisor_id', req.query.emisorId).order('fecha_apertura', { ascending: false }).limit(100); if (error)
        return reply.status(500).send({ error: error.message }); return reply.send(data); });
}
