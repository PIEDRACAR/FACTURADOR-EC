...
export async function registrarRutasDocumentos(app) {
    ...
    app.get('/api/documentos/sustentos', async (request, reply) => {
        const { emisorId } = request.query;
        if (!emisorId)
            return reply.status(400).send({ error: 'Falta emisorId.' });
        const { data, error } = await supabase.from('comprobantes')
            .select('id,emisor_id,secuencial,clave_acceso,created_at,importe_total,subtotal_0,subtotal_5,subtotal_8,subtotal_15,clientes(razon_social,identificacion),puntos_emision(establecimiento,punto_emision)')
    ...
    app.post('/api/documentos/:id/emitir', async (request, reply) => {
        ...
        try {
            ...
            if (doc.tipo === 'nota_credito' && doc.comprobante_sustento_id) {
                ...
                if (!sustento)
                    throw new Error('No se encontró el comprobante de sustento de la nota de crédito.');
                if (sustento.estado !== 'autorizado' && sustento.estado !== 'AUTORIZADO')
                    throw new Error('La nota de crédito solo puede sustentarse en un comprobante autorizado por el SRI.');
                if (String(datosNormalizados.identificacionComprador ?? '') === '9999999999999')
                    throw new Error('SRI: una factura emitida a CONSUMIDOR FINAL no puede ser anulada ni modificada mediante nota de crédito una vez transmitida.');
                const fechaSustento = new Date(sustento.created_at);
                const limite = new Date(fechaSustento);
                limite.setMonth(limite.getMonth() + 12);
                if (Date.now() > limite.getTime())
                    throw new Error('SRI: la nota de crédito supera el plazo máximo de 12 meses desde la emisión del comprobante de sustento.');
                if (!datosNormalizados.numDocModificado && sustento.secuencial)
                    datosNormalizados.numDocModificado = String(sustento.secuencial);
                if (!datosNormalizados.numAutDocModificado && sustento.clave_acceso)
                    datosNormalizados.numAutDocModificado = String(sustento.clave_acceso);
            }
            await supabase.from('documentos_sri_borrador').update({ intentos: Number(doc.intentos ?? 0) + 1, ultimo_error: null }).eq('id', doc.id);
            const result = await emitirDocumentoSri(doc.emisor_id, doc.tipo, datosNormalizados);
