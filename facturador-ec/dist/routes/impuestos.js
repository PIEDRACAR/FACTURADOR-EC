...
export async function registrarRutasImpuestos(app) {
    ...
    app.post('/impuestos/reglas', async (request, reply) => {
        if (!await esProveedorAdmin(request))
            return reply.status(403).send({ error: 'Solo el administrador de CONTSERTRIB puede publicar reglas tributarias.' });
        const b = request.body ?? {};
        const tipos = new Set(['GENERAL', 'FIJA', 'TURISMO', 'CERO', 'EXENTO', 'NO_OBJETO']);
        if (!b.clave || !b.nombre || !b.fechaInicio || !tipos.has(String(b.tipo)))
            return reply.status(400).send({ error: 'Completa clave, nombre, tipo y fecha de inicio.' });
        const porcentaje = Number(b.porcentaje);
        if (!Number.isFinite(porcentaje) || porcentaje < 0 || porcentaje > 100)
            return reply.status(400).send({ error: 'Porcentaje inválido.' });
        if (!/^\d+$/.test(String(b.codigoSri)))
            return reply.status(400).send({ error: 'El código SRI debe ser numérico.' });
    ...
    app.patch('/impuestos/reglas/:id', async (request, reply) => {
        if (!await esProveedorAdmin(request))
            return reply.status(403).send({ error: 'Solo el administrador de CONTSERTRIB puede modificar reglas tributarias.' });
        const b = request.body ?? {};
        const cambios = { updated_at: new Date().toISOString() };
        if (b.activo !== undefined)
            cambios.activo = Boolean(b.activo);
        if (b.fechaFin !== undefined)
            cambios.fecha_fin = b.fechaFin || null;
        ...
        if (b.prioridad !== undefined)
            cambios.prioridad = Math.floor(Number(b.prioridad));
