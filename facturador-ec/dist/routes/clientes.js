...
function normalizarIdentificacion(v) { return v.replace(/\D/g, ''); }
function validarEmail(v) { return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v); }
function mapearSRI(data) {
...
export async function registrarRutasClientes(app) {
    ...
    // Búsqueda rápida para POS: desde 3 caracteres devuelve solo coincidencias del negocio activo.
    // Se mantiene acotada para que la respuesta sea inmediata incluso con miles de clientes.
    app.get('/clientes/buscar', async (request, reply) => {
        const emisorId = request.query.emisorId;
        const q = String(request.query.q ?? '').trim().replace(/[^0-9A-Za-zÁÉÍÓÚÜÑáéíóúüñ .-]/g, '');
        if (!emisorId)
            return reply.status(400).send({ error: 'Falta el parámetro emisorId.' });
        if (q.length < 3)
            return reply.send({ items: [], total: 0, minimo: 3 });
        const { data, error } = await supabase
            .from('clientes')
            .select('id,tipo_identificacion,identificacion,razon_social,email,telefono,direccion')
            .eq('emisor_id', emisorId)
            .or(`razon_social.ilike.%${q}%,identificacion.ilike.${q}%,email.ilike.%${q}%`)
            .order('razon_social', { ascending: true })
            .limit(15);
        if (error)
            return reply.status(500).send({ error: error.message });
        return reply.send({ items: data ?? [], total: (data ?? []).length, minimo: 3 });
    });
    ...
    app.get('/clientes/consultar', async (request, reply) => {
        ...
        if (!emisorId)
            return reply.status(400).send({ error: 'Falta el parámetro emisorId.' });
        if (!identificacion)
            return reply.status(400).send({ error: 'Indica una identificación.' });
        if (tipo === '05' && !/^\d{10}$/.test(identificacion))
            return reply.status(400).send({ error: 'La cédula debe tener 10 dígitos.' });
        if (tipo === '04' && !/^\d{13}$/.test(identificacion))
            return reply.status(400).send({ error: 'El RUC debe tener 13 dígitos.' });
        ...
        if (tipo === '05' && env.registroCivilLookupUrl) {
            try {
                ...
                if (r.ok) {
                    ...
                    if (d)
                        return reply.send({ encontrado: true, fuente: 'REGISTRO_CIVIL', datos: d });
                }
            }
            catch (e) {
                request.log.warn({ err: e }, 'Consulta Registro Civil no disponible; se intenta SRI.');
            }
        }
        // Primero consultamos los clientes ya registrados en CONTSERTRIB.
        // Esto permite encontrar por cédula a una persona aunque no tenga RUC.
        const { data: clienteLocal } = await supabase
            .from('clientes')
            .select('razon_social,email,telefono,direccion')
            .eq('emisor_id', emisorId)
        ...
        if (clienteLocal) {
            return reply.send({
                encontrado: true,
                fuente: 'CLIENTE_LOCAL',
                datos: { razonSocial: clienteLocal.razon_social, email: clienteLocal.email, telefono: clienteLocal.telefono, direccion: clienteLocal.direccion },
            });
        }
        ...
        try {
            ...
            if (!r.ok)
                return reply.send({ encontrado: false, fuente: 'SRI', mensaje: `SRI respondió HTTP ${r.status}.` });
            const mapped = mapearSRI(await r.json());
            if (mapped)
                return reply.send({ encontrado: true, fuente: 'SRI', datos: mapped, identificacionValida: true });
            if (tipo === '05')
                return reply.send({ encontrado: false, fuente: 'SRI', identificacionValida: true, mensaje: 'La cédula no tiene un RUC consultable en el catastro del SRI.' });
            return reply.send({ encontrado: false, fuente: 'SRI', identificacionValida: false, mensaje: 'No se encontró el RUC en el catastro del SRI.' });
        }
        catch (e) {
    ...
    app.patch('/clientes/:id', async (request, reply) => {
        ...
        if (b.email !== undefined) {
            ...
            cambios.email = b.email.trim().toLowerCase();
        }
