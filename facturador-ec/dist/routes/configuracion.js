...
export async function registrarRutasConfiguracion(app) {
    app.get('/configuracion/datos', async (request, reply) => {
        ...
        const [{ data: emisor }, { data: config }, { data: usuarios }] = await Promise.all([
            supabase.from('emisores').select('id,ruc,razon_social,nombre_comercial,direccion_matriz,ambiente,obligado_contabilidad,actividad_turistica,registro_turismo,luaf_vigente,numero_registro_turismo').eq('id', emisorId).single(),
    ...
    app.patch('/configuracion/guardar', async (request, reply) => {
        ...
        if (b.actividadTuristica !== undefined)
            cambios.actividad_turistica = Boolean(b.actividadTuristica);
        if (b.registroTurismo !== undefined)
            cambios.registro_turismo = Boolean(b.registroTurismo);
        if (b.luafVigente !== undefined)
            cambios.luaf_vigente = Boolean(b.luafVigente);
        if (b.numeroRegistroTurismo !== undefined)
            cambios.numero_registro_turismo = String(b.numeroRegistroTurismo).trim() || null;
    ...
    app.get('/configuracion/establecimientos', async (request, reply) => {
        const emisorId = request.query.emisorId;
        if (!emisorId)
            return reply.status(400).send({ error: 'Falta emisorId.' });
    ...
    app.post('/configuracion/establecimientos', async (request, reply) => {
        const b = request.body ?? {};
        const codigo = String(b.codigo ?? '').replace(/\D/g, '').padStart(3, '0');
        const tipoEstablecimiento = b.tipoEstablecimiento === 'MATRIZ' ? 'MATRIZ' : 'SUCURSAL';
        if (!b.emisorId || !/^\d{3}$/.test(codigo) || !String(b.direccion ?? '').trim())
            return reply.status(400).send({ error: 'Código de establecimiento de 3 dígitos y dirección son obligatorios.' });
        const plan = await obtenerPlanSaas(b.emisorId);
        const usadosEst = await contarEstablecimientos(b.emisorId);
        if (plan && usadosEst >= Number(plan.max_establecimientos ?? 9999))
            return reply.status(409).send({ error: `El plan ${plan.nombre} permite hasta ${plan.max_establecimientos} establecimientos activos.` });
        if (tipoEstablecimiento === 'MATRIZ') {
            const { data: matriz } = await supabase.from('establecimientos_emisor').select('id').eq('emisor_id', b.emisorId).eq('tipo_establecimiento', 'MATRIZ').maybeSingle();
            if (matriz)
                return reply.status(409).send({ error: 'Este RUC ya tiene una matriz. Use tipo Sucursal para agregar otro establecimiento.' });
        }
    ...
    app.patch('/configuracion/establecimientos/:id', async (request, reply) => {
        const b = request.body ?? {};
        const cambios = {};
        if (b.nombreComercial !== undefined)
            cambios.nombre_comercial = String(b.nombreComercial).trim() || null;
        if (b.direccion !== undefined)
            cambios.direccion = String(b.direccion).trim();
        if (b.tipoEstablecimiento !== undefined) {
            const tipo = b.tipoEstablecimiento === 'MATRIZ' ? 'MATRIZ' : 'SUCURSAL';
            if (tipo === 'MATRIZ') {
                const { data: otra } = await supabase.from('establecimientos_emisor').select('id').eq('emisor_id', (await supabase.from('establecimientos_emisor').select('emisor_id').eq('id', request.params.id).single()).data?.emisor_id || '').eq('tipo_establecimiento', 'MATRIZ').neq('id', request.params.id).maybeSingle();
                if (otra)
                    return reply.status(409).send({ error: 'El RUC ya tiene otra matriz.' });
            }
            cambios.tipo_establecimiento = tipo;
        }
        if (b.activo !== undefined)
            cambios.activo = !!b.activo;
        if (!Object.keys(cambios).length)
            return reply.status(400).send({ error: 'No se enviaron cambios.' });
        ...
        if (!estActual)
            return reply.status(404).send({ error: 'Establecimiento no encontrado.' });
        if (b.activo === true && !estActual.activo) {
            const plan = await obtenerPlanSaas(estActual.emisor_id);
            const usados = await contarEstablecimientos(estActual.emisor_id);
            if (plan && usados >= Number(plan.max_establecimientos ?? 9999))
                return reply.status(409).send({ error: `El plan ${plan.nombre} permite hasta ${plan.max_establecimientos} establecimientos activos.` });
        }
        ...
        if (error)
            return reply.status(500).send({ error: error.message });
        if (b.activo === false)
            await supabase.from('puntos_emision').update({ activo: false }).eq('emisor_id', estActual.emisor_id).eq('establecimiento', estActual.codigo);
        return reply.send({ ok: true });
    });
    app.delete('/configuracion/establecimientos/:id', async (request, reply) => {
        ...
        if ((pts ?? []).length) {
            await supabase.from('establecimientos_emisor').update({ activo: false }).eq('id', est.id);
            await supabase.from('puntos_emision').update({ activo: false }).eq('emisor_id', est.emisor_id).eq('establecimiento', est.codigo);
            return reply.send({ ok: true, modo: 'desactivado', mensaje: 'El establecimiento tiene puntos de emisión y/o historial. Se desactivó para conservar la trazabilidad.' });
        }
        const { error } = await supabase.from('establecimientos_emisor').delete().eq('id', est.id);
        if (error)
            return reply.status(500).send({ error: error.message });
        return reply.send({ ok: true, modo: 'eliminado', mensaje: 'Establecimiento eliminado.' });
    });
    app.get('/configuracion/puntos-emision', async (request, reply) => {
        const { emisorId, establecimiento } = request.query;
        if (!emisorId)
            return reply.status(400).send({ error: 'Falta emisorId.' });
        let q = supabase.from('puntos_emision').select('*').eq('emisor_id', emisorId).order('establecimiento').order('punto_emision');
        if (establecimiento)
            q = q.eq('establecimiento', establecimiento);
        const { data, error } = await q;
        if (error)
            return reply.status(500).send({ error: error.message });
        return reply.send({ items: data ?? [] });
    });
    app.post('/configuracion/puntos-emision', async (request, reply) => {
        const b = request.body ?? {};
        ...
        if (!b.emisorId || !/^\d{3}$/.test(establecimiento) || !/^\d{3}$/.test(puntoEmision))
        ...
        if (plan && usadosPuntos >= Number(plan.max_puntos_emision ?? 9999))
            return reply.status(409).send({ error: `El plan ${plan.nombre} permite hasta ${plan.max_puntos_emision} puntos de emisión activos.` });
        ...
        if (!est)
            return reply.status(400).send({ error: 'Primero registra el establecimiento en Configuración.' });
        if (est.activo === false)
            return reply.status(400).send({ error: 'El establecimiento está inactivo.' });
    ...
    app.patch('/configuracion/puntos-emision/:id', async (request, reply) => {
        const b = request.body ?? {};
        const { data: punto } = await supabase.from('puntos_emision').select('id,emisor_id,establecimiento').eq('id', request.params.id).maybeSingle();
        if (!punto)
            return reply.status(404).send({ error: 'Punto de emisión no encontrado.' });
        ...
        if (b.activo !== undefined)
            cambios.activo = !!b.activo;
        if (b.activo === true) {
            const { data: actual } = await supabase.from('puntos_emision').select('activo,emisor_id').eq('id', request.params.id).maybeSingle();
            if (actual && !actual.activo) {
                const plan = await obtenerPlanSaas(actual.emisor_id);
                const usados = await contarPuntos(actual.emisor_id);
                if (plan && usados >= Number(plan.max_puntos_emision ?? 9999))
                    return reply.status(409).send({ error: `El plan ${plan.nombre} permite hasta ${plan.max_puntos_emision} puntos de emisión activos.` });
            }
        }
        const { error } = await supabase.from('puntos_emision').update(cambios).eq('id', request.params.id);
        if (error)
            return reply.status(500).send({ error: error.message });
        return reply.send({ ok: true });
    });
    app.delete('/configuracion/puntos-emision/:id', async (request, reply) => {
        ...
        if (Number(count ?? 0) > 0) {
            const { error } = await supabase.from('puntos_emision').update({ activo: false }).eq('id', request.params.id);
            if (error)
                return reply.status(500).send({ error: error.message });
            return reply.send({ ok: true, modo: 'desactivado', mensaje: 'El punto tiene comprobantes históricos. Se desactivó para conservar la trazabilidad tributaria.' });
        }
        const { error } = await supabase.from('puntos_emision').delete().eq('id', request.params.id);
        if (error)
            return reply.status(500).send({ error: error.message });
        return reply.send({ ok: true, modo: 'eliminado', mensaje: 'Punto de emisión eliminado.' });
    });
    app.get('/notificaciones', async (request, reply) => {
        ...
        for (const e of emailRows.filter(x => emailPermitidos.has(x.comprobante_id))) {
    ...
    app.post('/notificaciones/leer-todas', async (request, reply) => {
        ...
        let emailOk = new Set();
        if (emailIds.length) {
            const { data: ec } = await supabase.from('comprobantes').select('id').eq('emisor_id', emisorId).in('id', emailIds);
            emailOk = new Set((ec ?? []).map(x => x.id));
        }
        for (const e of (correos.data ?? []).filter(x => emailOk.has(x.comprobante_id)))
