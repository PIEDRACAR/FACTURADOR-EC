...
export async function registrarRutasConfiguracion(app) {
    ...
    app.patch('/configuracion/guardar', async (request, reply) => {
        ...
        if (!b.emisorId)
            return reply.status(400).send({ error: 'Falta emisorId.' });
        if (b.rucProveedorFacturacion && !RUC_RE.test(b.rucProveedorFacturacion))
            return reply.status(400).send({ error: 'El RUC del proveedor de facturación debe tener 13 dígitos.' });
        const cambios = {};
    ...
    app.get('/notificaciones', async (request, reply) => {
        ...
        for (const p of stock.data ?? []) {
            ...
            if (actual <= crit)
                items.push({ tipo: 'inventario', prioridad: 'alta', titulo: `STOP: ${p.nombre}`, detalle: `Stock ${actual}. Nivel crítico ${crit}.` });
            else if (actual <= min)
                items.push({ tipo: 'inventario', prioridad: 'media', titulo: `Stock bajo: ${p.nombre}`, detalle: `Stock ${actual}. Mínimo ${min}.` });
        }
        for (const r of rechazados.data ?? [])
            items.push({ tipo: 'sri', prioridad: 'alta', titulo: `Comprobante rechazado ${r.secuencial ?? ''}`.trim(), detalle: r.motivo_error ?? 'Revisar respuesta del SRI.', fecha: r.created_at });
        for (const e of correos.data ?? [])
            items.push({ tipo: 'correo', prioridad: 'media', titulo: 'Correo no enviado', detalle: e.detalle ?? `No se pudo enviar a ${e.destinatario}.`, fecha: e.created_at });
