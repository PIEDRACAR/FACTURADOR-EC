...
export async function registrarRutasAuth(app) {
    ...
    app.get('/auth/permisos', async (request, reply) => {
        ...
        const rolActual = await (async () => { const { data } = await supabase.from('usuarios_emisor').select('rol').eq('user_id', sesion.userId).eq('emisor_id', emisorId).maybeSingle(); return data?.rol; })();
        if (rolActual !== 'admin')
            return reply.status(403).send({ error: 'Solo el administrador puede administrar permisos.' });
        const { data: objetivo } = await supabase.from('usuarios_emisor').select('rol').eq('user_id', userId).eq('emisor_id', emisorId).maybeSingle();
        if (!objetivo)
            return reply.status(404).send({ error: 'El usuario no pertenece a este negocio.' });
    ...
    app.patch('/auth/usuarios/permisos', async (request, reply) => {
        const { emisorId, userId, permisos } = request.body ?? {};
        if (!emisorId || !userId || !Array.isArray(permisos))
            return reply.status(400).send({ error: 'Faltan emisorId, userId o permisos.' });
        const token = request.cookies?.[NOMBRE_COOKIE];
        const sesion = token ? await obtenerSesion(token) : null;
        if (!sesion)
            return reply.status(401).send({ error: 'Sesión no válida.' });
        ...
        if (objetivo.rol === 'admin')
            return reply.status(400).send({ error: 'El administrador conserva acceso total y no necesita permisos individuales.' });
        try {
            await guardarPermisosUsuario(userId, emisorId, permisos);
            return reply.send({ ok: true, permisos: [...new Set(permisos)] });
        }
        catch (err) {
            return reply.status(500).send({ error: err instanceof Error ? err.message : 'No se pudieron guardar los permisos.' });
        }
    });
