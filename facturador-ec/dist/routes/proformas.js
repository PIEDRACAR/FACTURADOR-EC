...
import { resolverIva, perfilDesdeTarifa, porcentajeDecimal, codigoSRI } from '../services/impuestos.js';
...
async function generarNumeroProforma(emisorId) {
...
export async function registrarRutasProformas(app) {
    /** Crea una proforma (cotización), resolviendo precios reales del catálogo. */
    app.post('/proformas', async (request, reply) => {
        ...
        if (!body.fechaValidez)
            return reply.status(400).send({ error: 'Falta fechaValidez.' });
        const idsProductos = body.items.map((i) => i.productoId).filter((id) => !!id);
        const productosPorId = new Map();
        if (idsProductos.length > 0) {
            const { data: productos, error: errorProductos } = await supabase
                .from('productos')
                .select('id, descripcion, precio_venta, tarifa_iva, perfil_iva')
        ...
        for (const item of body.items) {
            ...
            if (item.productoId) {
                ...
                perfilIva = producto.perfil_iva || perfilDesdeTarifa(tarifaIva);
            ...
            const regla = await resolverIva({ emisorId: body.emisorId, fecha: fechaEmisionEcuador(), perfil: perfilIva });
            const iva = redondear(base * porcentajeDecimal(regla));
    ...
    app.get('/proformas/listado', async (request, reply) => {
    ...
    app.get('/proformas/:id', async (request, reply) => {
        const { data, error } = await supabase
            .from('proformas')
            ...
            .single();
        if (error || !data)
            return reply.status(404).send({ error: 'Proforma no encontrada.' });
        return reply.send(data);
    });
    /** Convierte una proforma vigente en una venta real: crea el comprobante (con inventario) y lo emite ante el SRI. */
    app.post('/proformas/:id/convertir', async (request, reply) => {
        const { data: proforma, error: errorProforma } = await supabase
            .from('proformas')
            .select('*, clientes(*), proforma_items(*)')
            .eq('id', request.params.id)
        ...
        const idsProductos = proforma.proforma_items
            .map((i) => i.producto_id)
            .filter((id) => !!id);
        const tarifasPorProducto = new Map();
        const perfilesPorProducto = new Map();
        const codigosPorProducto = new Map();
        if (idsProductos.length > 0) {
            const { data: productos } = await supabase.from('productos').select('id, codigo_principal, tarifa_iva, perfil_iva').in('id', idsProductos);
            for (const p of productos ?? []) {
                tarifasPorProducto.set(p.id, p.tarifa_iva);
                perfilesPorProducto.set(p.id, p.perfil_iva || perfilDesdeTarifa(p.tarifa_iva));
        ...
        for (const item of proforma.proforma_items) {
            ...
            const perfilIva = item.producto_id ? perfilesPorProducto.get(item.producto_id) ?? perfilDesdeTarifa(tarifaIva) : 'GENERAL';
            const base = redondear(item.cantidad * item.precio_unitario - item.descuento);
            const regla = await resolverIva({ emisorId: proforma.emisor_id, fecha: fechaEmisionEcuador(), perfil: perfilIva });
            const tarifaAplicada = Number(regla.porcentaje);
            const codigoPorcentaje = codigoSRI(regla);
            const porcentaje = porcentajeDecimal(regla);
            const valorIva = redondear(base * porcentaje);
            if (regla.tipo === 'GENERAL')
                subtotal15 = redondear(subtotal15 + base);
            else if (tarifaAplicada === 8)
                subtotal8 = redondear(subtotal8 + base);
            else if (tarifaAplicada === 5)
                subtotal5 = redondear(subtotal5 + base);
            else
                subtotal0 = redondear(subtotal0 + base);
            ...
            detallesFactura.push({
                ...
                impuestos: [{ codigo: '2', codigoPorcentaje, tarifa: tarifaAplicada, baseImponible: base, valor: valorIva }],
            });
        }
        ...
        const cliente = proforma.clientes;
        const impuestosAgrupados = new Map();
        for (const d of detallesFactura)
            for (const imp of d.impuestos ?? []) {
                const key = `${imp.codigo}:${imp.codigoPorcentaje}:${imp.tarifa}`;
                const x = impuestosAgrupados.get(key) ?? { codigo: String(imp.codigo), codigoPorcentaje: String(imp.codigoPorcentaje), tarifa: Number(imp.tarifa), base: 0, valor: 0 };
                x.base = redondear(x.base + Number(imp.baseImponible));
                x.valor = redondear(x.valor + Number(imp.valor));
                impuestosAgrupados.set(key, x);
            }
        const totalConImpuestos = [...impuestosAgrupados.values()].map(x => ({ codigo: x.codigo, codigoPorcentaje: x.codigoPorcentaje, tarifa: x.tarifa, baseImponible: x.base, valor: x.valor }));
        const fechaEcuador = fechaEmisionEcuador();
        const [yy, mm, dd] = fechaEcuador.split('-');
        const fechaEmision = `${dd}/${mm}/${yy}`;
        const facturaData = {
            fechaEmision,
        ...
        try {
            const resultado = await emitirFactura({ emisorId: proforma.emisor_id, comprobanteId: comprobanteId, facturaData });
            await supabase
                .from('proformas')
...
//# sourceMappingURL=proformas.js.map