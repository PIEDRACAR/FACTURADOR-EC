...
import { resolverIva, perfilDesdeTarifa, porcentajeDecimal, codigoSRI, redondearImpuesto } from '../services/impuestos.js';
/**
...
export async function registrarRutasPos(app) {
    ...
    app.get('/productos', async (request, reply) => {
        ...
        let consulta = supabase
            .from('productos')
            .select('id, codigo_principal, codigo_auxiliar, descripcion, unidad_medida, precio_venta, costo_promedio, tarifa_iva, perfil_iva, stock_actual, stock_critico, stock_minimo, stock_maximo, activo')
    ...
    app.post('/productos', async (request, reply) => {
        ...
        const faltantes = [];
        if (!b?.emisorId)
            faltantes.push('emisorId');
        if (!b?.codigoPrincipal)
            faltantes.push('codigoPrincipal');
        if (!b?.descripcion)
            faltantes.push('descripcion');
        if (b?.precioVenta === undefined || b.precioVenta < 0)
            faltantes.push('precioVenta');
        if (!b?.tarifaIva)
            faltantes.push('tarifaIva');
        ...
        const { data, error } = await supabase
            ...
            perfil_iva: b.perfilIva || perfilDesdeTarifa(b.tarifaIva),
    ...
    app.patch('/productos/:id', async (request, reply) => {
        ...
        if (b.descripcion !== undefined)
            cambios.descripcion = b.descripcion;
        if (b.codigoAuxiliar !== undefined)
            cambios.codigo_auxiliar = b.codigoAuxiliar || null;
        if (b.unidadMedida !== undefined)
            cambios.unidad_medida = b.unidadMedida;
        if (b.precioVenta !== undefined)
            cambios.precio_venta = b.precioVenta;
        if (b.costoPromedio !== undefined)
            cambios.costo_promedio = b.costoPromedio;
        if (b.tarifaIva !== undefined)
            cambios.tarifa_iva = b.tarifaIva;
        if (b.perfilIva !== undefined)
            cambios.perfil_iva = b.perfilIva;
        if (b.stockActual !== undefined)
            cambios.stock_actual = b.stockActual;
        if (b.stockCritico !== undefined)
            cambios.stock_critico = b.stockCritico;
        if (b.stockMinimo !== undefined)
            cambios.stock_minimo = b.stockMinimo;
        if (b.stockMaximo !== undefined)
            cambios.stock_maximo = b.stockMaximo;
    ...
    app.get('/pos/puntos-emision', async (request, reply) => {
    ...
    app.post('/pos/venta', async (request, reply) => {
        ...
        if (!consumo.ok)
            return reply.status(402).send({ error: consumo.mensaje, consumo: consumo.usados, limite: consumo.limite, plan: consumo.plan });
        let puntoEmision;
        try {
        ...
        const idsProductos = body.items.map((i) => i.productoId).filter((id) => !!id);
        const productosPorId = new Map();
        if (idsProductos.length > 0) {
            const { data: productos, error: errorProductos } = await supabase
                .from('productos')
                .select('id, codigo_principal, descripcion, precio_venta, tarifa_iva, perfil_iva, costo_promedio, stock_actual')
        ...
        for (const item of body.items) {
            ...
            if (item.productoId) {
                ...
                perfilIva = producto.perfil_iva || perfilDesdeTarifa(producto.tarifa_iva);
                ...
                perfilIva = item.perfilIvaLibre || perfilDesdeTarifa(tarifaIva);
                costoUnitario = 0;
            }
            const precioTotalSinImpuesto = redondear(item.cantidad * precioUnitario - descuento);
            const fechaFiscal = fechaEmisionEcuador();
            const reglaIva = await resolverIva({ emisorId: body.emisorId, fecha: fechaFiscal, perfil: perfilIva });
            const tarifaAplicada = Number(reglaIva.porcentaje);
            const codigoPorcentaje = codigoSRI(reglaIva);
            const porcentajeIva = porcentajeDecimal(reglaIva);
            const valorIva = redondearImpuesto(precioTotalSinImpuesto * porcentajeIva);
            if (reglaIva.tipo === 'GENERAL')
                subtotal15 = redondear(subtotal15 + precioTotalSinImpuesto);
            else if (tarifaAplicada === 8)
                subtotal8 = redondear(subtotal8 + precioTotalSinImpuesto);
            else if (tarifaAplicada === 5)
                subtotal5 = redondear(subtotal5 + precioTotalSinImpuesto);
            else
                subtotal0 = redondear(subtotal0 + precioTotalSinImpuesto);
            ...
            itemsParaGuardar.push({
                ...
                tarifa_iva: String(tarifaAplicada),
                valor_iva: valorIva,
            });
        ...
        const comprobante = { id: comprobanteId };
        ...
        for (const d of detallesFactura) {
            for (const imp of d.impuestos ?? []) {
                const key = `${imp.codigo}:${imp.codigoPorcentaje}:${Number(imp.tarifa).toFixed(3)}`;
                const actual = impuestosAgrupados.get(key) ?? { codigo: String(imp.codigo), tarifa: Number(imp.tarifa), base: 0, valor: 0 };
                actual.base = redondear(actual.base + Number(imp.baseImponible));
                actual.valor = redondear(actual.valor + Number(imp.valor));
                impuestosAgrupados.set(key, actual);
            }
        }
        const totalConImpuestos = [...impuestosAgrupados.entries()].map(([key, x]) => {
            const codigoPorcentaje = key.split(':')[1];
            return { codigo: x.codigo, codigoPorcentaje, tarifa: x.tarifa, baseImponible: x.base, valor: x.valor };
        });
        ...
        try {
            ...
            if (resultado.estado === 'AUTORIZADO') {
                try {
                    ...
                    const xml = resultado.xmlFirmado ?? '';
