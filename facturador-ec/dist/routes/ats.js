...
function validarRuc(ruc) { return /^\d{13}$/.test(ruc) && ruc.endsWith('001'); }
export async function registrarRutasAts(app) {
    app.get('/ats/resumen', async (request, reply) => {
        ...
        if (!feature.ok)
            return reply.status(402).send({ error: feature.mensaje, plan: feature.plan?.nombre });
        const anio = Number(request.query.anio);
        const mes = Number(request.query.mes);
        if (!Number.isInteger(anio) || !Number.isInteger(mes))
            return reply.status(400).send({ error: 'Indica año y mes.' });
    ...
    app.post('/ats/compras', async (request, reply) => {
        const b = request.body ?? {};
        if (!b.emisorId || !b.idProv || !b.secuencial || !b.fechaEmision)
            return reply.status(400).send({ error: 'Emisor, proveedor, secuencial y fecha son obligatorios.' });
        const feature = await comprobarCaracteristica(b.emisorId, 'ats');
        if (!feature.ok)
            return reply.status(402).send({ error: feature.mensaje });
        const idProv = String(b.idProv).trim();
        if (idProv.length < 3 || idProv.length > 13)
            return reply.status(400).send({ error: 'La identificación del proveedor debe tener entre 3 y 13 caracteres.' });
        const bases = Number(b.baseNoObjeto || 0) + Number(b.baseIva0 || 0) + Number(b.baseIvaDiferente0 || 0) + Number(b.baseExenta || 0);
        if (bases <= 0)
            return reply.status(400).send({ error: 'Debe existir al menos una base imponible mayor a cero.' });
    ...
    app.delete('/ats/compras/:id', async (request, reply) => {
        const { error } = await supabase.from('ats_compras').delete().eq('id', request.params.id);
        if (error)
            return reply.status(500).send({ error: error.message });
        return reply.send({ ok: true });
    });
    app.post('/ats/generar', async (request, reply) => {
        const { emisorId, anio, mes } = request.body ?? {};
        if (!emisorId || !anio || !mes)
            return reply.status(400).send({ error: 'Emisor, año y mes son obligatorios.' });
        try {
            return reply.send(await generarAts(emisorId, Number(anio), Number(mes)));
        }
        catch (e) {
            return reply.status(400).send({ error: e instanceof Error ? e.message : String(e) });
        }
    });
    app.get('/ats/generaciones/:id/xml', async (request, reply) => {
        const { data, error } = await supabase.from('ats_generaciones').select('xml,emisor_id,anio,mes').eq('id', request.params.id).single();
        if (error || !data)
            return reply.status(404).send({ error: 'Generación ATS no encontrada.' });
