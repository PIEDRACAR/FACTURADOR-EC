...
import { crearOEncontrarUsuario, agregarUsuarioANegocio, crearSesion } from '../auth/sesiones.js';
const RUC_REGEX = /^\d{13}$/;
export async function registrarRutasEmisores(app) {
    /**
     ...
     */
    app.post('/emisores/registrar', async (request, reply) => {
        const body = request.body;
        const faltantes = [];
        if (!body?.ruc)
            faltantes.push('ruc');
        if (!body?.razonSocial)
            faltantes.push('razonSocial');
        if (!body?.direccionMatriz)
            faltantes.push('direccionMatriz');
        if (typeof body?.obligadoContabilidad !== 'boolean')
            faltantes.push('obligadoContabilidad');
        if (!body?.ambiente)
            faltantes.push('ambiente');
        if (!body?.p12Base64)
            faltantes.push('p12Base64');
        if (!body?.p12Password)
            faltantes.push('p12Password');
        if (!body?.fechaExpiracionCertificado)
            faltantes.push('fechaExpiracionCertificado');
        if (!body?.emailAdmin)
            faltantes.push('emailAdmin');
        if (!body?.passwordAdmin)
            faltantes.push('passwordAdmin');
        if (faltantes.length > 0) {
            return reply.status(400).send({ error: `Faltan campos obligatorios: ${faltantes.join(', ')}` });
        }
        if (body.passwordAdmin.length < 6) {
        ...
        // 4) Usuario administrador — se crea (o reutiliza, si ya existe con ese
        // correo en otro negocio) y se le da acceso total a este negocio.
        let sesionCreada = false;
        try {
            const userId = await crearOEncontrarUsuario(body.emailAdmin, body.passwordAdmin);
            await agregarUsuarioANegocio(userId, emisorId, 'admin');
            const token = await crearSesion(userId);
            reply.setCookie('sesion', token, { path: '/', httpOnly: true, sameSite: 'lax', maxAge: 60 * 60 * 24 * 30 });
            sesionCreada = true;
        }
        catch (errUsuario) {
            request.log.error(errUsuario);
            // El negocio y el certificado ya quedaron bien — no se revierte nada,
            // solo se avisa que falta resolver el usuario a mano si esto falla.
        }
