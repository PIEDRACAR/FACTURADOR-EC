 ...
 * de cuándo se cobra.
 */
export async function registrarRutasCuentas(app) {
    // ========== CUENTAS POR PAGAR ==========
    app.get('/cuentas-por-pagar/listado', async (request, reply) => {
    ...
    app.post('/cuentas-por-pagar', async (request, reply) => {
        ...
        const { data, error } = await supabase
            ...
            .single();
        if (error || !data)
            return reply.status(500).send({ error: error?.message ?? 'No se pudo crear la cuenta.' });
        return reply.status(201).send({ id: data.id });
    });
    app.post('/cuentas-por-pagar/:id/pagos', async (request, reply) => {
        const { monto, formaPagoCodigo, nota } = request.body ?? {};
        if (!monto || monto <= 0)
            return reply.status(400).send({ error: 'Indica un monto de pago válido.' });
        ...
        if (error || !data?.[0]) {
            ...
            if (mensaje.includes('cuenta_no_encontrada'))
                return reply.status(404).send({ error: 'Cuenta no encontrada.' });
            if (mensaje.includes('monto_excede_saldo'))
                return reply.status(400).send({ error: 'El pago supera el saldo pendiente de esta cuenta.' });
    ...
    // ========== CUENTAS POR COBRAR ==========
    app.get('/cuentas-por-cobrar/listado', async (request, reply) => {
        const { emisorId, estado } = request.query;
        if (!emisorId)
            return reply.status(400).send({ error: 'Falta el parámetro emisorId.' });
        let consulta = supabase
    ...
    app.post('/cuentas-por-cobrar', async (request, reply) => {
        const b = request.body;
    ...
    app.post('/cuentas-por-cobrar/:id/pagos', async (request, reply) => {
        const { monto, formaPagoCodigo, nota } = request.body ?? {};
        if (!monto || monto <= 0)
        ...
        if (error || !data?.[0]) {
            ...
            if (mensaje.includes('cuenta_no_encontrada'))
                return reply.status(404).send({ error: 'Cuenta no encontrada.' });
            if (mensaje.includes('monto_excede_saldo'))
                return reply.status(400).send({ error: 'El cobro supera el saldo pendiente de esta cuenta.' });
