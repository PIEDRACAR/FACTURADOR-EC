...
async function obtenerListadoVentas(emisorId, desde, hasta) {
    ...
    const ventas = (data ?? []).map((v) => ({
        ...
        identificacion: v.clientes?.identificacion ?? '',
...
export async function registrarRutasReportes(app) {
    app.get('/reportes/caja', async (request, reply) => {
        ...
        try {
            ...
            if (desde)
                pagosQ = pagosQ.gte('created_at', desde);
            if (hasta)
                pagosQ = pagosQ.lte('created_at', hasta + 'T23:59:59');
            ...
            for (const c of pagos ?? [])
                for (const p of c.comprobante_formas_pago ?? []) {
                    ...
                    if (p.forma_pago_codigo === '01')
                        resumen.ventasEfectivo += v;
                    else
                        resumen.ventasOtros += v;
                }
            for (const m of movimientos) {
                const v = Number(m.monto) || 0;
                if (m.tipo === 'ingreso')
                    resumen.ingresosManuales += v;
                else
                    resumen.egresosManuales += v;
            }
            const filas = (cajas ?? []).map((c) => {
                ...
                for (const v of ventas)
                    for (const p of v.comprobante_formas_pago ?? [])
                        if (p.forma_pago_codigo === '01')
                            ventasEfectivo += Number(p.valor) || 0;
    ...
    app.get('/reportes/auditoria', async (request, reply) => {
        const { emisorId, desde, hasta } = request.query;
        if (!emisorId)
            return reply.status(400).send({ error: 'Falta el parámetro emisorId.' });
        try {
            ...
            const muestrasConRuc = (muestras ?? []).filter((x) => /RUC Proveedor/i.test(String(x.xml_firmado || '')) && rucProveedor && String(x.xml_firmado).includes(rucProveedor)).length;
            ...
            switch (tipo) {
                ...
                case 'caja': {
                    ...
                    const filas = (q.data ?? []).map((x) => {
                        ...
                        const ventasEfectivo = (pagos ?? []).filter(v => { const t = new Date(v.created_at).getTime(); return t >= inicio && t <= fin; }).reduce((a, v) => a + (v.comprobante_formas_pago ?? []).filter((p) => p.forma_pago_codigo === '01').reduce((s, p) => s + (Number(p.valor) || 0), 0), 0);
