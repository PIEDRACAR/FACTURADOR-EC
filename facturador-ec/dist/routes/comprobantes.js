...
export async function registrarRutasComprobantes(app) {
    ...
    app.post('/comprobantes/factura/emitir', async (request, reply) => {
        ...
        try {
            // La tarifa tributaria debe coincidir con la que quedó congelada en los
            // ítems del comprobante. Así una llamada directa a este endpoint no puede
            // cambiar el IVA de una venta ya preparada por el POS.
            const { data: items, error: errorItems } = await supabase
                .from('comprobante_items')
                .select('tarifa_iva,valor_iva,precio_total_sin_impuesto')
                .eq('comprobante_id', comprobanteId);
            if (errorItems)
                return reply.status(500).send({ error: errorItems.message });
            if (items?.length && facturaData.detalles?.length === items.length) {
                for (let i = 0; i < items.length; i++) {
                    const detalle = facturaData.detalles[i];
                    const impuesto = Array.isArray(detalle.impuestos) ? detalle.impuestos[0] : null;
                    const tarifa = Number(items[i].tarifa_iva);
                    if (impuesto && Number(impuesto.tarifa) !== tarifa) {
                        return reply.status(409).send({ error: `La tarifa IVA de la línea ${i + 1} no coincide con la tarifa tributaria congelada del comprobante (${tarifa}%).` });
                    }
                }
            }
     ...
     */
    app.post('/comprobantes/:id/anulacion', async (request, reply) => {
        const motivo = String(request.body?.motivo ?? '').trim();
        const emisorIdBody = String(request.body?.emisorId ?? '').trim();
        if (motivo.length < 5)
            return reply.status(400).send({ error: 'El motivo de anulación es obligatorio (mínimo 5 caracteres).' });
        ...
        if (emisorIdBody && emisorIdBody !== comprobante.emisor_id)
            return reply.status(403).send({ error: 'El comprobante no pertenece al negocio seleccionado.' });
        if (comprobante.estado !== 'autorizado')
            return reply.status(409).send({ error: 'Solo se puede solicitar la anulación de un comprobante autorizado.' });
        if (!comprobante.clave_acceso)
            return reply.status(409).send({ error: 'El comprobante no tiene clave de acceso.' });
        ...
        const fechaLocalEcuador = (fecha) => new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Guayaquil', year: 'numeric', month: '2-digit', day: '2-digit' }).format(fecha);
        ...
        const feriadosEcuador = (anio) => {
            ...
            const agregarDias = (base, dias) => {
                const d2 = new Date(base);
                d2.setUTCDate(d2.getUTCDate() + dias);
        ...
        const esDiaNoLaborable = (anio, mes, dia) => {
        ...
        let diaLimite = 7;
        while (esDiaNoLaborable(anioSiguiente, mesSiguiente, diaLimite))
            diaLimite++;
        ...
        const { data: existente } = await supabase
            ...
            .maybeSingle();
        if (existente)
            return reply.status(409).send({ error: existente.estado === 'anulado' ? 'La factura ya figura como anulada en CONTSERTRIB.' : 'Ya existe una solicitud de anulación pendiente para esta factura.', solicitud: existente });
        ...
        if (insertError || !solicitud)
            return reply.status(500).send({ error: insertError?.message || 'No se pudo registrar la solicitud de anulación.' });
    ...
    app.get('/comprobantes/:id/anulacion', async (request, reply) => {
    ...
    app.post('/comprobantes/:id/anulacion/confirmar', async (request, reply) => {
        const confirmacion = String(request.body?.confirmacion ?? '').trim();
        if (confirmacion !== 'ANULADO')
            return reply.status(400).send({ error: 'Para confirmar debes escribir exactamente ANULADO.' });
        ...
        if (se || !solicitud)
            return reply.status(409).send({ error: 'No existe una solicitud de anulación pendiente para esta factura.' });
        if (comprobante.estado === 'anulado')
            return reply.send({ ok: true, estado: 'anulado', mensaje: 'La factura ya estaba marcada como anulada.' });
        if (comprobante.estado !== 'autorizado')
            return reply.status(409).send({ error: 'El comprobante no está en estado autorizado.' });
        ...
        if (ue)
            return reply.status(500).send({ error: ue.message });
        ...
        if (us)
            return reply.status(500).send({ error: us.message });
    ...
    app.post('/comprobantes/:id/reenviar-email', async (request, reply) => {
        ...
        if (emisorIdBody && emisorIdBody !== comprobante.emisor_id)
            return reply.status(403).send({ error: 'El comprobante no pertenece al negocio seleccionado.' });
        if (comprobante.estado !== 'autorizado')
            return reply.status(409).send({ error: 'Solo se pueden reenviar comprobantes autorizados por el SRI.' });
        if (!comprobante.xml_firmado)
            return reply.status(409).send({ error: 'La factura no tiene XML firmado guardado.' });
        const cliente = comprobante.clientes;
        const destinatario = (request.body?.email ?? cliente?.email ?? '').trim().toLowerCase();
        if (!emailValido(destinatario))
            return reply.status(400).send({ error: 'El cliente no tiene un correo válido. Puedes indicar uno para este reenvío.' });
        try {
            const ride = await generarRidePdf(comprobante.id);
            const secuencial = comprobante.secuencial ?? 'sin-secuencial';
            const emisor = comprobante.emisores;
     ...
     */
    app.get('/comprobantes/:id/ticket', async (request, reply) => {
    ...
    `).eq('id', request.params.id).single();
        if (error || !c)
            return reply.status(404).type('text/plain').send('Comprobante no encontrado.');
        if (c.estado !== 'autorizado')
            return reply.status(409).type('text/plain').send('Solo se puede reimprimir un comprobante autorizado por el SRI.');
        const emisor = c.emisores;
        const punto = c.puntos_emision;
        const cliente = c.clientes;
        const items = c.comprobante_items ?? [];
        const pagos = c.comprobante_formas_pago ?? [];
    ...
    app.get('/comprobantes/:id/email-historial', async (request, reply) => {
    ...
    app.get('/comprobantes/:id/archivos', async (request, reply) => {
        try {
            const { data: comprobante, error } = await supabase
                .from('comprobantes')
            ...
            if (error || !comprobante)
                return reply.status(404).send({ error: 'Comprobante no encontrado.' });
            if (comprobante.estado !== 'autorizado')
                return reply.status(409).send({ error: 'Solo se pueden archivar comprobantes autorizados por el SRI.' });
