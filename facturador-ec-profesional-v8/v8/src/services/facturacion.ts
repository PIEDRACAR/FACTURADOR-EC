...
async function obtenerRucProveedorSistema(emisorId: string): Promise<string | null> {
  const { data } = await supabase.from('configuracion_sistema').select('ruc_proveedor_facturacion,incluir_ruc_proveedor').eq('emisor_id', emisorId).maybeSingle();
  if (data?.incluir_ruc_proveedor && data.ruc_proveedor_facturacion) return String(data.ruc_proveedor_facturacion).trim();
  return process.env.RUC_PROVEEDOR_FACTURACION?.trim() || null;
}
...
async function emitirFacturaConCampoAdicional({
  ...
  for (let intento = 0; intento <= MAX_ERROR_70_RETRIES; intento++) {
    ...
    // RECIBIDA — esperar y consultar autorización.
    await esperar(AUTHORIZATION_DELAY_MS);
    const autorizacion = await fe.checkAuthorization(claveAcceso);
...
function construirResultadoDesdeAutorizacion(
  autorizacion: { estado: string; numeroAutorizacion: string | null; fechaAutorizacion: string | null; mensaje?: string },
  claveAcceso: string,
  secuencial: string,
  xmlOriginal: string,
  xmlFirmado: string,
...
export async function emitirFactura({
  ...
  if (!rucProveedor) {
    throw new Error('No está configurado el RUC del proveedor del sistema de facturación. Ve a Configuración → Proveedor de facturación y registra un RUC válido de 13 dígitos.');
  }
