...
export async function registrarRutasEmisores(app: FastifyInstance) {
  ...
  app.post<{ Body: SolicitudRegistroBody }>('/emisores/registrar', async (request, reply) => {
    ...
    return reply.status(201).send({
      ...
      userId,
      cuentaCreada,
      acceso: diasPrueba > 0 && cuentaCreada ? { url:`${env.appUrl}/login`, email, password } : null,
      ...
      mensaje: diasPrueba > 0
        ? (cuentaCreada ? `Cuenta creada correctamente. Tienes ${diasPrueba} días de promoción.` : 'Solicitud registrada. La promoción quedó pendiente de completar por el administrador.')
