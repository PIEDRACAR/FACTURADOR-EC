...
export async function registrarRutasCorreo(app) {
    app.post('/correo-prueba/enviar', async (request, reply) => {
        const to = request.body?.to?.trim().toLowerCase();
        if (!to || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to))
            return reply.status(400).send({ error: 'Indica un correo válido.' });
        if (!env.resendApiKey || !env.emailFrom)
            return reply.status(503).send({ error: 'Resend no está configurado en Railway. Verifica RESEND_API_KEY y EMAIL_FROM.' });
        try {
