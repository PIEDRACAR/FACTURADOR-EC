 ...
 */
export async function registrarRutasDashboard(app) {
    app.get('/dashboard/resumen', async (request, reply) => {
        ...
        const { data: ventas7Data } = await supabase
            .from('comprobantes')
        ...
        const ventasUltimos7Dias = Array.from({ length: 7 }, (_, i) => {
            const fecha = new Date(inicio7);
            fecha.setDate(inicio7.getDate() + i);
            const key = fecha.toISOString().slice(0, 10);
            const total = (ventas7Data ?? []).filter(v => new Date(v.created_at).toISOString().slice(0, 10) === key).reduce((a, v) => a + Number(v.importe_total || 0), 0);
            return { key, label: fecha.toLocaleDateString('es-EC', { weekday: 'short', day: '2-digit' }), total: Math.round(total * 100) / 100 };
        });
        ...
        if (cajaAbierta?.id) {
            ...
            for (const venta of pagosCaja ?? []) {
                for (const pago of (venta.comprobante_formas_pago ?? [])) {
                    if (pago.forma_pago_codigo === '01')
                        cajaActual += Number(pago.valor || 0);
                }
            }
            const { data: movimientos } = await supabase.from('movimientos_caja').select('tipo, monto, forma_pago').eq('caja_id', cajaAbierta.id);
            for (const m of movimientos ?? [])
                if (m.forma_pago === '01')
                    cajaActual += m.tipo === 'ingreso' ? Number(m.monto || 0) : -Number(m.monto || 0);
        }
        ...
        return reply.send({
            ...
            ventasRecientes: (ventasMesData ?? []).slice(0, 6).map(v => ({
                ...
                cliente: Array.isArray(v.clientes) ? (v.clientes[0]?.razon_social ?? 'Consumidor Final') : (v.clientes?.razon_social ?? 'Consumidor Final'),
                email: Array.isArray(v.clientes) ? (v.clientes[0]?.email ?? '') : (v.clientes?.email ?? ''),
            })),
            productosStockBajo,
            productosStop,
            productosSobrestock,
     ...
     */
    app.get('/emisores/buscar', async (request, reply) => {
        ...
        if (!data)
            return reply.status(404).send({ error: 'No se encontró ningún negocio registrado con ese RUC.' });
