...
export async function emitirDocumentoSri(emisorId: string, tipo: string, datos: Record<string, unknown>): Promise<EmissionResult> {
  ...
  if (recepcion.estado === 'DEVUELTA') {
    const mensajes = [
      ...(recepcion.mensajes ?? []),
      ...((recepcion as any).comprobantes?.comprobante?.mensajes ?? []),
    ];
    const msg = mensajes.map((m:any) => `${m.identificador ? `${m.identificador}: ` : ''}${m.mensaje ?? ''}${m.informacionAdicional ? ` — ${m.informacionAdicional}` : ''}`).filter(Boolean).join('; ');
