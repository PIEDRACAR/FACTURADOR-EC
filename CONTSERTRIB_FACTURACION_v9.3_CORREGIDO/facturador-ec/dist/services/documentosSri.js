...
export async function emitirDocumentoSri(emisorId, tipo, datos) {
    ...
    const rucProveedor = (conf?.incluir_ruc_proveedor && conf?.ruc_proveedor_facturacion) ? String(conf.ruc_proveedor_facturacion).trim() : (process.env.RUC_PROVEEDOR_FACTURACION?.trim() ?? '');
    ...
    if (recepcion.estado === 'DEVUELTA') {
        const msg = (recepcion.mensajes ?? []).map((m) => `${m.identificador}: ${m.mensaje}${m.informacionAdicional ? ` — ${m.informacionAdicional}` : ''}`).join('; ');
        return { estado: 'DEVUELTA', ambiente: ambiente === '2' ? 'produccion' : 'pruebas', claveAcceso: clave, secuencial, xmlOriginal: xml, xmlFirmado: firmado, numeroAutorizacion: null, fechaAutorizacion: null, mensaje: msg };
    }
    await new Promise(r => setTimeout(r, 1500));
    const auth = await fe.checkAuthorization(clave);
    const authAny = auth;
    const mensajeAutorizacion = authAny.mensaje ?? (authAny.mensajes ?? []).map((m) => `${m.identificador ?? ''}: ${m.mensaje ?? ''}${m.informacionAdicional ? ` — ${m.informacionAdicional}` : ''}`).join('; ');
    return { estado: auth.estado, ambiente: ambiente === '2' ? 'produccion' : 'pruebas', claveAcceso: clave, secuencial, xmlOriginal: xml, xmlFirmado: firmado, numeroAutorizacion: auth.numeroAutorizacion ?? null, fechaAutorizacion: auth.fechaAutorizacion ?? null, mensaje: mensajeAutorizacion };
}
