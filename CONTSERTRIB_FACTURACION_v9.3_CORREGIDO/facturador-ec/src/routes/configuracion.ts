...
export async function registrarRutasConfiguracion(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { emisorId?: string } }>('/notificaciones', async (request, reply) => {
    ...
    const [stock, rechazados, correos, leidas] = await Promise.all([
      ...
      supabase.from('comprobantes').select('id,secuencial,motivo_error,created_at').eq('emisor_id', emisorId).eq('estado','rechazado').order('created_at',{ascending:false}).limit(20),
    ...
    for (const e of correos.data ?? []) { const id=`correo:${e.id}`; items.push({id,tipo:'correo',prioridad:'media',titulo:'Correo no enviado',detalle:e.detalle??`No se pudo enviar a ${e.destinatario}.`,fecha:e.created_at,leida:leidasSet.has(id)}); }
  ...
  app.post<{ Body: { emisorId?: string } }>('/notificaciones/leer-todas', async (request, reply) => {
    ...
    const [stock, rechazados, correos] = await Promise.all([
      ...
      supabase.from('comprobantes').select('id').eq('emisor_id', emisorId).eq('estado','rechazado').limit(50),
    ...
    for (const e of correos.data ?? []) ids.push(`correo:${e.id}`);
