...
function construirResultadoDesdeAutorizacion(
  ...
  return {
    ...
    mensaje: autorizacion.mensaje ?? autorizacion.mensajes?.map((m: any) => `${m.identificador ?? ''}: ${m.mensaje ?? ''}${m.informacionAdicional ? ' — ' + m.informacionAdicional : ''}`).filter(Boolean).join('; ') ?? undefined,
  } as unknown as EmissionResult;
}
