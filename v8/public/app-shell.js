(function () {
  ...
  function renderShell(negocio) {
    ...
    aside.innerHTML = `
      <div class="ec-brand"><div class="ec-brand-icon">${icon('invoice')}</div><div><strong>Facturador EC</strong><small>Sistema de Facturación Electrónica</small></div></div>
    ...
    header.innerHTML = `<button class="ec-menu-toggle" id="ec-menu-toggle" aria-label="Abrir menú">${icon('menu')}</button><div class="ec-head-logo"><div class="ec-brand-icon">${icon('invoice')}</div><b>Facturador EC</b></div><label class="ec-search"><span>${icon('search')}</span><input id="ec-global-search" placeholder="Buscar facturas, clientes, productos..." autocomplete="off"></label><div class="ec-head-spacer"></div><button class="ec-head-business" id="ec-business-button"><span class="ec-head-business-icon">${icon('box')}</span><span><strong>${esc(negocio.nombreComercial || negocio.razonSocial || 'Mi empresa')}</strong><small>RUC ${esc(negocio.ruc || '')}</small></span>${icon('chevron','ec-caret')}</button><button class="ec-notify" title="Notificaciones" aria-label="Notificaciones">${icon('bell')}<b>3</b></button><div class="ec-head-user"><span class="ec-avatar">${esc((negocio.usuarioNombre || negocio.rol || 'U').slice(0,1).toUpperCase())}</span><div><strong>${esc(negocio.usuarioNombre || 'Usuario')}</strong><small>${esc(negocio.rol || 'Administrador')}</small></div>${icon('chevron','ec-caret')}</div>`;
    ...
    document.querySelector('.ec-notify')?.addEventListener('click',async()=>{try{const r=await fetch('/notificaciones?emisorId='+encodeURIComponent(emisorId));const d=await r.json();const items=d.items||[];const msg=items.length?items.slice(0,12).map((x,i)=>`${i+1}. ${x.titulo}: ${x.detalle}`).join('\n\n'):'No tienes notificaciones pendientes.';alert('🔔 NOTIFICACIONES\n\n'+msg)}catch(e){alert('No se pudieron cargar las notificaciones.')}});
  }
