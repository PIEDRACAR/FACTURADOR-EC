...
export async function emitirDocumentoSri(emisorId: string, tipo: string, datos: Record<string, unknown>): Promise<EmissionResult> {
  ...
  return { estado: auth.estado, ambiente: ambiente === '2' ? 'produccion':'pruebas', claveAcceso: clave, secuencial, xmlOriginal: xml, xmlFirmado: firmado, numeroAutorizacion: auth.numeroAutorizacion ?? null, fechaAutorizacion: auth.fechaAutorizacion ?? null, mensaje: auth.mensaje } as any;
}
