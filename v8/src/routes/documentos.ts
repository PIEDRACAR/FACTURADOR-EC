...
export async function registrarRutasDocumentos(app: FastifyInstance) {
  ...
  app.post<{Body:{emisorId?:string;tipo?:string;clienteId?:string;comprobanteSustentoId?:string;datos?:Record<string,unknown>}}>('/documentos', async (request,reply)=>{
  ...
  app.post<{Params:{id:string}}>('/documentos/:id/emitir', async (request, reply) => {
    ...
    try {
      const result:any = await emitirDocumentoSri(doc.emisor_id, doc.tipo, normalizarDatosDocumento(doc.tipo, doc.datos ?? {}));
