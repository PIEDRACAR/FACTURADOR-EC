...
export async function renderizarRidePdf(c: DatosRide['comprobante']): Promise<Buffer> {
  ...
  // Zona reservada al logo: idéntica a los comprobantes complementarios.
  doc.roundedRect(L+10,22,250,58,4).fill(UI.blueSoft).strokeColor(UI.line).stroke();
  if(hasLogo){ try{doc.image(Buffer.from(c.logo_ride_base64!,'base64'),L+18,28,{fit:[234,46],valign:'center'});}catch{} }
  else{doc.fillColor(UI.blue).font('Helvetica-Bold').fontSize(11).text('CONTSERTRIB',L+20,43,{width:230,align:'center'});doc.font('Helvetica').fontSize(7).fillColor(UI.muted).text('FACTURACIÓN ELECTRÓNICA',L+20,59,{width:230,align:'center'});}

  // Bloque del emisor, debajo de la marca, como en el RIDE de referencia.
  doc.roundedRect(L,86,leftW,116,4).lineWidth(.8).strokeColor(UI.line).stroke();
  ...
  doc.font('Helvetica-Bold').fontSize(6.7).fillColor(UI.navy).text(autorizado?'DOCUMENTO ELECTRÓNICO':'BORRADOR PARA REVISIÓN',L+10,184,{width:leftW-20,align:'center'});
  ...
  if(c.clave_acceso){try{const barras=await generarCodigoBarras(c.clave_acceso);doc.image(barras,boxX+14,fy+41,{width:boxW-28,height:26});}catch{}}
  doc.font('Helvetica-Bold').fontSize(6.1).fillColor(UI.muted).text('CLAVE DE ACCESO',boxX+10,fy+69,{width:boxW-20,align:'center'});
  doc.font('Helvetica').fontSize(5.7).fillColor(UI.ink).text(c.clave_acceso||'—',boxX+8,fy+79,{width:boxW-16,height:9,align:'center',characterSpacing:.08,ellipsis:true});
  if(!autorizado){doc.roundedRect(L+8,188,leftW-16,10,2).fill('#FEF3C7');doc.fillColor('#A16207').font('Helvetica-Bold').fontSize(6.1).text('BORRADOR · SIN VALIDEZ TRIBUTARIA',L+10,190,{width:leftW-20,align:'center'});}
  ...
  // La clave de acceso y el código de barras quedan exclusivamente en el bloque superior.
