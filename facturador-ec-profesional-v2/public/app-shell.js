(function () {
  ...
  function renderShell(negocio) {
    ...
    aside.innerHTML = `
      <div class="ec-brand"><span class="ec-brand-icon">🧾</span><div><strong>Facturador EC</strong><small>SRI Ecuador</small></div></div>
      <div class="ec-business"><small>NEGOCIO ACTIVO</small><strong>${escapeHtml(negocio.nombreComercial || negocio.razonSocial)}</strong><span>${escapeHtml(negocio.rol || '')}</span></div>
      <nav class="ec-menu">
        ${modulo.map(([href,icon,label]) => `<a class="${path===href?'active':''}" href="${href}?emisorId=${q}"><span>${icon}</span>${label}</a>`).join('')}
      </nav>
